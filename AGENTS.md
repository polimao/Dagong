...
- This is an `Electron + React + TypeScript` desktop app. The product name is `Dagong`; the top-level build entries are `package.json`, `electron.vite.config.ts`, and `electron-builder.config.cjs`.
- The only active agent runtime is the bundled `dagong/` TypeScript package. The GUI talks to `dagong serve` over local `HTTP + SSE`; the renderer does not run the agent loop directly.
- The main data path is `Renderer -> preload -> main -> Dagong runtime`:
  - `src/renderer/src`: React workbench, Code/Design/Write/Connect phone UI, and Zustand state.
  - `src/preload`: the constrained IPC bridge exposed to the renderer as `window.dagongGui`.
  ...
  - `dagong/`: `dagong serve`, HTTP routes, thread/session stores, tool execution, model adapters, agent loop, cache, and usage tracking.
- The single-runtime plan and the legacy surfaces that must stay removed are documented in `docs/AGENTS.md` and `docs/dagong-architecture.md`. Read both before changing runtime, agent selector, diagnostics, or legacy provider behavior.

## Code Organization

- Put new UI interactions in `src/renderer/src`, Electron/system integration in `src/main`, cross-layer contracts in `src/shared`, and agent runtime behavior in `dagong/src`.
...
- For runtime HTTP/SSE issues, start with:
  - `src/main/runtime/dagong-adapter.ts`
  - `src/main/runtime-sse-ipc.ts`
  - `src/preload/index.ts`
  - `src/renderer/src/agent/dagong-runtime.ts`
  - `src/renderer/src/agent/dagong-mapper.ts`
  - `dagong/src/server/runtime-factory.ts`
  - `dagong/src/server/routes/`
- Do not recreate old runtime paths, provider switchers, runtime diagnostics panels, CodeWhale/Reasonix adapters, process managers, or RPC bridges. Legacy provider fields should only be read during settings migration and folded into `agents.dagong`.
...
- Endpoint behavior is a shared contract, not a local detail of one caller. When changing `baseUrl`, `endpointFormat`, request bodies, or provider presets, inspect all relevant consumers:
  ...
  - `src/shared/app-settings-dagong.ts`
  ...
  - `dagong/src/adapters/model/deepseek-compat-model-client.ts`
  - `dagong/src/loop/agent-loop.ts`
...
- For model request 404s, user-facing guidance should point at provider configuration, especially Base URL and Endpoint format. Dagong logs should include sanitized `baseUrl`, final `requestUrl`, provider, model, `endpointFormat`, HTTP status, and a summarized response body.
...
## Dagong Runtime Notes

- `dagong/src/server/runtime-factory.ts` is the runtime composition root. In production serve mode, the event bus, thread/session stores, and runtime event recorder are wired there.
...
- The agent loop is in `dagong/src/loop/agent-loop.ts`. Tool calls, approvals, user input, plan/goal handling, model-history repair, usage, and cache accounting can all pass through it.
...
- Settings -> Agents should show only Dagong configuration. Do not restore the agent switcher, connection status bar, runtime diagnostics dialog, runtime insights panel, or `/usage` and `/runtime` runtime-control commands.
...
- Image and attachment paths must be propagated from renderer to Dagong through contract fields such as `localFilePath` / `FilePath`. Appending a path only in final text is usually insufficient; check renderer, main bridge, attachment store, agent loop, and model fallback.
- The current bridge name is `window.dagongGui`. Do not use the old `window.dsGui` name.
...
- Build: `npm run build`
- Lint: `npm run lint`
- Changes under `dagong/` should pass `npm run build:dagong`; top-level `dev` and `build` both run it first.
...
- For local macOS arm64 testing, unzip `dist/Dagong-0.1.0-mac-arm64.zip` after packaging and test the extracted app, usually at `dist/Dagong-0.1.0-mac-arm64-unzipped/Dagong.app`.
- Before a new packaged-app test, make sure old `Dagong.app` processes have exited. Multiple worktree builds share the same bundle identifier, so macOS may reuse an old instance.
- Startup debugging should not rely on a single port check. Combine the real process path, `open -n <Dagong.app>`, `lsof -iTCP:18787,18788,18899`, `curl http://127.0.0.1:18899/health`, and `~/Library/Application Support/Dagong/logs/dagong-*.log`.
- Port `18787` may be occupied by a non-Dagong process. Keep it separate from Dagong runtime/health ports such as `18788` and `18899`.
...
- DeepSeek-GUI/Dagong PRs should target `develop` explicitly. Do not rely on the GitHub default branch, and do not accidentally open PRs against `master`.
...
- Settings save/runtime behavior issue: inspect `src/shared/app-settings-provider.ts`, `src/shared/app-settings-dagong.ts`, settings store migration, and the renderer settings section.
...
- `/new`, session resume, fork, archive/search issue: trace the full path from renderer command/store to Dagong routes instead of changing only the UI command.
