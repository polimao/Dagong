...
MagicPocket is the local HTTP/SSE agent runtime for the MagicPocket desktop app. It exposes a
TypeScript-typed agent loop with a stable, GUI-friendly contract:

- `magicpocket serve` starts a local HTTP server with `/v1/*` routes.
...
The name MagicPocket is inspired by the great fish in Zhuangzi's line,
"In the northern sea there is a fish; its name is MagicPocket." In
...
MagicPocket's core goal is to improve the ROI of every token. Tokens should be
...
Run from the `magicpocket/` directory.
...
Start a MagicPocket runtime, set `KUN_RUNTIME_URL` and `KUN_RUNTIME_TOKEN`, then run the five-task smoke set:
...
TTFT, full latency, tool time, SSE delivery delay, token/cache/cost counters, and MagicPocket process peak RSS. The runtime
token is accepted only through `KUN_RUNTIME_TOKEN`, so it does not leak through process arguments.

## CLI

`magicpocket serve` accepts the following flags:

| Flag | Description | Default |
| --- | --- | --- |
| `--config` | JSON config file. If omitted, MagicPocket reads `{--data-dir}/config.json` when present | optional |
...
```bash
magicpocket serve \
  --config ~/.deepseekgui/magicpocket/config.json \
  --host 127.0.0.1 \
  --port 18899 \
  --data-dir ~/.deepseekgui/magicpocket \
  --runtime-token dev-token \
  --api-key "$DEEPSEEK_API_KEY" \
  --model deepseek-v4-pro
```

MagicPocket can also run as a standalone agent without the GUI:

```bash
magicpocket run --data-dir ~/.deepseekgui/magicpocket --workspace "$PWD" "summarize this repo"
magicpocket chat --data-dir ~/.deepseekgui/magicpocket --workspace "$PWD"
magicpocket exec --data-dir ~/.deepseekgui/magicpocket --workspace "$PWD" --list-tools
magicpocket exec --data-dir ~/.deepseekgui/magicpocket --workspace "$PWD" read --args '{"path":"README.md"}'
```

- `magicpocket run` creates a thread, runs one turn, streams assistant text, and exits.
- `magicpocket chat` starts a line-oriented REPL. Use `/exit`, `/quit`, or an empty line to stop.
- `magicpocket exec --list-tools` prints the effective dynamic tool registry for the chosen config/workspace.
- `magicpocket exec <tool> --args <json>` invokes one tool directly. Use `--json` on `run` or `exec` for machine-readable output.
...
MagicPocket supports a JSON config file so runtime behavior can be managed
...
MagicPocket also reads `{data-dir}/config.json` when it exists. In the GUI's
default setup this is:

```text
~/.deepseekgui/magicpocket/config.json
```
...
```json
{
  "serve": {
    "host": "127.0.0.1",
    "port": 18899,
    "dataDir": "~/.deepseekgui/magicpocket",
    "runtimeToken": "",
    "apiKey": "",
...
MagicPocket defaults to hybrid session storage: `threads/{threadId}/messages.jsonl`
...
- `capabilities.mcp` starts configured MCP clients and imports their tools into the dynamic registry. Workspace-scoped servers require `trustedWorkspaceRoots`. Remote HTTP/SSE servers can use `oauth`; MagicPocket stores OAuth tokens under the data directory instead of the config file. Use `GET /v1/mcp/oauth` to inspect redacted OAuth state and `DELETE /v1/mcp/oauth/{serverId}` to clear a server's saved authorization.
...
lifecycle without rebuilding MagicPocket. They are configured under the
top-level `hooks` key in `config.json` (so the GUI's
`~/.deepseekgui/magicpocket/config.json` works out of the box) and run inside
...
```json
{
  "hooks": [
    ...
    { "phase": "TurnEnd", "command": "~/.magicpocket-hooks/notify.sh" }
  ]
}
```
...
See `../docs/magicpocket-hooks.en.md` for the full reference: per-phase stdin
...
```
{--data-dir}/
  config.json      # Optional MagicPocket runtime config
...
3. Restart MagicPocket or refresh diagnostics.
...
- Memory is not injected: enable `capabilities.memory`, confirm
  ...
  `lastInjectedIds`.
- `magicpocket run`, `magicpocket chat`, or `magicpocket exec` cannot authenticate or load
  config: pass the same `--config`, `--data-dir`, `--api-key`,
  `--base-url`, and `--runtime-token` values used by `magicpocket serve`.
  `magicpocket exec --list-tools --json` is the quickest way to verify the
...
starts MagicPocket through `magicpocket-process.ts` and routes all
...
CodeWhale provider because MagicPocket speaks the same HTTP/SSE
contract. Settings live under `agents.magicpocket` in
...
`agents.magicpocket`; the saved settings file no longer keeps live
