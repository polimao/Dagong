...
export class HybridThreadStore implements ThreadStore {
  ...
  private async maybeCompactMetadata(threadId: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `[magicpocket] metadata compaction skipped for ${threadId}: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
...
function warnSqlite(action: string, error: unknown): void {
  ...
  console.warn(`[magicpocket] hybrid sqlite ${action} failed; using JSONL fallback: ${message}`)
}
