...
const TOOL_DESCRIPTION = [
  ...
  '.magicpocketsdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .magicpocketsdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.magicpocketsdd/plan/foo.md`)
 ...
 *    `.magicpocketsdd/plan/<feature>.md` under the active workspace.
...
function resolveReservedTarget(
  ...
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .magicpocketsdd/plan' }
  }
 ...
 * allocates a fresh, non-colliding `.magicpocketsdd/plan/<feature>.md`.
 */
