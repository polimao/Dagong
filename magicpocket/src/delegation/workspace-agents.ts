 ...
 *
 * Loads `<workspace>/.magicpocket/agents/*.md` and produces a profile map that
 ...
 *     color: "#3b82f6"
 *     ---
 *     Body becomes the systemPrompt verbatim (magicpocket's base prompt is
...
export async function loadWorkspaceAgentProfiles(workspace: string): Promise<WorkspaceAgentProfile[]> {
  if (!workspace) return []
  const dir = join(workspace, '.magicpocket', 'agents')
  let entries: string[]
  try {
