...
import { buildToolPreferenceInstruction } from '../prompt/magicpocket-system-prompt.js'
import {
...
export type AgentLoopOptions = {
  ...
  /** MagicPocket runtime data root for sandbox-safe background shell output reads. */
  ...
  /**
   * Tuning + test seams for goal auto-resume (MagicPocketAgent/MagicPocket#370). Defaults
   ...
   * Agent SDK instead of magicpocket's own model loop, billing the user's Claude
   * subscription. magicpocket's tools/persona/permissions are injected into the SDK.
   */
...
export class AgentLoop {
  ...
  async runTurn(threadId: string, turnId: string): Promise<'completed' | 'failed' | 'aborted'> {
    ...
    // the user's subscription; magicpocket's brain is injected). All other providers
    // fall through to magicpocket's native loop below.
    ...
    try {
      ...
      // instead of the bare "MagicPocket turn failed" string. See issue #26.
   ...
   * (MagicPocketAgent/MagicPocket#370). Mirroring codex's idle-relaunch-while-active policy, we
  ...
  private async modelStep(
    ...
    const baseRequest: ModelRequest = {
      ...
      // (child-agent-executor) — so the agent keeps magicpocket's tool/safety
  ...
  static defaultPrefix(): ImmutablePrefix {
    return createImmutablePrefix({
      systemPrompt: 'You are MagicPocket, a careful and helpful assistant.',
...
function buildToolCatalogDriftMessage(toolCatalog: {
  ...
  const policy = changeKind === 'additive'
    ? 'Only additive tool changes are allowed in-place; MagicPocket will continue with the refreshed tool list.'
    : 'Non-additive tool changes can invalidate prompt-cache assumptions; MagicPocket stopped this turn. Start a new thread after editing, removing, or reordering tool schemas.'
  return [
