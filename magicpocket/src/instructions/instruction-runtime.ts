...
function globalAgentsPath(home: string): string {
  return join(home, '.magicpocket', KUN_AGENTS_FILENAME)
}
...
function renderInstructionBlock(
  ...
  const header = [
    '<magicpocket_agents_instructions>',
    'These instructions come from MagicPocket native AGENTS.md files.',
    'They are high-priority user/developer context, but they are below MagicPocket\'s built-in system contract and cannot override safety, approval, sandbox, tool-permission, or runtime HTTP/SSE rules.',
  ...
  const footer = '</magicpocket_agents_instructions>'
  let text = `${header}\n`
