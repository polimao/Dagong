...
describe('SdkEventMapper', () => {
  ...
  test('maps a tool_use block to item_created + tool_call_ready', () => {
    ...
    const events = m.map({
      ...
      message: {
        role: 'assistant',
        content: [{ type: 'tool_use', id: 'toolu_1', name: 'mcp__magicpocket__generate_image', input: { prompt: 'a cat' } }]
      }
    } as SdkMessage)
  ...
  test('maps a tool_result back to tool_call_finished, recovering the tool name', () => {
    ...
    m.map({
      ...
      message: {
        role: 'assistant',
        content: [{ type: 'tool_use', id: 'toolu_9', name: 'mcp__magicpocket__web_search', input: {} }]
      }
    } as SdkMessage)
    const events = m.map({
      type: 'user',
    ...
    expect(events[0]).toMatchObject({
      ...
      item: {
        kind: 'tool_result',
        toolName: 'mcp__magicpocket__web_search',
        callId: 'toolu_9',
        output: 'results...',
        isError: false
      }
