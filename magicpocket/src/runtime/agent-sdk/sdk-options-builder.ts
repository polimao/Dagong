/**
 * Assembles the `query()` options for a magicpocket subscription turn — the glue that
 * injects magicpocket's brain (persona, tools, permissions) into the SDK's loop.
 *
 * The assembly is pure and unit-tested. The two callbacks it carries
 * (`canUseTool`, hook callbacks) are factories that close over magicpocket's real
 ...
 * deliberately did NOT bridge from magicpocket). Listed in allowedTools so they are
...
/**
 * Claude Code built-in tools we suppress on the magicpocket-driven SDK path.
 * AskUserQuestion has no UI in this embedding (the model would ask and get no
 * answer); magicpocket's own bridged `user_input` panel handles interactive questions.
 */
...
/**
 * Map magicpocket's ApprovalPolicy onto the SDK permission mode. magicpocket's fine-grained
...
/**
 * Claude Code (the subscription engine) only accepts Anthropic models. A magicpocket
...
/** Compose magicpocket's persona append text for the claude_code system-prompt preset. */
export function buildClaudeSystemPrompt(
  magicpocketSystemPrompt: string,
  threadPersona?: string
): SdkSystemPromptPreset {
  const base = magicpocketSystemPrompt.trim()
...
/** magicpocket's permission decision for a (toolName, input) pair on the active turn. */
...
/**
 * Bridge magicpocket's approval engine to the SDK `canUseTool` callback. Every tool the
 * SDK is about to run is adjudicated by magicpocket (which can route to the GUI
  ...
  return async (toolName, input): Promise<SdkPermissionResult> => {
    ...
    try {
      ...
      if (decision.allow) {
        ...
        // Echo the original input through when magicpocket doesn't rewrite it.
        return { behavior: 'allow', updatedInput: decision.updatedInput ?? safeInput }
      }
      // The deny variant requires a non-empty `message`.
      return {
        behavior: 'deny',
        message: decision.message ?? 'Denied by magicpocket permission policy',
...
export interface AssembleSdkOptionsParams {
  ...
  planMode?: boolean
  /** `mcp__magicpocket__*` names from the tool bridge. */
...
export function assembleSdkOptions(params: AssembleSdkOptionsParams): SdkQueryOptions {
  ...
  const options: SdkQueryOptions = {
    cwd: params.cwd,
    systemPrompt: buildClaudeSystemPrompt(params.magicpocketSystemPrompt, params.threadPersona),
    allowedTools,
    ...
    // Only load magicpocket-provided config; don't auto-absorb the host's ~/.claude.
