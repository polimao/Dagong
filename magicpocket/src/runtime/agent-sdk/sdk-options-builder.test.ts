...
describe('buildClaudeSystemPrompt', () => {
  test('appends magicpocket persona onto the claude_code preset', () => {
    const sp = buildClaudeSystemPrompt('You are magicpocket.', 'Persona: terse.')
    expect(sp).toEqual({ type: 'preset', preset: 'claude_code', append: 'You are magicpocket.\n\nPersona: terse.' })
  })
  test('omits persona when absent', () => {
    expect(buildClaudeSystemPrompt('You are magicpocket.').append).toBe('You are magicpocket.')
  })
})
...
describe('buildCanUseTool', () => {
  test('allow passes through, with optional updatedInput', async () => {
    ...
    // when magicpocket doesn't rewrite, it echoes the original input through.
...
describe('assembleSdkOptions', () => {
  const base = {
    cwd: '/ws',
    magicpocketSystemPrompt: 'You are magicpocket.',
    approvalPolicy: 'on-request' as const,
    bridgedToolModelNames: ['mcp__magicpocket__generate_image', 'mcp__magicpocket__memory_create'],
    baseEnv: { ANTHROPIC_API_KEY: 'k', PATH: '/bin' },
    oauthToken: 'sk-ant-oat01-z'
  }

  test('unions SDK built-ins with bridged magicpocket tools and turns on partial streaming', () => {
    ...
    expect((opts.systemPrompt as { append: string }).append).toBe('You are magicpocket.')
