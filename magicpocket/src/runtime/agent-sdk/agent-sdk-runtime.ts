 ...
 * bundled Claude Code binary) while injecting magicpocket's brain — persona, exclusive
 * tools, permissions — and re-projecting the SDK's stream onto magicpocket's events.
 *
 * The orchestration depends only on the injected `SdkRuntimeDeps` seam, so it is
 * fully unit-testable with a fake SDK + fake deps. The concrete binding to magicpocket's
...
export interface SdkTurnContext {
  ...
  /** magicpocket tool catalog to consider bridging (overlap/excluded are filtered here). */
  bridgeableTools: BridgeableTool[]
  /**
   * Prior-conversation transcript replayed each turn so the model has magicpocket's
...
export interface SdkRuntimeDeps {
  ...
  /** Execute a magicpocket tool in-process (raw — permission/hooks handled by the SDK seam).
   *  `signal` aborts in-flight interactive work (e.g. a pending user_input). */
  executeMagicPocketTool(
    threadId: string,
    turnId: string,
    toolName: string,
    args: Record<string, unknown>,
    signal?: AbortSignal
  ): Promise<MagicPocketToolResult>
  /** magicpocket's per-call permission decision (routes to the GUI approval panel). */
  decideToolApproval(
    threadId: string,
    turnId: string,
    toolName: string,
  ...
  /** The stable magicpocket system prompt (persona) appended to the claude_code preset. */
...
export class AgentSdkRuntime {
  ...
  async runTurn(threadId: string, turnId: string, signal: AbortSignal): Promise<TurnStatus> {
    ...
    try {
      ...
      // Bridge magicpocket-exclusive tools into an in-process MCP server.
      const bridged = buildBridgedToolSpecs(selectBridgeableTools(ctx.bridgeableTools), (name, args) =>
        this.deps.executeMagicPocketTool(threadId, turnId, name, args, abort.signal)
      )
      const mcpServers = bridged.length ? { magicpocket: toSdkMcpServer(sdk, bridged) } : undefined

      const options = assembleSdkOptions({
        cwd: ctx.workspace,
        magicpocketSystemPrompt: this.deps.magicpocketSystemPrompt(),
        threadPersona: ctx.threadPersona,
        approvalPolicy: ctx.approvalPolicy,
        // Deliberately NOT mapping magicpocket's plan turn to the SDK's 'plan' permission
        // mode: that mode blocks tool execution, which would also block magicpocket's
        // bridged create_plan tool (the whole point of a plan turn). magicpocket's plan
      ...
      // magicpocket owns canonical history, so each SDK turn is stateless: replay the
