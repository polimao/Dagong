/**
 * Translates the Claude Agent SDK message stream into magicpocket's native runtime
 * events, so a subscription turn renders in the GUI exactly like a turn driven
 * by magicpocket's own agent loop. This is the load-bearing half of the fusion: the SDK
 * owns the loop, but every assistant token, tool call, and tool result it emits
 * is re-projected onto magicpocket's event contract.
 *
 ...
 *
 * Streaming model (mirrors magicpocket's native loop exactly, or the GUI double-renders):
 * a magicpocket `assistant_text_delta` event carries an INCREMENTAL CHUNK (the GUI
...
/** Claude Code built-in tool names that imply a richer magicpocket tool kind. */
...
/** Collapse an SDK tool_result content payload into a magicpocket tool output value. */
...
/**
 * Map the Agent SDK's per-request usage onto magicpocket's UsageSnapshot. Anthropic's
