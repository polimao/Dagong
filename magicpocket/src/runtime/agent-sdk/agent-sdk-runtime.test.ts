...
function makeDeps(overrides: Partial<SdkRuntimeDeps> = {}): {
  ...
  const deps: SdkRuntimeDeps = {
    ...
    executeMagicPocketTool: async () => ({ output: 'tool-ok' }),
  ...
  } as SdkMessage,
  {
    ...
    message: {
      ...
      content: [
        ...
        { type: 'tool_use', id: 'toolu_1', name: 'mcp__magicpocket__generate_image', input: { prompt: 'cat' } }
      ]
    }
  } as SdkMessage,
  {
...
describe('AgentSdkRuntime.runTurn', () => {
  test('drives the SDK stream into magicpocket events/items and completes the turn', async () => {
  ...
  test('a query failure records an error event and fails the turn', async () => {
    const { deps, events, finished } = makeDeps({
      loadSdk: async () => ({
        ...
        createSdkMcpServer: () => ({ type: 'sdk', name: 'magicpocket', instance: {} }),
        tool: () => ({})
      })
    })
    const status = await new AgentSdkRuntime(deps).runTurn('th', 'tn', new AbortController().signal)
