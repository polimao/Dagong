/**
 * Pure assembly of the per-turn context that magicpocket injects into a subscription
 * (Claude Agent SDK) turn. The SDK owns the loop, but — unlike magicpocket's native
 * loop — it does NOT see magicpocket's conversation history, skill catalog, memories, or
 ...
 *
 * Design: magicpocket owns the canonical history, so every SDK turn is stateless from
 * magicpocket's side — we replay the prior conversation as a transcript preamble each
