/**
 * Binds the decoupled {@link AgentSdkRuntime} to magicpocket's real runtime services.
 * This is the only place that touches the SDK package and magicpocket's concrete stores,
...
import type { BridgeableTool, MagicPocketToolResult } from './sdk-tool-bridge.js'
...
import type { ServeProviderConfig } from '../../config/magicpocket-config.js'
...
export interface AgentSdkRuntimeFactoryDeps {
  ...
  /** Native MagicPocket AGENTS.md instruction engine — injects global/workspace instructions per turn. */
  instructionRuntime?: InstructionRuntime
  /** Long-term memory store — injects relevant memories per turn. */
  memoryStore?: MemoryStore
  /** Interactive-input gate — lets the bridged `user_input` tool surface magicpocket's GUI panel. */
...
/** Lazily load the real SDK without a static import (so magicpocket typechecks without it). */
 ...
 * GUI plan — the SDD "下一步"/Plan-mode flow — we must expose it so the magicpocket
...
export function createAgentSdkRuntime(deps: AgentSdkRuntimeFactoryDeps): AgentSdkRuntime {
  ...
  // resume from it: magicpocket owns the canonical history and replays it as a transcript
  ...
  /**
   * Bridge magicpocket's `user_input` tool to its GUI panel: persist the request item +
  ...
  const toolContext = (
    ...
    // (executeMagicPocketTool) on plan turns — both are gated on it.
    ...(opts?.planMode ? { threadMode: 'plan' as const } : {}),
    ...(opts?.guiPlan ? { guiPlan: opts.guiPlan } : {}),
    // Wire interactive input to magicpocket's GUI panel (advertises `user_input`).
  ...
  const runtimeDeps: SdkRuntimeDeps = {
    ...
    async loadTurnContext(threadId, turnId): Promise<SdkTurnContext | null> {
      ...
      // Plan turns expose create_plan (and narrow magicpocket tools to the plan-allowed
      ...
      // The SDK doesn't see magicpocket's history or per-turn context, so assemble both
      // here (parity with the native loop's `contextInstructions`). magicpocket owns the
    ...
    async executeMagicPocketTool(threadId, turnId, toolName, args, signal): Promise<MagicPocketToolResult> {
      const thread = await deps.threadStore.get(threadId)
    ...
    baseEnv: () => process.env,
    magicpocketSystemPrompt: () => deps.prefix.systemPrompt,
