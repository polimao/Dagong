...
import {
  ...
  mapMagicPocketResultToSdkContent,
...
describe('selectBridgeableTools', () => {
  test('drops Claude Code overlap tools and excluded tools, keeps magicpocket-exclusive', () => {
    const tools = [
      tool('read'),
      tool('bash'),
      tool('edit'),
    ...
    // now bridged so magicpocket's GUI input panel handles interactive questions.
...
describe('mapMagicPocketResultToSdkContent', () => {
  test('passes string output through', () => {
    expect(mapMagicPocketResultToSdkContent({ output: 'hello' })).toEqual({
      content: [{ type: 'text', text: 'hello' }]
    })
  })

  test('JSON-stringifies structured output', () => {
    const out = mapMagicPocketResultToSdkContent({ output: { a: 1 } })
    expect(out.content[0].text).toContain('"a": 1')
  })

  test('flags errors', () => {
    expect(mapMagicPocketResultToSdkContent({ output: 'bad', isError: true })).toMatchObject({ isError: true })
  })
})

describe('buildBridgedToolSpecs', () => {
  test('handler invokes the magicpocket executor and maps the result', async () => {
  ...
  test('a magicpocket isError result is propagated', async () => {
...
describe('bridgedToolModelNames', () => {
  test('namespaces under mcp__magicpocket__', () => {
    const specs = buildBridgedToolSpecs([tool('memory_create'), tool('lsp')], async () => ({ output: '' }))
    expect(bridgedToolModelNames(specs)).toEqual(['mcp__magicpocket__memory_create', 'mcp__magicpocket__lsp'])
  })
})
