...
import { MagicPocketCapabilitiesConfig } from '../../contracts/capabilities.js'
...
const planTurn = (id: string, workspaceRoot: string): ThreadRecord['turns'][number] =>
  ({
    id,
    prompt: 'plan it',
    guiPlan: { operation: 'draft', workspaceRoot, relativePath: '.magicpocket/plan.md', planId: 'p1' }
...
describe('resolveTurnPlanContext', () => {
  test('exposes the GUI plan + planMode for a plan turn in the same workspace', () => {
    ...
    expect(resolved.guiPlan?.relativePath).toBe('.magicpocket/plan.md')
...
describe('createAgentSdkRuntime turn context', () => {
  test('injects native AGENTS.md instructions and records turn metadata', async () => {
    const root = await mkdtemp(join(tmpdir(), 'magicpocket-sdk-instructions-'))
    try {
      ...
      const runtime = createAgentSdkRuntime({
        ...
        instructionRuntime: new InstructionRuntime(
          MagicPocketCapabilitiesConfig.parse({ instructions: { enabled: true } }).instructions,
          { homeDir: home }
        )
      })
