...
import { createMagicPocketServeRuntime } from '../server/runtime-factory.js'
...
export const KUN_CLI_USAGE = `magicpocket <command> [options]
...
Common options:
  ...
  --data-dir <path>          Root directory for MagicPocket data
  --workspace <path>         Workspace root for run/chat/exec
  --model <model>            Model id
  --approval-policy <p>      on-request | untrusted | never | auto | suggest
  --json                     Emit machine-readable JSON where supported
  --jsonl                    Stream one machine-readable event per line for magicpocket run
...
export type MagicPocketCliCommand = 'serve' | 'run' | 'chat' | 'exec' | 'help'

export function splitMagicPocketCliCommand(argv: readonly string[]): {
  command: MagicPocketCliCommand
  args: string[]
  error?: string
} {
  const first = argv[0]
...
export async function runAgentCommand(
  command: Exclude<MagicPocketCliCommand, 'serve' | 'help'>,
...
async function runOneShot(argv: readonly string[], io: CliIo): Promise<number> {
  ...
  if (!parsed.ok) return writeParseError(parsed, io, 'magicpocket run')
  const jsonl = hasFlag(argv, 'jsonl')
  if (parsed.json && jsonl) {
    io.stderr.write('magicpocket run: --json and --jsonl are mutually exclusive\n')
    return ServeExitCode.usage
  }
  const prompt = stringFlag(argv, ['prompt', 'p']) ?? positionals(argv).join(' ').trim()
  if (!prompt) {
    io.stderr.write('magicpocket run: missing prompt\n')
    return ServeExitCode.usage
  }
  ...
  try {
    ...
    io.stderr.write(`magicpocket run: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'magicpocket run')
  }
}
...
async function runChat(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'magicpocket chat')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
    const thread = await runtime.threadService.create({
    ...
    io.stderr.write(`magicpocket chat: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'magicpocket chat')
  }
}
...
async function runExec(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'magicpocket exec')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
  } catch (error) {
    io.stderr.write(`magicpocket exec: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  }
  ...
  try {
    ...
    if (!toolName) {
      io.stderr.write('magicpocket exec: missing tool name (use --list-tools to inspect tools)\n')
      return ServeExitCode.usage
    }
    const argsText = stringFlag(argv, ['args']) ?? '{}'
    const args = parseJsonObject(argsText)
    if (!args.ok) {
      io.stderr.write(`magicpocket exec: ${args.message}\n`)
    ...
    await shutdownRuntime(runtime, io, 'magicpocket exec')
  }
}
...
function createRuntime(options: ServeOptions, io: CliIo): Promise<ServerRuntime> {
  return io.createRuntime ? io.createRuntime(options) : createMagicPocketServeRuntime(options)
}
