...
import {
  magicpocketConfigPathForDataDir,
  readMagicPocketConfigFile,
  readOptionalMagicPocketConfigFile,
  type LoadedMagicPocketConfig
} from '../config/magicpocket-config.js'

/**
 * Parse the `magicpocket serve` command line into validated options.
 *
 ...
 * main process when MagicPocket is started programmatically.
 */
...
export const SERVE_USAGE = `magicpocket serve [options]
...
function loadServeConfig(
  ...
  env: Record<string, string | undefined>
): LoadedMagicPocketConfig | null {
  const explicitConfigPath =
    stringFlag(raw, 'config') ??
    stringFlag(raw, 'config-file') ??
    env.KUN_CONFIG
  if (explicitConfigPath) {
    return readMagicPocketConfigFile(explicitConfigPath)
  }
  const dataDir = dataDirFromRawOrEnv(raw, env)
  return readOptionalMagicPocketConfigFile(magicpocketConfigPathForDataDir(dataDir))
}
