...
import {
  KUN_CLI_USAGE,
  runAgentCommand,
  splitMagicPocketCliCommand
} from './agent-cli.js'
import { startMagicPocketServe, type MagicPocketServeHandle } from '../server/runtime-factory.js'
import {
...
async function serveMain(argv: readonly string[]): Promise<number> {
  ...
  if (!parsed.ok) {
    process.stderr.write(`magicpocket serve: ${parsed.message}\n`)
    if (parsed.issues) {
      process.stderr.write(`${JSON.stringify(parsed.issues, null, 2)}\n`)
    }
    return parsed.exitCode
  }
  let handle: MagicPocketServeHandle | null = null
  installServeCrashHandlers(() => handle)
  const server = await startMagicPocketServe(parsed.options)
  handle = server
  ...
  const startupInfo = {
    ...
    pid: info.pid,
    message: `magicpocket runtime listening on http://${server.host}:${server.port}`
  }
...
/**
 * When the GUI launches magicpocket without `ELECTRON_RUN_AS_NODE` (the host
...
async function selfVerifyHealth(host: string, port: number): Promise<void> {
  ...
  process.stderr.write(
    `[magicpocket] warning: self-health-probe on http://${host}:${port}/health did not pass within ${SELF_VERIFY_TIMEOUT_MS}ms — proceeding anyway\n`
  )
}

export async function main(argv: readonly string[]): Promise<number> {
  await hideMacosDockIfRunningAsElectron()
  const command = splitMagicPocketCliCommand(argv)
  if (command.command === 'help') {
    if (command.error) {
      process.stderr.write(`magicpocket: ${command.error}\n`)
...
main(process.argv.slice(2)).then(
  ...
  (error) => {
    process.stderr.write(`magicpocket serve: ${String(error)}\n`)
