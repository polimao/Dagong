...
import type { MagicPocketServeHandle } from '../server/runtime-factory.js'
 ...
 */
export function installServeCrashHandlers(getHandle: () => MagicPocketServeHandle | null): void {
  let crashing = false
  const crash = (kind: string, error: unknown): void => {
    if (crashing) return
    crashing = true
    const detail = error instanceof Error ? (error.stack ?? error.message) : String(error)
    process.stderr.write(`magicpocket serve: ${kind}: ${detail}\n`)
  ...
  process.on('unhandledRejection', (reason) => {
    ...
    process.stderr.write(`magicpocket serve: unhandledRejection (non-fatal, runtime stays up): ${detail}\n`)
  })
}
