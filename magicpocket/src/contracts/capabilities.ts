...
export const SkillsCapabilityConfig = CapabilityToggleConfig.extend({
  ...
  workspaceRoots: z.array(z.string().min(1)).default([]),
  /** Global skill roots (e.g. ~/.magicpocket/skills). Scanned after project roots. */
...
export const MagicPocketCapabilitiesConfig = z
  .object({
  ...
  .strict()
export type MagicPocketCapabilitiesConfig = z.infer<typeof MagicPocketCapabilitiesConfig>

export const DEFAULT_KUN_CAPABILITIES_CONFIG: MagicPocketCapabilitiesConfig = MagicPocketCapabilitiesConfig.parse({})
...
export function buildRuntimeCapabilityManifest(input: {
  config?: MagicPocketCapabilitiesConfig
  model: ModelCapabilityMetadata
  mcp?: {
  ...
  const config = MagicPocketCapabilitiesConfig.parse(input.config ?? {})
