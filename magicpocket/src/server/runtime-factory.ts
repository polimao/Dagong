...
import {
  ...
  type MagicPocketCapabilitiesConfig
...
import { KUN_SYSTEM_PROMPT } from '../prompt/magicpocket-system-prompt.js'
...
export type MagicPocketServeRuntimeOptions = {
  host: string
  port: number
  configPath?: string
  dataDir: string
  ...
  capabilities?: MagicPocketCapabilitiesConfig
...
export type MagicPocketServeHandle = NodeHttpServerHandle & {
 ...
 */
export async function createMagicPocketServeRuntime(
  options: MagicPocketServeRuntimeOptions
): Promise<ServerRuntime> {
  await mkdir(options.dataDir, { recursive: true })
  let activeOptions: MagicPocketServeRuntimeOptions = { ...options }
  ...
  let prefix = createImmutablePrefix({
    ...
    pinnedConstraints: [
      ...
      'system: keep the stable MagicPocket prefix byte-stable for prompt-cache reuse'
    ]
  })
  ...
  const withBackgroundShellTools = (
    tools: LocalTool[],
    optionsForTools: MagicPocketServeRuntimeOptions = activeOptions
  ): LocalTool[] => {
  ...
  // subscription set as the main model). magicpocket-process signals that via env so we
...
function agentSdkProviderIdsForOptions(options: MagicPocketServeRuntimeOptions): Set<string> {
...
function agentSdkProviderSignature(options: MagicPocketServeRuntimeOptions): string {
  return [...agentSdkProviderIdsForOptions(options)].sort().join('\n')
}

function mergeRuntimeConfigApplyOptions(
  current: MagicPocketServeRuntimeOptions,
  request: RuntimeConfigApplyRequest
): MagicPocketServeRuntimeOptions {
...
function tokenEconomyConfigForOptions(
  options: Pick<MagicPocketServeRuntimeOptions, 'tokenEconomyMode' | 'tokenEconomy'>
): TokenEconomyConfig {
  return {
...
function toolOutputLimitsForOptions(
  options: Pick<MagicPocketServeRuntimeOptions, 'toolOutputLimits'>
...
function builtinToolOptionsForOptions(options: MagicPocketServeRuntimeOptions) {
...
export async function startMagicPocketServe(
  options: MagicPocketServeRuntimeOptions
): Promise<MagicPocketServeHandle> {
  const runtime = await createMagicPocketServeRuntime(options)
  ...
  // progress" forever with nothing running (MagicPocketAgent/MagicPocket#370).
  void runtime.turnService
    .reconcileOrphanedTurns()
    .then(async (threadIds) => {
      if (threadIds.length > 0) {
        console.warn(`[magicpocket] marked orphaned turn(s) on ${threadIds.length} thread(s) as failed after restart`)
      }
      if (threadIds.length > 0 && runtime.resumeInterruptedGoals) {
        const resumed = await runtime.resumeInterruptedGoals(threadIds)
        if (resumed > 0) {
          console.warn(`[magicpocket] auto-resumed ${resumed} interrupted goal(s) after restart`)
        }
      }
    })
    .catch((error) => {
      console.warn('[magicpocket] orphaned turn reconciliation failed:', error)
    })
  ...
  void runtime.delegationRuntime
    ...
    .then((count) => {
      if (count > 0) {
        console.warn(`[magicpocket] marked ${count} orphaned subagent run(s) as failed after restart`)
      }
    })
    .catch((error) => {
      console.warn('[magicpocket] orphaned child-run reconciliation failed:', error)
    })
  return {
    ...server,
    runtime,
