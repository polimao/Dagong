...
export const MagicPocketServeConfigSchema = z
  .object({
...
export const MagicPocketConfigSchema = z
  .object({
    serve: MagicPocketServeConfigSchema.optional(),
    models: ModelConfigSchema.optional(),
    contextCompaction: ContextCompactionConfigSchema.optional(),
    runtime: RuntimeTuningConfigSchema.optional(),
    roles: RolesConfigSchema.optional(),
    capabilities: MagicPocketCapabilitiesConfig.default(DEFAULT_KUN_CAPABILITIES_CONFIG),
    hooks: HooksConfigSchema.optional(),
    quality: QualityConfigSchema.optional()
  })
  .strict()

export type MagicPocketConfig = z.infer<typeof MagicPocketConfigSchema>
export type QualityConfig = z.infer<typeof QualityConfigSchema>
export const DEFAULT_QUALITY_CONFIG: QualityConfig = QualityConfigSchema.parse({})
export type MagicPocketServeConfig = z.infer<typeof MagicPocketServeConfigSchema>
...
export type LoadedMagicPocketConfig = {
  path: string
  config: MagicPocketConfig
}

export function readMagicPocketConfigFile(path: string): LoadedMagicPocketConfig {
  ...
  try {
    ...
    throw new Error(`Failed to parse MagicPocket config JSON at ${resolvedPath}: ${message}`)
  }
  const parsed = MagicPocketConfigSchema.safeParse(json)
  if (!parsed.success) {
    throw new Error(
      `Invalid MagicPocket config at ${resolvedPath}: ${JSON.stringify(parsed.error.issues, null, 2)}`
    )
  }
  return { path: resolvedPath, config: parsed.data }
}

export function readOptionalMagicPocketConfigFile(path: string | undefined): LoadedMagicPocketConfig | null {
  if (!path) return null
  const resolvedPath = expandHomePath(path)
  if (!existsSync(resolvedPath)) return null
  return readMagicPocketConfigFile(resolvedPath)
}

export function magicpocketConfigPathForDataDir(dataDir: string | undefined): string | undefined {
