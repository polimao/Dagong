...
MagicPocket 是同名桌面应用的本地 HTTP/SSE 代理运行时。它为 GUI 提供稳定、类型化且 GUI 友好的代理循环合约：

- `magicpocket serve` 会启动一个本地 HTTP 服务器，并暴露 `/v1/*` 路由。
...
MagicPocket 也可以在无 GUI 的情况下独立运行：
...
- `magicpocket run` 会创建一个线程，执行一个回合并流式输出助手文本后退出。
- `magicpocket chat` 启动行式 REPL。使用 `/exit`、`/quit` 或空行退出。
- `magicpocket exec --list-tools` 打印当前配置 / 工作区下生效的动态工具列表。
- `magicpocket exec <tool> --args <json>` 直接调用单个工具。`run` 或 `exec` 上可配合 `--json` 获取机器可读输出。
...
MagicPocket 使用 JSON 配置文件管理运行时行为，避免重建后重配或硬编码参数。
...
MagicPocket 默认使用混合存储：`threads/{threadId}/messages.jsonl` 与 `events.jsonl` 是会话的标准回放日志；`index.sqlite3` 仅保存可重建的线程元数据（列表与搜索加速）。将 `serve.storage.backend` 设置为 `"file"` 可以回退到旧版 JSON 索引，或设置 `serve.storage.sqlitePath` 覆盖默认的 `{dataDir}/index.sqlite3`。
...
- `capabilities.mcp` 启动配置化 MCP 客户端并将工具加入动态注册表；工作区级服务器要求设置 `trustedWorkspaceRoots`。远程 HTTP/SSE MCP 可配置 `oauth`，MagicPocket 会把 OAuth token 存在数据目录下，而不是写进 config。使用 `GET /v1/mcp/oauth` 可查看脱敏后的 OAuth 状态，使用 `DELETE /v1/mcp/oauth/{serverId}` 可清除某个服务保存的授权。
...
Hooks 允许外部命令观察并干预 agent 生命周期，无需重新编译 MagicPocket。在
`config.json` 顶层 `hooks` 键下配置（GUI 默认的
`~/.deepseekgui/magicpocket/config.json` 直接生效），主循环、子代理和 CLI
共用同一套 hook。
...
`AgentLoop` 的 `hooks` 选项传入进程内函数 hook（从 `magicpocket/hooks` 导出）。

命令 hook 以运行时权限执行任意 shell 命令——请把 `config.json` 当作
可信输入对待。

完整参考见 `../docs/magicpocket-hooks.md`：各阶段 stdin 载荷、结果字段、
失败语义与示例 hook 脚本。
...
- `magicpocket run`/`magicpocket chat`/`magicpocket exec` 报错：核对 `--config`、`--data-dir`、`--api-key`、`--base-url`、`--runtime-token` 一致；先用 `magicpocket exec --list-tools --json` 检查工具注册表。
- 功能显示为 disabled：通常表示配置标志为 false；显示为 unavailable 通常表示标志为 true，但 provider/存储/模型未就绪或初始化失败。

## GUI 集成

Legacy provider 退役后，主流程由 `magicpocket-process.ts` 启动 MagicPocket，并通过 `runtimeRequest` 使用 bearer token 调用活动 base URL。渲染层保持与旧 `AgentProvider` 的接口兼容：`agents.magicpocket` 下保存 `binaryPath`、`port`、`autoStart`、`apiKey`、`baseUrl`、`runtimeToken`、`dataDir`、`model`、`approvalPolicy`、`sandboxMode`、`insecure`。
...
旧的持久化设置（`agentProvider: "codewhale"` 或 `"reasonix"`）会由 `migrateLegacyAppSettings` 做一次性迁移；旧凭据、baseUrl、端口与模型会映射到 `agents.magicpocket`；迁移后保存的设置不再包含 CodeWhale/Reasonix 配置。
