...
import { createMagicPocketServeRuntime, seedUsageCarryover } from '../src/server/runtime-factory.js'
...
describe('runtime factory usage carryover', () => {
  ...
  it('hot-applies model and tool capabilities into runtime info and diagnostics', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'magicpocket-runtime-apply-'))
    tempDirs.push(dataDir)
    const runtime = await createMagicPocketServeRuntime({
      host: '127.0.0.1',
      port: 0,
      dataDir,
      runtimeToken: 'tok',
      ...
      capabilities: MagicPocketCapabilitiesConfig.parse({})
    })
