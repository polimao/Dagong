...
import { MagicPocketCapabilitiesConfig } from '../src/contracts/capabilities.js'
...
describe('SkillRuntime', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'magicpocket-skills-'))
  })
  ...
  it('discovers a skill package symlinked into a root (e.g. cc switch)', async (ctx) => {
    ...
    const realDir = await mkdtemp(join(tmpdir(), 'magicpocket-skill-real-'))
    try {
  ...
  it('scopes the dynamic skill catalog to the current workspace', async () => {
    const workspaceA = await mkdtemp(join(tmpdir(), 'magicpocket-skill-workspace-a-'))
    const workspaceB = await mkdtemp(join(tmpdir(), 'magicpocket-skill-workspace-b-'))
    try {
      ...
      const disabledConfig = MagicPocketCapabilitiesConfig.parse({
        skills: {
          enabled: true,
          roots: [rootA],
          workspaceRoots: [workspaceA, workspaceB],
