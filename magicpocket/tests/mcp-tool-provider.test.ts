...
import { MagicPocketCapabilitiesConfig, type McpServerConfig } from '../src/contracts/capabilities.js'
...
describe('MCP tool provider', () => {
  ...
  it('adds nvm node bin directories to stdio MCP environments on Linux', async () => {
    const home = (await mkdtemp(join(tmpdir(), 'magicpocket-nvm-home-'))).replace(/\\/g, '/')
  ...
  it('formats missing stdio MCP commands with an actionable PATH hint', () => {
    const server = MagicPocketCapabilitiesConfig.parse({
      mcp: {
        enabled: true,
        servers: {
          filesystem: {
  ...
  it('keeps OAuth disabled unless remote MCP servers opt in', async () => {
    const root = await mkdtemp(join(tmpdir(), 'magicpocket-mcp-oauth-'))
    const config = MagicPocketCapabilitiesConfig.parse({
      mcp: {
        enabled: true,
        servers: {
...
describe('mcp oauth diagnostics state machine', () => {
  function oauthServer(): McpServerConfig {
    return MagicPocketCapabilitiesConfig.parse({
      mcp: {
        enabled: true,
        servers: {
          vercel: {
