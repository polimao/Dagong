...
function buildGuiPlan(relativePath = '.magicpocketsdd/plan/login.md', operation: 'draft' | 'refine' = 'draft') {
  return {
    operation,
...
describe('create_plan tool: path validation', () => {
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# nested',
        operation: 'draft',
        plan_relative_path: '.magicpocketsdd/plan/nested/a.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.magicpocketsdd/plan/nested/a.md')
      })
    )
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# not md',
        operation: 'draft',
        plan_relative_path: '.magicpocketsdd/plan/login.txt'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.magicpocketsdd/plan/login.txt')
      })
    )
    expect(result.isError).toBe(true)
  })
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# mismatch',
        operation: 'refine',
        plan_relative_path: '.magicpocketsdd/plan/other.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: {
          operation: 'refine',
          workspaceRoot: '/tmp/ws',
          relativePath: '.magicpocketsdd/plan/login.md',
          planId: 'plan_login'
        }
      })
    )
  ...
  it('rejects an operation that differs from the active GUI plan operation', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.magicpocketsdd/plan/login.md', 'refine')
      })
    )
    expect(result.isError).toBe(true)
...
describe('create_plan tool: execution safety', () => {
  it('allows a free-form plan-mode call without a GUI plan context and self-allocates a path', async () => {
    ...
    expect((result.output as { relative_path: string }).relative_path).toBe(
      '.magicpocketsdd/plan/disk-cleanup.md'
    )
  })
  ...
  it('rejects a free-form legacy plan path as a new target', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(JSON.stringify(result.output)).toMatch(/\.magicpocketsdd\/plan/)
  })
  ...
  it('still executes create_plan through the tool host during plan mode', async () => {
    ...
    expect(result.item.kind === 'tool_result'
      ...
      : ''
    ).toBe('.magicpocketsdd/plan/safe-plan.md')
  })
})
...
describe('create_plan tool: success and atomic write', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'magicpocket-plan-'))
    ...
    await mkdir(join(workspace, '.magicpocketsdd/plan'), { recursive: true })
    await writeFile(join(workspace, '.magicpocketsdd/plan/login.md'), previousMarkdown, 'utf8')
  })
  ...
  it('writes a fresh plan to the reserved path and returns structured metadata', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        ...
        guiPlan: {
          ...
          relativePath: '.magicpocketsdd/plan/login.md',
          planId: `${workspace}:.magicpocketsdd/plan/login.md`,
    ...
    expect(output.relative_path).toBe('.magicpocketsdd/plan/login.md')
    expect(output.operation).toBe('draft')
    expect(output.summary).toContain('.magicpocketsdd/plan/login.md')
    expect(output.content_hash).toMatch(/^[a-f0-9]{16}$/)
    expect(output.byte_size).toBe(Buffer.byteLength('# Login plan\n\n- step 1', 'utf8'))
    expect(output.absolute_path).toBe(join(workspace, '.magicpocketsdd/plan/login.md'))
  ...
  it('self-allocates a non-colliding path when plan mode has no reserved context', async () => {
    ...
    // `.magicpocketsdd/plan/login.md` already exists in this workspace.
    expect(output.relative_path).toBe('.magicpocketsdd/plan/login-2.md')
    const persisted = await readFile(output.absolute_path, 'utf8')
  ...
  it('leaves the previous plan untouched when the abort signal fires before rename', async () => {
    ...
    expect(result.isError).toBe(true)
    const persisted = await readFile(join(workspace, '.magicpocketsdd/plan/login.md'), 'utf8')
