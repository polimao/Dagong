...
describe('MagicPocket agent CLI commands', () => {
  let dataDir = ''

  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'magicpocket-cli-'))
  })
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitMagicPocketCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitMagicPocketCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitMagicPocketCliCommand(['nope']).error).toMatch(/unknown command/)
  })

  it('lists tools from magicpocket exec with JSON output', async () => {
    const c = capture()
  ...
  it('invokes a direct tool through magicpocket exec', async () => {
    const c = capture()
    const code = await runAgentCommand('exec', [
      '--data-dir',
      dataDir,
  ...
  it('lists dynamic runtime tools from magicpocket exec', async () => {
  ...
  it('shares config loading between serve and agent commands', async () => {
    const configPath = join(dataDir, 'magicpocket.config.json')
