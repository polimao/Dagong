...
describe('contracts', () => {
    ...
    const todos = ThreadTodoListSchema.parse({
      ...
      items: [{
        ...
        source: {
          kind: 'plan',
          planId: 'plan_1',
          relativePath: '.magicpocketsdd/plan/plan.md',
          ordinal: 0,
          contentHash: 'abc'
        },
        createdAt: '2026-06-03T00:00:00.000Z',
  ...
  it('accepts extension contracts for attachments, memory, child events, and structured errors', () => {
    ...
    expect(MagicPocketErrorBody.parse({
...
describe('cli', () => {
  ...
  it('loads serve and context compaction settings from an explicit config file', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'magicpocket-config-'))
    try {
      const configPath = join(dir, 'magicpocket.config.json')
  ...
  it('normalizes capability config to disabled defaults', () => {
    const config = MagicPocketCapabilitiesConfig.parse({})
  ...
  it('rejects a defaultProfile that is not defined in profiles', () => {
    expect(() => MagicPocketCapabilitiesConfig.parse({
  ...
  it('surfaces subagent profiles and policy in the runtime capability manifest', () => {
    const manifest = buildRuntimeCapabilityManifest({
      ...
      config: MagicPocketCapabilitiesConfig.parse({
        subagents: {
          enabled: true,
          maxParallel: 2,
          maxChildRuns: 6,
  ...
  it('loads config.json from the data dir when present', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'magicpocket-data-'))
    try {
  ...
  it('exposes a usage string', () => {
    expect(SERVE_USAGE).toContain('magicpocket serve')
  })
