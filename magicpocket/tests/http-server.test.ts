...
describe('HTTP server', () => {
  let dataDir = ''
  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'magicpocket-http-'))
  })
  afterEach(async () => {
  ...
  it('returns 200 on /health without auth', async () => {
    ...
    expect(body).toEqual({ status: 'ok', service: 'magicpocket', mode: 'serve' })
  })
  ...
  it('resumes a persisted session into a new MagicPocket thread', async () => {
    const h = buildHarness()
    await h.threadService.create(
      { workspace: '/tmp/project', model: 'deepseek-chat', mode: 'agent' },
