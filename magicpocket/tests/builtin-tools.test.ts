...
describe('MagicPocket built-in tools', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'magicpocket-tools-'))
    backgroundShellDataDir = await mkdtemp(join(tmpdir(), 'magicpocket-bg-shell-data-'))
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello magicpocket' },
    ...
    expect(editedDisk).toContain('hello magicpocket')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello magicpocket')
