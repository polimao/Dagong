...
import { DEFAULT_KUN_MODEL } from '../src/config/magicpocket-config.js'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'magicpocket-todos-'))
    try {
      const relativePath = '.magicpocketsdd/plan/demo.md'
...
describe('ThreadService.resumeSession', () => {
  it('uses the MagicPocket default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
