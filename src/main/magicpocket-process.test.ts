...
import { MagicPocketConfigSchema } from '../../magicpocket/src/config/magicpocket-config.js'
...
function createSettings(binaryPath: string): AppSettingsV1 {
  return {
    ...
    agents: {
      magicpocket: {
        ...defaultMagicPocketRuntimeSettings(testMagicPocketPort),
        binaryPath,
        autoStart: true
      }
    },
...
async function readMagicPocketLog(): Promise<string> {
  if (!tempRoot) throw new Error('temp root not initialized')
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const logFile = readdirSync(tempRoot).find((entry) => entry.startsWith('magicpocket-') && entry.endsWith('.log'))
    if (logFile) return readFileSync(join(tempRoot, logFile), 'utf8')
    await new Promise((resolve) => setTimeout(resolve, 25))
  }
  throw new Error('Expected a magicpocket log file to be created')
}
...
beforeEach(async () => {
  tempRoot = mkdtempSync(join(tmpdir(), 'magicpocket-process-'))
  testMagicPocketPort = await allocateTestPort()
  configureLogger({ dir: tempRoot, enabled: true, retentionDays: 7 })
})

afterEach(async () => {
  const module = await import('./magicpocket-process')
  await module.stopMagicPocketChildAndWait()
...
describe('startMagicPocketChild', () => {
  it('waits for the explicit MagicPocket ready marker before resolving', async () => {
    const script = writeScript(
      'ready-child.js',
      [
        "const http = require('node:http')",
        `const port = ${testMagicPocketPort}`,
        "const server = http.createServer((req, res) => {",
        "  res.setHeader('content-type', 'application/json')",
        "  res.end(JSON.stringify({ service: 'magicpocket', mode: 'serve', status: 'ok' }))",
        "})",
        "server.listen(port, '127.0.0.1', () => {",
        "  setTimeout(() => {",
        "    process.stdout.write('KUN_READY ' + JSON.stringify({ service: 'magicpocket', mode: 'serve', port }) + '\\n')",
        "  }, 50)",
        "})",
        "setInterval(() => {}, 1_000)"
      ].join('\n')
    ...
    const module = await import('./magicpocket-process')
    await expect(module.startMagicPocketChild(createSettings(script))).resolves.toBeUndefined()
    expect(module.isMagicPocketChildRunning()).toBe(true)
    await module.stopMagicPocketChildAndWait()
    const logText = await readMagicPocketLog()
    expect(logText).toContain('KUN_READY')
    expect(logText).toContain(`ready marker received on port ${testMagicPocketPort}`)
  })
  ...
  it('does not settle on the ready marker until the /health endpoint responds', async () => {
    ...
    const module = await import('./magicpocket-process')
    let resolved = false
    const start = module.startMagicPocketChild(createSettings(script)).then(() => {
      resolved = true
    })
  ...
  it('shares the startup promise while MagicPocket is spawned but not ready', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    ...
    const first = module.startMagicPocketChild(settings)

    for (let attempt = 0; attempt < 100 && !module.isMagicPocketChildRunning(); attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 10))
    }
    expect(module.isMagicPocketChildRunning()).toBe(true)

    let secondResolved = false
    const second = module.startMagicPocketChild(settings).then(() => {
      secondResolved = true
    })
  ...
  it('rejects when the child exits before reporting ready', async () => {
    ...
    const module = await import('./magicpocket-process')
    await expect(module.startMagicPocketChild(createSettings(script))).rejects.toThrow(
      /MagicPocket exited during startup with code 23[\s\S]*bind failed on port 18899/
    )
    expect(module.isMagicPocketChildRunning()).toBe(false)
    await module.stopMagicPocketChildAndWait()
    const logText = await readMagicPocketLog()
...
describe('resolveMagicPocketStartupTimeoutMs', () => {
  it('gives Windows the larger default and other platforms a smaller one', async () => {
    const { resolveMagicPocketStartupTimeoutMs } = await import('./magicpocket-process')
    expect(resolveMagicPocketStartupTimeoutMs('win32', {})).toBe(90_000)
    expect(resolveMagicPocketStartupTimeoutMs('darwin', {})).toBe(60_000)
    expect(resolveMagicPocketStartupTimeoutMs('linux', {})).toBe(60_000)
  })

  it('honors a valid KUN_STARTUP_TIMEOUT_MS override on every platform', async () => {
    const { resolveMagicPocketStartupTimeoutMs } = await import('./magicpocket-process')
    expect(resolveMagicPocketStartupTimeoutMs('win32', { KUN_STARTUP_TIMEOUT_MS: '120000' })).toBe(120_000)
    expect(resolveMagicPocketStartupTimeoutMs('linux', { KUN_STARTUP_TIMEOUT_MS: ' 30000 ' })).toBe(30_000)
  })

  it('clamps an out-of-range override to the 15s–10min bounds', async () => {
    const { resolveMagicPocketStartupTimeoutMs } = await import('./magicpocket-process')
    expect(resolveMagicPocketStartupTimeoutMs('linux', { KUN_STARTUP_TIMEOUT_MS: '1000' })).toBe(15_000)
    expect(resolveMagicPocketStartupTimeoutMs('linux', { KUN_STARTUP_TIMEOUT_MS: '99999999' })).toBe(600_000)
  })

  it('falls back to the platform default when the override is not a finite number', async () => {
    const { resolveMagicPocketStartupTimeoutMs } = await import('./magicpocket-process')
    expect(resolveMagicPocketStartupTimeoutMs('win32', { KUN_STARTUP_TIMEOUT_MS: 'soon' })).toBe(90_000)
    expect(resolveMagicPocketStartupTimeoutMs('darwin', { KUN_STARTUP_TIMEOUT_MS: '' })).toBe(60_000)
    expect(resolveMagicPocketStartupTimeoutMs('darwin', { KUN_STARTUP_TIMEOUT_MS: '   ' })).toBe(60_000)
  })
})

describe('waitForMagicPocketStartupSettled', () => {
  it('resolves immediately when no launch is in flight', async () => {
    const module = await import('./magicpocket-process')
    let resolved = false
    await Promise.race([
      module.waitForMagicPocketStartupSettled().then(() => {
        resolved = true
      }),
  ...
  it('does not resolve until an in-flight launch settles', async () => {
    ...
    const module = await import('./magicpocket-process')
    const settings = createSettings(script)
    const start = module.startMagicPocketChild(settings)
    ...
    let settled = false
    const settledPromise = module.waitForMagicPocketStartupSettled().then(() => {
      settled = true
    })
    await new Promise((resolve) => setTimeout(resolve, 50))
...
describe('reclaimMagicPocketPort', () => {
  it('reports a port as unavailable when another listener owns it', async () => {
    ...
    try {
      ...
      await expect(module.reclaimMagicPocketPort(address.port)).resolves.toEqual({
        ok: false,
  ...
  it('allows non-positive ports so MagicPocket can request an ephemeral port', async () => {
    const module = await import('./magicpocket-process')

    await expect(module.reclaimMagicPocketPort(0)).resolves.toEqual({ ok: true })
  })
  ...
  it('resolves the next available fallback port when the preferred port is unavailable', async () => {
    ...
    try {
      ...
      const resolved = await module.resolveAvailableMagicPocketPort(preferredPort)
      ...
      await expect(module.reclaimMagicPocketPort(resolved.port)).resolves.toEqual({ ok: true })
    } finally {
      await new Promise<void>((resolve) => server.close(() => resolve()))
    }
  })

  it('keeps the configured endpoint when the currently managed MagicPocket child owns it', async () => {
    ...
    const module = await import('./magicpocket-process')
    const settings = createSettings(script)
    settings.agents.magicpocket.port = preferredPort

    await module.startMagicPocketChild(settings)
    const resolved = await module.resolveAvailableMagicPocketPort(preferredPort)

    expect(resolved).toEqual({ port: preferredPort, changed: false })
    expect(module.isMagicPocketChildRunning()).toBe(true)
    expect(await readMagicPocketLog()).not.toContain(`killing stale magicpocket process holding port ${preferredPort}`)
  })
})

describe('resolveMagicPocketDataDir', () => {
  it('expands Windows-style home-relative data directories', async () => {
    const module = await import('./magicpocket-process')

    expect(module.resolveMagicPocketDataDir({ dataDir: '~\\deepseek\\magicpocket' })).toBe(join(homedir(), 'deepseek', 'magicpocket'))
  })

  it('does not expand non-home tilde prefixes', async () => {
    const module = await import('./magicpocket-process')

    expect(module.resolveMagicPocketDataDir({ dataDir: '~other\\magicpocket' })).toBe('~other\\magicpocket')
  })
})
...
describe('parseListeningPidsFromNetstat', () => {
  it('extracts the listening TCP PIDs for the port across IPv4/IPv6, ignoring everything else', async () => {
    const { parseListeningPidsFromNetstat } = await import('./magicpocket-process')
...
describe('syncGuiManagedMagicPocketConfig', () => {
  it('creates GUI-managed config with attachments enabled for image paste/upload', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./magicpocket-process')

    await module.syncGuiManagedMagicPocketConfig(tempRoot, defaultMagicPocketRuntimeSettings())
  ...
  it('exports per-model max output tokens into MagicPocket model profiles', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./magicpocket-process')

    await module.syncGuiManagedMagicPocketConfig(tempRoot, {
      ...defaultMagicPocketRuntimeSettings(),
      modelProfiles: {
        writer: {
    ...
    const parsed = JSON.parse(readFileSync(configPath, 'utf8')) as any
    expect(MagicPocketConfigSchema.safeParse(parsed).success).toBe(true)
  ...
  it('writes the image generation capability and omits cleared fields', async () => {
    ...
    await module.syncGuiManagedMagicPocketConfig(tempRoot, runtime)
  ...
  it('keeps the config stable across repeated syncs with imageGen configured', async () => {
    ...
    // every sync rewrites the file and restarts MagicPocket in a loop.
    await module.syncGuiManagedMagicPocketConfig(tempRoot, runtime)
  ...
  it('adds the built-in schedule MCP server to MagicPocket runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./magicpocket-process')
    const settings = createSettings('/tmp/fake-magicpocket-child.js')
  ...
  it('adds GUI project and configured global skill roots to MagicPocket runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./magicpocket-process')
    const settings = createSettings('/tmp/fake-magicpocket-child.js')
  ...
  it('writes GUI-managed MCP search settings without removing existing servers', async () => {
    ...
    await module.syncGuiManagedMagicPocketConfig(
      tempRoot,
      {
        ...defaultMagicPocketRuntimeSettings(),
        storage: {
          backend: 'hybrid',
          sqlitePath: '/tmp/magicpocket-index.sqlite3'
        },
        contextCompaction: {
  ...
  it('does not auto-import repo-local .magicpocket/mcp.json servers into the runtime', async () => {
    ...
    mkdirSync(join(repo, '.magicpocket'), { recursive: true })
    writeFileSync(join(repo, '.magicpocket', 'mcp.json'), JSON.stringify({
      servers: {
  ...
  it('replaces unparsable historical MagicPocket config with a valid GUI-managed config', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
  ...
  it('does not enable MCP when the capability is explicitly disabled', async () => {
    ...
    await module.syncGuiManagedMagicPocketConfig(tempRoot, defaultMagicPocketRuntimeSettings(), {
      scheduleMcp: {
        settings: createSettings('/tmp/fake-magicpocket-child.js'),
        launch: {
          appPath: '/tmp/deepseek-gui-test-app',
          execPath: '/tmp/electron',
          isPackaged: false
