...
import { DESKTOP_COMMANDS } from '../../shared/dagong-gui-api'
...
const dagongStorageBackendSchema = z.enum(['hybrid', 'file'])
const dagongCompactionSummaryModeSchema = z.enum(['heuristic', 'model'])
...
const dagongRuntimePatchSchema = z.object({
  ...
  storage: z.object({
    backend: dagongStorageBackendSchema.optional(),
  ...
  contextCompaction: z.object({
    ...
    summaryMode: dagongCompactionSummaryModeSchema.optional(),
  ...
  // Global small-model slot + per-role internal-LLM model overrides (agents.dagong.*).
...
const settingsPatchObjectSchema = z.object({
  ...
  agents: z.object({
    dagong: dagongRuntimePatchSchema.optional()
  }).strict().optional(),
  workspaceRoot: defaultPathSchema,
