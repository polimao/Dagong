...
beforeEach(async () => {
  userDataDir = await mkdtemp(join(tmpdir(), 'dagong-ui-plugin-data-'))
  sourceDir = await mkdtemp(join(tmpdir(), 'dagong-ui-plugin-src-'))
})
...
describe('seedUiPlugin (bundled plugins like idagong)', () => {
  it('seeds a plugin from in-memory bytes and it lists/loads like any other', async () => {
    ...
    const plugins = await listUiPlugins(userDataDir)
    expect(plugins.map((p) => p.manifest.id)).toContain('idagong')

    const loaded = await loadUiPluginFigures(userDataDir, 'idagong')
  ...
  it('rejects seeding when figure bytes are missing', async () => {
    const result = await seedUiPlugin(
      userDataDir,
      { id: 'idagong', name: 'x', version: '1.0.0', figures: { swim: 'img/a.png' } },
      {}
    )
    expect(result.ok).toBe(false)
  })
...
describe('bundled starlight example', () => {
  it('installs and loads end to end', async () => {
    ...
    expect(loaded.ok).toBe(true)
    if (!loaded.ok) return
    expect(loaded.manifest.name).toBe('星夜 Dagong')
    expect(loaded.figures.swim?.startsWith('data:image/png;base64,')).toBe(true)
    expect(loaded.manifest.features?.cameos).toBe(true)
