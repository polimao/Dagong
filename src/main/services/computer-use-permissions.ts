 ...
 * so the user only has to flip the toggle rather than add Dagong by hand.
 */
type MacPermissions = {
...
/**
 * Proactively enroll Dagong in the relevant macOS permission list and open
  ...
  try {
    if (kind === 'accessibility') {
      if (native?.askForAccessibilityAccess) {
        // Registers Dagong in the Accessibility list + opens the pane.
      ...
      if (native?.askForScreenCaptureAccess) {
        // CGRequestScreenCaptureAccess: registers Dagong in the Screen
