...
import {
  ...
  ImageGenHttpError,
  type ImageGenClient
} from '../../../dagong/src/adapters/tool/image-gen-tool-provider.js'
import { detectImage } from '../../../dagong/src/attachments/attachment-store.js'
...
export function isWriteInfographicConfigured(
  imageGeneration: Pick<DagongImageGenerationSettingsV1, 'enabled' | 'baseUrl' | 'apiKey' | 'model'>
): boolean {
  return (
...
function imagePromptMaxChars(imageGeneration: DagongImageGenerationSettingsV1): number | undefined {
...
export async function requestWriteInfographic(
  ...
  const imageGeneration = resolveDagongImageGenerationSettings(settings)
  ...
  try {
    ...
    // requirement '.dagongsdd/requirements/<id>/img').
