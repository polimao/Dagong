...
describe('requestWriteInlineCompletion', () => {
  it('calls DeepSeek FIM completions directly instead of chat completions', async () => {
    ...
    expect(body.prompt).toContain('Dagong inline completion')
  ...
  it('records missing API key failures in the debug log', async () => {
    ...
    expect(debugEntries[0].prompt).toContain('Dagong inline completion')
  ...
  it('falls back to the General baseUrl and Dagong model when write keeps defaults', async () => {
    const fetchMock = vi.fn(async () =>
    ...
    settings.agents.dagong.model = 'deepseek-chat'
  ...
  it('builds the unified action prompt without retrieval snippets when none are supplied', () => {
    ...
    expect(prompt).toContain('Dagong inline completion')
