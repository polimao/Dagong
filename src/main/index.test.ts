 ...
 * 都返回 *相对于 main bundle* 的路径(例如 'chunks/dagong-XXXX.png')。
...
describe('app icon loader', () => {
  ...
  describe('resolveAppIconPath', () => {
    it('joins a relative source with the provided baseDir', () => {
      const resolved = mod.resolveAppIconPath('chunks/dagong-XXXX.png', '/app/bundle')
      // 路径分隔符因平台而异(Windows 是 \,其它是 /),用 toMatch 避免硬编码
      expect(resolved.replace(/\\/g, '/')).toMatch(/(?:^[A-Za-z]:)?\/app\/bundle\/chunks\/dagong-XXXX\.png$/)
    })

    it('strips a leading slash before joining with baseDir (dev mode quirk)', () => {
      // Vite ?url import 在 dev 模式下会返回 '/chunks/dagong-XXXX.png'(带前导斜杠)。
      // 在 Windows 上 path.isAbsolute('/foo') === true,但实际文件并不在当前盘根下,
      // 而是在 main bundle 输出目录里 —— 必须把前导斜杠剥掉,当作相对路径 join。
      const resolved = mod.resolveAppIconPath('/chunks/dagong-XXXX.png', 'd:\\app\\bundle')
      expect(resolved.replace(/\\/g, '/')).toBe('d:/app/bundle/chunks/dagong-XXXX.png')
    })

    it('passes an absolute source through unchanged', () => {
      const absolute = 'C:\\Users\\me\\app.asar\\dagong.png'
  ...
  describe('createAppIcon', () => {
    ...
    it('reads the file via readFileSync and passes the buffer to createFromBuffer', () => {
      ...
      const icon = mod.createAppIcon('chunks/dagong-XXXX.png')
      ...
      expect(calledPath.replace(/\\/g, '/')).toContain('chunks/dagong-XXXX.png')
