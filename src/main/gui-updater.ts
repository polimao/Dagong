...
// 就再也收不到 Dagong 的升级包。域名优先使用 dagong-agent,旧域名仅作兜底。
const PRIMARY_R2_PUBLIC_BASE_URL = 'https://www.dagong-agent.com/api/r2'
const SECONDARY_R2_PUBLIC_BASE_URL = 'https://dagong-agent.com/api/r2'
...
function envWithLegacyFallback(dagongName: string, legacyName: string): string {
  return process.env[dagongName]?.trim() || process.env[legacyName]?.trim() || ''
}
  ...
  try {
    const res = await fetch(updateFeedManifestUrl(feedUrl), {
      ...
      headers: {
        ...
        'User-Agent': `dagong/${app.getVersion()}`
      },
...
async function runScheduledGuiUpdateCheck(): Promise<void> {
  ...
  backgroundCheckPromise = (async () => {
    try {
      ...
      console.warn('[dagong-gui updater] scheduled GUI update check failed:', error)
    } finally {
...
async function checkManualUpdate(
  ...
  try {
    ...
    const res = await fetch(url, {
      headers: {
        Accept: 'application/x-yaml,text/yaml,text/plain,*/*',
        'User-Agent': `dagong/${currentVersion}`
      }
    })
    if (!res.ok) {
      return {
...
export function initializeGuiUpdater(
  ...
  autoUpdater.logger = {
    info: (message?: unknown) => console.info('[dagong-gui updater]', message),
    warn: (message?: unknown) => console.warn('[dagong-gui updater]', message),
    error: (message?: unknown) => console.error('[dagong-gui updater]', message)
  }
  ...
  autoUpdater.on('update-downloaded', (event: UpdateDownloadedEvent) => {
    ...
    pendingVersionStateWrite = recordPendingUpdate(event)
      .catch((error) => {
        console.warn('[dagong-gui updater] failed to save release notes:', error)
      })
      .finally(() => {
  ...
  nativeAutoUpdater?.on?.('before-quit-for-update', () => {
    void runBeforeInstallUpdate().catch((error) => {
      console.warn('[dagong-gui updater] failed to stop runtimes before update quit:', error)
    })
  })
...
export async function showPostUpdateReleaseNotes(): Promise<void> {
  ...
  const options: MessageBoxOptions = {
    type: 'info',
    title: isZh ? 'Dagong 已更新' : 'Dagong updated',
    message: isZh ? `已更新到 Dagong ${currentVersion}` : `Dagong has been updated to ${currentVersion}`,
    detail:
      pendingUpdate?.releaseNotes ??
      (isZh
        ? '此版本的完整更新内容可在 Dagong 更新日志中查看。'
        : 'See the Dagong changelog for the complete release notes.'),
