...
/**
 * Built-in "design system & craft" skill. Seeded once into ~/.dagong/skills/ on
 * first launch (idempotent seed marker, mirrors ensureBundledUiPlugins). The
 * Dagong runtime discovers it from the skills root and the agent can load it
...
- If \`.dagong-design/DESIGN_SYSTEM.md\` exists in the workspace, read it first and honor it: brand color, tone, type, radius, density, the named preset. It is the contract shared between the design canvas and the code.
...
export function ensureBundledSkills(dagongHomeDir: string): Promise<void> {
  seedPromise ??= (async () => {
    const skillsRoot = join(dagongHomeDir, 'skills')
