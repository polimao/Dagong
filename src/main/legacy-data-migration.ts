...
/**
 * 一次性把 “DeepSeek GUI” 时代的本地数据搬到 Dagong 的新命名下。
 *
 ...
 *        - 设置 / dagong sqlite 里残留的旧绝对路径仍然可以解析;
...
export const NEW_HOME_DATA_ROOT = '.dagong'

export type HomeDataMigrationMapping = {
  /** 相对 home 的旧路径段,如 ['.deepseekgui', 'dagong'] */
  legacySegments: readonly string[]
  /** 相对 home 的新路径段,如 ['.dagong', 'data'] */
...
export const HOME_DATA_MIGRATION_MAPPINGS: readonly HomeDataMigrationMapping[] = [
  // dagong 运行时数据(sqlite、线程、config.json)。新家叫 data,避免 ~/.dagong/dagong。
  { legacySegments: [LEGACY_HOME_DATA_ROOT, 'dagong'], nextSegments: [NEW_HOME_DATA_ROOT, 'data'] },
...
const SETTINGS_FILE_NAME_NEW = 'dagong-settings.json'
...
export function migrateLegacyUserDataDir(input: {
  /** electron app.getPath('userData') —— 已经是新名字(…/Dagong)。 */
  userDataPath: string
...
export function migrateLegacyHomeDataDirs(input: {
  ...
  for (const mapping of mappings) {
    ...
    if (!tryLinkLegacyPath(legacyPath, nextPath, log)) {
      // 链接建不起来时优先保证旧绝对路径(dagong sqlite 里的线程 cwd、
 ...
 */
export function runLegacyDagongDataMigration(input: {
