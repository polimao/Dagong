...
describe('JsonSettingsStore', () => {
  it('defaults GUI updates to the stable channel for new settings', async () => {
    ...
    expect(loaded.agents.dagong.approvalPolicy).toBe(DEFAULT_APPROVAL_POLICY)
  ...
  it('creates a default write workspace with welcome.md', async () => {
    ...
    expect(loaded.write.defaultWorkspaceRoot).toContain('.dagong')
  ...
  it('migrates legacy deepseek.autoStart=false into Dagong', async () => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'ds-gui-settings-'))
    ...
    expect(loaded.agents.dagong.autoStart).toBe(false)
  })

  it('migrates existing Dagong credentials into General provider settings', async () => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'ds-gui-settings-'))
    ...
    expect(loaded.agents.dagong.apiKey).toBe('')
    expect(loaded.agents.dagong.baseUrl).toBe('')
  })
  ...
  it('keeps custom model providers when migrated settings are reloaded', async () => {
    ...
    expect(firstLoaded.agents.dagong.providerId).toBe('custom-provider-2')
    ...
    expect(secondLoaded.agents.dagong.providerId).toBe('custom-provider-2')
  })
  ...
  it('loads settings from the legacy lowercase userData directory and writes them into the current path', async () => {
    ...
    const currentUserDataDir = join(supportRoot, 'Dagong')
    const currentSettingsPath = join(currentUserDataDir, 'dagong-settings.json')
  ...
  it('migrates legacy deepseek-runtime agentProvider to Dagong', async () => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'ds-gui-settings-'))
    ...
    expect(loaded.agents.dagong.port).toBe(18787)
  })
  ...
  it('backs up invalid JSON and replaces it with defaults', async () => {
    ...
    const replaced = await readFile(join(userDataDir, 'dagong-settings.json'), 'utf8')
    expect(() => JSON.parse(replaced)).not.toThrow()
  })

  it('backs up non-object settings JSON and replaces it with defaults', async () => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'ds-gui-settings-'))
    const settingsPath = join(userDataDir, 'dagong-settings.json')
    await writeFile(settingsPath, 'null', 'utf8')

    const store = new JsonSettingsStore(userDataDir)
    const loaded = await store.load()
    const files = await readdir(userDataDir)
    const backupName = files.find((file) => file.startsWith('dagong-settings.invalid-'))
  ...
  it('ignores null entries in persisted Claw channels and schedule tasks', async () => {
    ...
    await writeFile(
      join(userDataDir, 'dagong-settings.json'),
      JSON.stringify({
        version: 1,
        claw: {
          channels: [null]
  ...
  it('loads the legacy file name inside the current userData dir and re-saves it under the new name', async () => {
    // userData 整目录迁移后的常见形态:目录已经叫 Dagong,里面还是旧文件名。
    const userDataDir = await mkdtemp(join(tmpdir(), 'ds-gui-settings-'))
    await writeFile(
      join(userDataDir, 'deepseek-gui-settings.json'),
    ...
    const rewritten = await readFile(join(userDataDir, 'dagong-settings.json'), 'utf8')
  ...
  it('merges Dagong settings patches', async () => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'ds-gui-settings-'))
    const store = new JsonSettingsStore(userDataDir)
    await store.load()
    ...
    expect(saved.agents.dagong.model).toBe('deepseek-reasoner')
    expect(saved.agents.dagong.approvalPolicy).toBe('on-request')
  })
  ...
  it('omits agentProvider when writing normalized settings to disk', async () => {
    ...
    expect(persisted.agents).toEqual(
      expect.objectContaining({
        dagong: expect.objectContaining({ model: 'deepseek-chat' })
      })
    )
  })

  it('folds legacy Claw thread ids into the single Dagong mapping', async () => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'ds-gui-settings-'))
