...
import { runtimeRequestViaHost } from './magicpocket-adapter'
...
function settingsForPort(port: number): AppSettingsV1 {
  return {
    ...
    agents: {
      magicpocket: {
        ...defaultMagicPocketRuntimeSettings(port),
...
describe('runtimeRequestViaHost', () => {
  it('forwards daily usage requests to the MagicPocket runtime with bearer auth', async () => {
    let seenUrl = ''
