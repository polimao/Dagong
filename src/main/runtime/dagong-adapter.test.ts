...
import { runtimeRequestViaHost } from './dagong-adapter'
...
function settingsForPort(port: number): AppSettingsV1 {
  return {
    ...
    agents: {
      dagong: {
        ...defaultDagongRuntimeSettings(port),
...
describe('runtimeRequestViaHost', () => {
  it('forwards daily usage requests to the Dagong runtime with bearer auth', async () => {
    let seenUrl = ''
