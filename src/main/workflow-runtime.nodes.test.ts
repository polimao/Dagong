...
// The generate-image node lazily imports the dagong image client. Replace it with a
// stub so the test never hits a real provider (and never pulls native deps in).
vi.mock('../../dagong/src/adapters/tool/image-gen-tool-provider.js', () => ({
...
function buildSettings(
  ...
  const base: AppSettingsV1 = {
    ...
    agents: { dagong: { ...defaultDagongRuntimeSettings(), model: 'test-model', apiKey: 'test-key' } },
...
/** The prompt the AI node actually sent to the dagong runtime (from the /turns request). */
...
describe('ai-agent', () => {
  it('runs the prompt through the dagong runtime and returns the reply', async () => {
    cover('ai-agent')
  ...
  it('interpolates {{ }} from the upstream payload into the prompt', async () => {
    ...
    expect(turnPrompt(rr)).toBe('echo Dagong')
  }, 15_000)
  ...
  it('appends the upstream input to the prompt when it uses no {{ }}', async () => {
    ...
    expect(prompt).toContain('Dagong')
  }, 15_000)
          ...
          patch: (s) => ({
            ...
            agents: {
              dagong: {
                ...s.agents.dagong,
                imageGeneration: {
                  ...s.agents.dagong.imageGeneration,
                  enabled: true,
                  providerId: '',
...
describe('output', () => {
  ...
  it('drills into a json path (json mode)', async () => {
    const result = await testNode({ id: 'o', type: 'output', config: { mode: 'json', jsonPath: 'user.name' } }, '{"user":{"name":"Dagong"}}')
    expect(parseOut(result)).toBe('Dagong')
  }, 15_000)

  it('coerces a missing json path to null (json mode)', async () => {
    const result = await testNode({ id: 'o', type: 'output', config: { mode: 'json', jsonPath: 'user.missing.deep' } }, '{"user":{"name":"Dagong"}}')
    expect(result.status).toBe('success')
...
describe('custom', () => {
  it('runs a custom module with its injected $fields', async () => {
    ...
    const result = await testNode({ id: 'c', type: 'custom', config: { moduleId: 'mod-greet', values: { who: 'Dagong' } } }, '{}', {
      modules: [module]
    })
    expect(result.status).toBe('success')
    expect(parseOut(result)).toEqual({ greeting: 'hi Dagong' })
  }, 15_000)
