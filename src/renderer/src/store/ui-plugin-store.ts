...
/**
 * 形象工坊运行时:单一 uiMode('default' | 'idagong' | 插件 id),
 * 负责 DOM 属性(data-idagong-mode / data-ui-plugin)、token 样式注入与插件图集加载。
 */
...
function uiPluginApi(): Window['dagongGui'] | null {
  if (typeof window === 'undefined') return null
  return window.dagongGui ?? null
}

function applyUiModeDom(mode: string, runtime: UiPluginRuntime | null): void {
  if (typeof document === 'undefined') return
  const root = document.documentElement
  root.setAttribute('data-idagong-mode', mode === UI_MODE_IKUN ? 'on' : 'off')
...
export const useUiPluginStore = create<UiPluginState>((set, get) => ({
  ...
  initUiPlugins: async () => {
    ...
    // 插件模式(含预装的 idagong):先把 idagong 属性立即点亮避免闪烁,再异步加载图集;失败则回退默认
  ...
  activateUiMode: async (mode: string) => {
    ...
    // 'idagong' 不再特殊:它是预装插件,与第三方插件走同一条加载链路;
    // applyUiModeDom 会在 id 为 idagong 时同时点亮 data-idagong-mode 手工机制
    const api = uiPluginApi()
    if (typeof api?.loadUiPlugin !== 'function') {
      // 桌面接口不可用(如纯渲染测试):idagong 仍可退化为仅属性模式
...
/** 是否应启用主会话出没彩蛋(idagong 内置 或 插件声明 features.cameos) */
