...
export function createNavigationActions(
  ...
  probeRuntime: async (mode = 'user', options) => {
    ...
    try {
      if (typeof window.dagongGui === 'undefined') {
        throw new Error(
          'Preload bridge missing (window.dagongGui). Restart the app or check BrowserWindow preload path.'
        )
      }
      ...
      try {
        if (typeof window.dagongGui === 'undefined') {
          set({
            ...
            runtimeConnection: 'offline',
            runtimeErrorDetail: 'Preload bridge missing (window.dagongGui). Restart the app or check BrowserWindow preload path.',
        ...
        if (!runtimeStatusUnsubscribe && typeof window.dagongGui.onRuntimeStatus === 'function') {
          runtimeStatusUnsubscribe = window.dagongGui.onRuntimeStatus((status) => {
        ...
        if (!trayActionUnsubscribe && typeof window.dagongGui.onTrayAction === 'function') {
          trayActionUnsubscribe = window.dagongGui.onTrayAction((action) => {
            set({ route: 'chat' })
        ...
        if (!clawChannelActivityUnsubscribe && typeof window.dagongGui.onClawChannelActivity === 'function') {
          clawChannelActivityUnsubscribe = window.dagongGui.onClawChannelActivity(({ channelId, threadId }) => {
            void (async () => {
              const state = get()
  ...
  chooseWorkspace: async ({ createThreadAfter = false, selectThreadAfter = true } = {}) => {
    try {
      ...
      if (typeof window.dagongGui === 'undefined' || typeof window.dagongGui.pickWorkspaceDirectory !== 'function') {
        throw new Error(i18n.t('common:workspacePickerUnavailable'))
      }
      const picked = await window.dagongGui.pickWorkspaceDirectory(get().workspaceRoot || undefined)
  ...
  clearWorkspace: async () => {
    try {
      if (typeof window.dagongGui === 'undefined' || typeof window.dagongGui.setSettings !== 'function') {
        return
      }
  ...
  deleteWorkspace: async (workspacePath) => {
    ...
    try {
      ...
      if (normalizeWorkspaceRoot(get().workspaceRoot) === normalizedPath) {
        try {
          if (typeof window.dagongGui?.setSettings === 'function') {
            const next = await rendererRuntimeClient.setSettings({ workspaceRoot: '' })
            set({
              workspaceRoot: normalizeWorkspaceRoot(next.workspaceRoot),
              codeWorkspaceRoots: get().codeWorkspaceRoots,
  ...
  refreshThreads: async () => {
    ...
    try {
      ...
      // Preserve the active Dagong thread when it is not in the listing yet.
