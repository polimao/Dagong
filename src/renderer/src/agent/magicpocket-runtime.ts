...
import { getMagicPocketRuntimeSettings } from '@shared/app-settings'
import {
  ...
  KUN_SKILLS_PATH,
  magicpocketApprovalPath,
  magicpocketThreadCompactPath,
  magicpocketThreadEventsPath,
  magicpocketThreadForkPath,
  magicpocketThreadGoalPath,
  magicpocketThreadReviewPath,
  magicpocketThreadRewindPath,
  magicpocketThreadTodosPath,
  magicpocketThreadInterruptPath,
  magicpocketThreadPath,
  magicpocketThreadSteerPath,
  magicpocketThreadTurnsPath,
  magicpocketAttachmentContentPath,
  magicpocketUserInputPath,
  magicpocketMemoryRecordPath,
  magicpocketMcpOAuthServerPath,
  magicpocketSessionResumePath,
  normalizeThreadMode,
  type MagicPocketThreadMode
...
import {
  buildQuery,
  chatBlockFromItem,
  dispatchMagicPocketRuntimeEvents,
  goalFromCore,
  mergeChatBlocks,
  todosFromCore,
  threadFromCore
} from './magicpocket-mapper'
...
/**
 * GUI-side adapter for the MagicPocket HTTP/SSE contract.
 *
 ...
 * mapping live in `magicpocket-contract.ts` and `magicpocket-mapper.ts`.
 */
export class MagicPocketRuntimeProvider implements AgentProvider {
  readonly id = 'magicpocket' as const
  readonly displayName = 'MagicPocket'
  ...
  async getThreadDetail(threadId: string): Promise<{
    ...
    const response = await rendererRuntimeClient.runtimeRequest(magicpocketThreadPath(threadId), 'GET')
    if (!response.ok) {
  ...
  async sendUserMessage(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadTurnsPath(threadId),
      'POST',
      JSON.stringify(body)
    )
    if (!response.ok) {
  ...
  async rewindThread(threadId: string, turnId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadRewindPath(threadId),
      'POST',
  ...
  async reviewThread(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadReviewPath(threadId),
      'POST',
      JSON.stringify(body)
    )
    if (!response.ok) {
  ...
  async steerUserMessage(threadId: string, turnId: string, text: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadSteerPath(threadId, turnId),
      'POST',
      JSON.stringify({ text })
    )
    if (!response.ok) {
  ...
  async interruptTurn(threadId: string, turnId: string, options?: { discard?: boolean }): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadInterruptPath(threadId, turnId),
      'POST',
  ...
  async renameThread(threadId: string, title: string, auto?: boolean): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadPath(threadId),
      'PATCH',
  ...
  async deleteThread(threadId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(magicpocketThreadPath(threadId), 'DELETE')
    if (!response.ok) {
  ...
  async compactThread(threadId: string, reason?: string): Promise<{ replacedTokens: number }> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadCompactPath(threadId),
      'POST',
  ...
  async getThreadGoal(threadId: string): Promise<NonNullable<NormalizedThread['goal']> | null> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadGoalPath(threadId),
      'GET'
    )
    if (!response.ok) {
  ...
  async getThreadTodos(threadId: string): Promise<NonNullable<NormalizedThread['todos']> | null> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketThreadTodosPath(threadId),
      'GET'
    )
    if (!response.ok) {
  ...
  async submitApprovalDecision(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketApprovalPath(approvalId),
      'POST',
  ...
  async submitUserInputResponse(inputId: string, answers: UserInputAnswer[]): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketUserInputPath(inputId),
      'POST',
  ...
  async clearMcpOAuthCredentials(serverId?: string): Promise<string[]> {
    const response = await rendererRuntimeClient.runtimeRequest(
      serverId ? magicpocketMcpOAuthServerPath(serverId) : KUN_MCP_OAUTH_PATH,
      'DELETE'
    )
    if (!response.ok) {
  ...
  async authorizeMcpOAuthCredentials(serverId: string): Promise<CoreMcpOAuthAuthorizeResponseJson> {
    const response = await rendererRuntimeClient.runtimeRequest(magicpocketMcpOAuthServerPath(serverId), 'POST')
    if (!response.ok) {
  ...
  async getAttachmentContent(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${magicpocketAttachmentContentPath(attachmentId)}${query}`,
      'GET'
    )
    if (!response.ok) {
  ...
  async updateMemory(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${magicpocketMemoryRecordPath(memoryId)}${query}`,
      'PATCH',
      JSON.stringify(patch)
    )
    if (!response.ok) {
  ...
  async deleteMemory(memoryId: string, options: { workspace?: string } = {}): Promise<CoreMemoryRecordJson> {
    const query = buildQuery({ workspace: options.workspace })
    const response = await rendererRuntimeClient.runtimeRequest(`${magicpocketMemoryRecordPath(memoryId)}${query}`, 'DELETE')
    if (!response.ok) {
  ...
  async forkThread(
    ...
    const url = magicpocketThreadForkPath(threadId)
    const response =
  ...
  async resumeSession(
    sessionId: string,
    options?: { model?: string; mode?: MagicPocketThreadMode }
  ): Promise<{ threadId: string; sessionId: string }> {
    const settings = await rendererRuntimeClient.getSettings()
    const runtime = getMagicPocketRuntimeSettings(settings)
    const response = await rendererRuntimeClient.runtimeRequest(
      magicpocketSessionResumePath(sessionId),
      'POST',
      JSON.stringify({
  ...
  async subscribeThreadEvents(
    ...
    await new Promise<void>(async (resolve) => {
      ...
      const offData = rendererRuntimeClient.onSseEvent((payload) => {
        ...
        const task = dispatchMagicPocketRuntimeEvents(batch, sink, (runtimeEvent, eventSink) =>
  ...
  private async handleApprovalRequest(event: CoreRuntimeEventJson, sink: ThreadEventSink): Promise<void> {
    ...
    try {
      ...
      const policy = eventPolicy ?? getMagicPocketRuntimeSettings(await rendererRuntimeClient.getSettings()).approvalPolicy
      switch (policy) {
        case 'auto':
...
export { magicpocketThreadEventsPath }
