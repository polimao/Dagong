...
import { getDagongRuntimeSettings } from '@shared/app-settings'
import {
  ...
  KUN_SKILLS_PATH,
  dagongApprovalPath,
  dagongThreadCompactPath,
  dagongThreadEventsPath,
  dagongThreadForkPath,
  dagongThreadGoalPath,
  dagongThreadReviewPath,
  dagongThreadRewindPath,
  dagongThreadTodosPath,
  dagongThreadInterruptPath,
  dagongThreadPath,
  dagongThreadSteerPath,
  dagongThreadTurnsPath,
  dagongAttachmentContentPath,
  dagongUserInputPath,
  dagongMemoryRecordPath,
  dagongMcpOAuthServerPath,
  dagongSessionResumePath,
  normalizeThreadMode,
  type DagongThreadMode
...
/**
 * GUI-side adapter for the Dagong HTTP/SSE contract.
 *
 ...
 * mapping live in `dagong-contract.ts` and `dagong-mapper.ts`.
 */
export class DagongRuntimeProvider implements AgentProvider {
  readonly id = 'dagong' as const
  readonly displayName = 'Dagong'
  ...
  async getThreadDetail(threadId: string): Promise<{
    ...
    const response = await rendererRuntimeClient.runtimeRequest(dagongThreadPath(threadId), 'GET')
    if (!response.ok) {
  ...
  async sendUserMessage(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongThreadTurnsPath(threadId),
      'POST',
      JSON.stringify(body)
    )
    if (!response.ok) {
  ...
  async rewindThread(threadId: string, turnId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongThreadRewindPath(threadId),
      'POST',
  ...
  async reviewThread(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongThreadReviewPath(threadId),
      'POST',
      JSON.stringify(body)
    )
    if (!response.ok) {
  ...
  async steerUserMessage(threadId: string, turnId: string, text: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongThreadSteerPath(threadId, turnId),
      'POST',
      JSON.stringify({ text })
    )
    if (!response.ok) {
  ...
  async interruptTurn(threadId: string, turnId: string, options?: { discard?: boolean }): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongThreadInterruptPath(threadId, turnId),
      'POST',
  ...
  async archiveThread(threadId: string, archived: boolean): Promise<void> {
    const response = await window.dagongGui.runtimeRequest(
      dagongThreadPath(threadId),
      'PATCH',
  ...
  async deleteThread(threadId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(dagongThreadPath(threadId), 'DELETE')
    if (!response.ok) {
  ...
  async compactThread(threadId: string, reason?: string): Promise<{ replacedTokens: number }> {
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongThreadCompactPath(threadId),
      'POST',
  ...
  async getThreadTodos(threadId: string): Promise<NonNullable<NormalizedThread['todos']> | null> {
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongThreadTodosPath(threadId),
      'GET'
    )
    if (!response.ok) {
  ...
  async clearMcpOAuthCredentials(serverId?: string): Promise<string[]> {
    const response = await rendererRuntimeClient.runtimeRequest(
      serverId ? dagongMcpOAuthServerPath(serverId) : KUN_MCP_OAUTH_PATH,
      'DELETE'
    )
    if (!response.ok) {
  ...
  async authorizeMcpOAuthCredentials(serverId: string): Promise<CoreMcpOAuthAuthorizeResponseJson> {
    const response = await rendererRuntimeClient.runtimeRequest(dagongMcpOAuthServerPath(serverId), 'POST')
    if (!response.ok) {
  ...
  async getAttachmentContent(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${dagongAttachmentContentPath(attachmentId)}${query}`,
      'GET'
    )
    if (!response.ok) {
  ...
  async updateMemory(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${dagongMemoryRecordPath(memoryId)}${query}`,
      'PATCH',
      JSON.stringify(patch)
    )
    if (!response.ok) {
  ...
  async deleteMemory(memoryId: string, options: { workspace?: string } = {}): Promise<CoreMemoryRecordJson> {
    const query = buildQuery({ workspace: options.workspace })
    const response = await rendererRuntimeClient.runtimeRequest(`${dagongMemoryRecordPath(memoryId)}${query}`, 'DELETE')
    if (!response.ok) {
  ...
  async forkThread(
    ...
    const url = dagongThreadForkPath(threadId)
    const response =
  ...
  async resumeSession(
    sessionId: string,
    options?: { model?: string; mode?: DagongThreadMode }
  ): Promise<{ threadId: string; sessionId: string }> {
    const settings = await rendererRuntimeClient.getSettings()
    const runtime = getDagongRuntimeSettings(settings)
    const response = await rendererRuntimeClient.runtimeRequest(
      dagongSessionResumePath(sessionId),
      'POST',
      JSON.stringify({
  ...
  async subscribeThreadEvents(
    ...
    await new Promise<void>(async (resolve) => {
      ...
      const offData = rendererRuntimeClient.onSseEvent((payload) => {
        ...
        const task = dispatchDagongRuntimeEvents(batch, sink, (runtimeEvent, eventSink) =>
  ...
  private async handleApprovalRequest(event: CoreRuntimeEventJson, sink: ThreadEventSink): Promise<void> {
    ...
    try {
      ...
      const policy = eventPolicy ?? getDagongRuntimeSettings(await rendererRuntimeClient.getSettings()).approvalPolicy
      switch (policy) {
        case 'auto':
