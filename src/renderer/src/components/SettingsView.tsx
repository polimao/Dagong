...
import type { SkillRootListItem } from '@shared/dagong-gui-api'
...
import { loadDagongDiagnostics } from '../lib/load-dagong-diagnostics'
...
export function SettingsView(): ReactElement {
  ...
  const [mcpConfigPath, setMcpConfigPath] = useState('~/.dagong/mcp.json')
  ...
  const formDagong = form ? getDagongRuntimeSettings(form) : null
  const formPort = formDagong?.port
  ...
  const settingsPlatform = typeof window !== 'undefined' ? window.dagongGui?.platform ?? '' : ''
  const settingsHomeDir = typeof window !== 'undefined' ? window.dagongGui?.homeDir ?? '' : ''
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window.dagongGui === 'undefined') {
  ...
  useEffect(() => {
    if (typeof window.dagongGui?.getLogPath !== 'function') return
    void window.dagongGui.getLogPath().then((p) => setLogPath(p)).catch(() => undefined)
  }, [category])
  ...
  const loadWriteDebugEntries = useCallback(async (): Promise<void> => {
    ...
    try {
      const completionEntries = typeof window.dagongGui?.listWriteInlineCompletionDebugEntries === 'function'
        ? await window.dagongGui.listWriteInlineCompletionDebugEntries()
        : []
  ...
  const refreshSkillRoots = useCallback(async (): Promise<void> => {
    if (typeof window.dagongGui?.listSkillRoots !== 'function') return
    ...
    try {
      ...
      const result = await window.dagongGui.listSkillRoots()
  ...
  const loadMcpConfig = async (): Promise<void> => {
    if (typeof window.dagongGui?.getDagongConfigFile !== 'function') return
    setMcpLoading(true)
    setMcpNotice(null)
    try {
      const config = await window.dagongGui.getDagongConfigFile()
  ...
  const openSkillRoot = async (path: string): Promise<void> => {
    ...
    if (typeof window.dagongGui?.openSkillRoot !== 'function') return
    setSkillNotice(null)
    const result = await window.dagongGui.openSkillRoot(path)
    if (!result.ok) {
  ...
  const saveMcpConfig = async (): Promise<void> => {
    if (typeof window.dagongGui?.setDagongConfigFile !== 'function') return
    setMcpBusy(true)
    setMcpNotice(null)
    try {
      const result = await window.dagongGui.setDagongConfigFile(mcpConfigText)
  ...
  const openMcpConfigDir = async (): Promise<void> => {
    if (typeof window.dagongGui?.openDagongConfigDir !== 'function') return
  ...
  const refreshDagongDiagnostics = useCallback(async (): Promise<void> => {
    const provider = getProvider()
    setRuntimeDiagnosticsBusy(true)
    setRuntimeDiagnosticsNotice(null)
    try {
      const loaded = await loadDagongDiagnostics(provider, { listAllMemories: true })
  ...
  useEffect(() => {
    ...
    void refreshDagongDiagnostics()
  }, [category, refreshDagongDiagnostics])
  ...
  const persistSettings = async (snapshot: AppSettingsV1, version: number): Promise<void> => {
    ...
    try {
      ...
      setSaveStatus('error')
      void window.dagongGui?.logError?.('settings', 'Failed to apply settings', { message }).catch(() => undefined)
    }
  }
  ...
  flushOnUnmountRef.current = (): void => {
    ...
    void rendererRuntimeClient
      ...
      .catch((e) => {
        const message = e instanceof Error ? e.message : String(e)
        void window.dagongGui?.logError?.('settings', 'Failed to flush settings on unmount', { message }).catch(
          () => undefined
        )
      })
  }
  ...
  const dagong = getDagongRuntimeSettings(form)
  ...
  const updateDagong = (patch: Partial<AppSettingsV1['agents']['dagong']>): void => {
    update({ agents: dagongSettingsPatch(patch) })
  }
  ...
  const pickWorkspace = async (): Promise<void> => {
    try {
      ...
      const picked = await window.dagongGui.pickWorkspaceDirectory(expandHomePath(form.workspaceRoot) || undefined)
  ...
  const clearWriteDebugEntries = async (): Promise<void> => {
    ...
    setWriteDebugError(null)
    try {
      if (typeof window.dagongGui?.clearWriteInlineCompletionDebugEntries === 'function') {
        await window.dagongGui.clearWriteInlineCompletionDebugEntries()
      }
