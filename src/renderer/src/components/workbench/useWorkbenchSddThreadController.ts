...
export function useWorkbenchSddThreadController({
  ...
  const startNewSddRequirement = useCallback(async (): Promise<void> => {
    ...
    let targetWorkspace = ''
    try {
      const picked = await window.dagongGui.pickWorkspaceDirectory(suggestedWorkspace || undefined)
    ...
    const result = await window.dagongGui.createWorkspaceFile({
  ...
  const findSddDraftForSidebarThread = useCallback(async (
    ...
    const listWorkspaceDirectory = window.dagongGui?.listWorkspaceDirectory
    const readWorkspaceFile = window.dagongGui?.readWorkspaceFile
