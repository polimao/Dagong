...
export function GeneralSettingsSection({ ctx }: { ctx: Record<string, any> }): ReactElement {
  ...
  const platform = typeof window !== 'undefined' ? window.dagongGui?.platform ?? '' : ''
                  ...
                  control={
                    <div className="flex w-full min-w-0 flex-col items-start gap-2">
                      ...
                      <button
                        ...
                        disabled={typeof window.dagongGui?.openLogDir !== 'function'}
                        onClick={async () => {
                          if (typeof window.dagongGui?.openLogDir !== 'function') return
                          setLogDirOpenError(null)
                          try {
                            const result = await window.dagongGui.openLogDir()
