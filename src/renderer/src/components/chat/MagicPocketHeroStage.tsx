...
import { MagicPocketStateFigure } from './AnimatedWorkLogo'

/**
 * 迷你工作台舞台:标题栏 + 骨架画布 + 居中熟睡的小 MagicPocket。
 *
 * `waking` 打开「唤醒中」加载特效:Zzz 睡息、声纳涟漪、气泡上浮、
 * 草稿打字光标和偶尔翻身。仅在运行时连接页(确实还在重连)启用;
 * 就绪后的空状态与报错态沿用安静版舞台,避免误读成仍在加载(#78)。
 */
export function MagicPocketHeroStage({ waking = false }: { waking?: boolean }): ReactElement {
  return (
    <div
    ...
    >
      <div className="ds-runtime-wake-shell">
        ...
        <div className="ds-runtime-wake-core">
          ...
          <span className="ds-runtime-wake-magicpocket-bob">
            <MagicPocketStateFigure kind="sleep" className="ds-runtime-wake-magicpocket" />
            {waking ? (
