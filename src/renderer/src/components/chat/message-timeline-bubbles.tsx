    ...
    void Promise.all(
      previewRequests.map(async (request) => {
        try {
          ...
          if (request.mode === 'workspace-image' && request.path && typeof window.dagongGui?.readWorkspaceImage === 'function') {
...
function MediaPreviewTile({
  ...
  const handleSaveAs = async (): Promise<void> => {
    if (saveState === 'saving' || typeof window.dagongGui?.saveWorkspaceFileAs !== 'function') return
    ...
    setSaveState('saving')
    try {
      const result = await window.dagongGui.saveWorkspaceFileAs({
        suggestedName: title,
      ...
      setSaveState('error')
      void window.dagongGui?.logError?.('file-save-as', 'Failed to save generated file', {
...
function AssistantExportButton({
  ...
  const handleExport = async (format: WriteExportFormat): Promise<void> => {
    if (typeof window.dagongGui?.exportWriteDocument !== 'function') {
    ...
    try {
      ...
      const title = `Dagong-answer-${date.toISOString().replace(/[:.]/g, '-')}`
      const result = await window.dagongGui.exportWriteDocument({
        title,
