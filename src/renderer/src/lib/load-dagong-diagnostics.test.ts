...
import { loadDagongDiagnostics } from './load-dagong-diagnostics'

describe('loadDagongDiagnostics', () => {
  it('loads runtime info, tool diagnostics, and memory records together', async () => {
    ...
    const loaded = await loadDagongDiagnostics(provider)
  ...
  it('can scope memory loading to the current workspace when explicitly requested', async () => {
    ...
    const loaded = await loadDagongDiagnostics(provider, { listAllMemories: false })
