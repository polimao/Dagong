...
import {
  ...
  readImagicpocketModePreference,
  writeImagicpocketModePreference
} from './imagicpocket-mode'
...
describe('iMagicPocket mode preference', () => {
  it('defaults to disabled when no preference exists', () => {
    installStorage()

    expect(readImagicpocketModePreference()).toBe(false)
  })
  ...
  it('accepts legacy truthy spellings while reading the preference', () => {
    ...
    for (const value of ['1', 'true', 'on']) {
      ...
      expect(readImagicpocketModePreference()).toBe(true)
    }
  })
  ...
  it('writes compact enabled and disabled values', () => {
    ...
    writeImagicpocketModePreference(true)
    expect(storage.getItem(IKUN_MODE_STORAGE_KEY)).toBe('1')

    writeImagicpocketModePreference(false)
