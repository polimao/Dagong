...
// 品牌升级后默认目录在 ~/.dagong 下;老版本/迁移失败的机器上仍可能出现
...
export function workspaceRootIdentityKey(path?: string): string {
  ...
  if (isDefaultWorkspacePath(normalized)) {
    return '~/.dagong/default_workspace'
  }
  return normalized
}
...
export function isClawWorkspacePath(path?: string): boolean {
  ...
  if (!trimmed) return false
  const normalized = normalizePathForMatch(trimmed)
  return normalized.includes('/.dagong/claw/') || normalized.includes('/.deepseekgui/claw/')
}

// 对话会话不绑定项目文件夹,默认在 ~/Documents/Dagong(macOS/Windows)或
// ~/.local/share/Dagong/conversations(Linux)下按时间戳创建工作目录。
export function defaultConversationWorkspaceRoot(): string {
  const platform = typeof window !== 'undefined' && window.dagongGui?.platform ? window.dagongGui.platform : ''
  return platform === 'linux' ? '~/.local/share/Dagong/conversations' : '~/Documents/Dagong'
}
// 兼容旧引用;动态取值。
...
// 仅供路径前缀比较:把 ~ 展开成 home(渲染层没有 node:os,这里用 window.dagongGui.homeDir)。
// 与主进程的 expandHomePath 行为一致;拿不到 homeDir 时退化为不展开,仍能匹配绝对路径。
function expandHomeForMatch(value: string): string {
  if (!value.startsWith('~')) return value
  const home = typeof window !== 'undefined' && window.dagongGui?.homeDir ? window.dagongGui.homeDir : ''
...
export function isInternalDeepSeekGuiWorkspace(path?: string): boolean {
  ...
  const normalized = normalizePathForMatch(trimmed)
  return (
    normalized === '~/.dagong/write_workspace'
    || normalized.endsWith('/.dagong/write_workspace')
    || normalized === '~/.dagong/design-workspace'
    || normalized.endsWith('/.dagong/design-workspace')
