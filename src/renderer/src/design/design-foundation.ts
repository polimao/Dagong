...
export function buildDesignSpecPrompt(options: {
  ...
  const lines = [
    'Dagong is asking you to LAY THE FOUNDATION for a multi-page design before any screen is built.',
...
export function buildDesignSystemBoardPrompt(options: {
  ...
  const lines = [
    'Dagong is asking you to design the VISUAL DESIGN SYSTEM for this product — the style guide every page will follow.',
    `Workspace: ${options.workspaceRoot}`,
...
export function buildDesignLogoPrompt(options: {
  ...
  const lines = [
    'Dagong is asking you to design the BRAND LOGO for this product.',
    `Workspace: ${options.workspaceRoot}`,
