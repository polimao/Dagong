...
describe('design workspace store', () => {
  ...
  it('can append an HTML version without activating that artifact', () => {
    ...
    expect(result).toEqual({
      ...
      basePath: '.dagong-design/doc/screen/v1.html',
      designMdPath: '.dagong-design/doc/screen/DESIGN.md'
    })

    const state = useDesignWorkspaceStore.getState()
    const screen = state.artifacts.find((item) => item.id === 'screen')
    expect(state.activeArtifactId).toBe('canvas')
    expect(screen?.relativePath).toBe('.dagong-design/doc/screen/v2.html')
    expect(screen?.designMdPath).toBe('.dagong-design/doc/screen/DESIGN.md')
    ...
    expect(writeWorkspaceFile).toHaveBeenCalledWith(expect.objectContaining({
      path: '.dagong-design/doc/screen/meta.json',
      workspaceRoot: '/workspace',
      content: expect.stringContaining('.dagong-design/doc/screen/v2.html')
    }))
  })
  ...
  it('can reuse a freshly pending initial HTML screen without appending a skeleton version', () => {
    const fresh = {
      ...
      versions: [
        {
          id: 'fresh-screen-v1',
          relativePath: '.dagong-design/doc/fresh-screen/v1.html',
          createdAt,
    ...
    expect(result).toEqual({
      artifactId: fresh.id,
      relativePath: '.dagong-design/doc/fresh-screen/v1.html',
      designMdPath: '.dagong-design/doc/fresh-screen/DESIGN.md'
    })
    ...
    expect(writeWorkspaceFile).toHaveBeenCalledWith(expect.objectContaining({
      path: '.dagong-design/doc/fresh-screen/meta.json',
      workspaceRoot: '/workspace',
      content: expect.stringContaining('Build the real first screen')
    }))
    expect(
      writeWorkspaceFile.mock.calls.some(([request]) =>
        (request as WriteWorkspaceFileRequest).content.includes('.dagong-design/doc/fresh-screen/v2.html')
      )
    ).toBe(false)
  })
  ...
  it('resets an HTML artifact preview status when selecting another version', () => {
    ...
    try {
      ...
      expect(buildHtmlSiblingManifest(useDesignWorkspaceStore.getState().artifacts, null)[0]).toMatchObject({
        htmlPath: '.dagong-design/doc/screen/v1.html',
        summary: 'Stable draft'
      })
      expect(writeWorkspaceFile).toHaveBeenCalledWith(expect.objectContaining({
        path: '.dagong-design/doc/screen/meta.json',
      ...
      expect(writeWorkspaceFile).toHaveBeenCalledWith(expect.objectContaining({
        path: '.dagong-design/documents.json',
        workspaceRoot: '/workspace',
  ...
  it('persists the design target from both quick toggle and context updates', () => {
    ...
    vi.stubGlobal('window', { dagongGui: { writeWorkspaceFile }, localStorage })

    useDesignWorkspaceStore.getState().setDesignTarget('app')

    expect(useDesignWorkspaceStore.getState().designContext.designTarget).toBe('app')
    expect(storage.get('dagong.design.target.v1')).toBe('app')
    ...
    expect(storage.get('dagong.design.target.v1')).toBe('web')
  ...
  it('persists accepted and archived statuses across every artifact in a direction', () => {
    ...
    expect(metaWrites.map((request) => request.path).sort()).toEqual([
      '.dagong-design/doc/details/meta.json',
      '.dagong-design/doc/screen/meta.json'
    ])
  ...
  it('creates the physical ID directory for a new 设计稿', async () => {
    ...
    vi.stubGlobal('window', {
      dagongGui: { writeWorkspaceFile, createWorkspaceDirectory }
    })
    ...
    expect(createWorkspaceDirectory).toHaveBeenCalledWith({ path: '.dagong-design', workspaceRoot: '/workspace' })
    expect(createWorkspaceDirectory).toHaveBeenCalledWith({ path: `.dagong-design/${id}`, workspaceRoot: '/workspace' })
  })
  ...
  it('opens the canvas assistant by default unless the user collapsed it', async () => {
    ...
    expect(storage.get('dagong.design.canvasAssistantOpen.v1')).toBe('0')
  ...
  it('toggles the canvas assistant open state and persists the collapsed mirror key', () => {
    ...
    expect(storage.get('dagong.design.canvasAssistantOpen.v1')).toBe('0')
    expect(storage.get('dagong.design.aiRailCollapsed.v1')).toBe('1')
    ...
    expect(storage.get('dagong.design.canvasAssistantOpen.v1')).toBe('1')
    expect(storage.get('dagong.design.aiRailCollapsed.v1')).toBe('0')
  })
  ...
  it('new 画布 nest under the active 设计稿 directory', () => {
    ...
    expect(relativePath).toBe(`.dagong-design/${id}/${artifactId}/v1.html`)
  ...
  it('keeps a user-created empty 设计稿 when rehydration reads a stale index', async () => {
    ...
    const readWorkspaceFile = vi.fn((request: { path: string }) => {
      if (request.path === '.dagong-design/documents.json') {
    ...
    vi.stubGlobal('window', {
      dagongGui: { writeWorkspaceFile, readWorkspaceFile, listWorkspaceDirectory }
    })
  ...
  it('keeps settings unloaded until existing design documents are rehydrated', async () => {
    ...
    const readWorkspaceFile = vi.fn((request: { path: string }) => {
      if (request.path === '.dagong-design/documents.json') return indexRead.promise
      return Promise.resolve({ ok: false as const, error: 'missing' })
    })
