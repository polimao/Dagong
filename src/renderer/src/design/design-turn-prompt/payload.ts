...
async function readWorkspaceText(workspaceRoot: string, path: string): Promise<string> {
  if (typeof window === 'undefined' || typeof window.dagongGui?.readWorkspaceFile !== 'function') return ''
  const result = await window.dagongGui.readWorkspaceFile({ path, workspaceRoot }).catch(() => null)
