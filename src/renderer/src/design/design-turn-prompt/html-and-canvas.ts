...
export function buildScreenTurnPrompt(options: ScreenTurnOptions): string {
  ...
  const lines = [
    options.basePath
      ? `Dagong is asking you to ITERATE on an existing screen design: "${options.screenName}".`
      : `Dagong is asking you to design a new screen: "${options.screenName}".`,
...
export function buildCanvasTurnPrompt(options: DesignTurnOptions): string {
  ...
  const lines = [
    codeCanvasMode
      ? 'Dagong is asking you to operate the Code sidebar whiteboard with the dedicated canvas tools.'
      : 'Dagong is asking you to operate the design canvas with the dedicated design tools.',
    `Workspace: ${options.workspaceRoot}`,
