...
function artifact(kind: DesignArtifact['kind'] = 'html'): DesignArtifact {
  return {
    ...
    relativePath: `.dagong-design/doc/${kind}_1/${kind === 'html' ? 'v1.html' : 'canvas.json'}`,
    designMdPath: kind === 'html' ? '.dagong-design/doc/html_1/DESIGN.md' : undefined,
    createdAt: now,
    updatedAt: now,
    versions: []
  }
...
describe('design code roundtrip', () => {
  it('prepares a design-to-code implementation turn and publishes the design system', async () => {
    const writeWorkspaceFile = vi.fn(async () => ({
      ok: true as const,
      path: '.dagong-design/DESIGN_SYSTEM.md',
      savedAt: now
    }))
    ...
    expect(result.prompt).toContain('Design source (a standalone HTML mockup): .dagong-design/doc/html_1/v1.html')
    expect(result.prompt).toContain('Project design system: .dagong-design/DESIGN_SYSTEM.md')
    expect(result.prompt).toContain('Target stack: React + Tailwind')
    expect(result.prompt).toContain('Read the design notes `.dagong-design/doc/html_1/DESIGN.md`')
  })
  ...
  it('keeps design-system publish failures non-fatal', async () => {
    ...
    expect(result.prompt).not.toContain('Project design system: .dagong-design/DESIGN_SYSTEM.md')
  })
  ...
  it('prepares a code-to-design artifact and reverse-design prompt', () => {
    ...
    expect(prepared.artifact).toMatchObject({
      ...
      kind: 'html',
      title: 'From Home.tsx',
      relativePath: '.dagong-design/doc_1/artifact_1/v1.html'
    })
    ...
    expect(prepared.prompt).toContain('Reserved artifact file: .dagong-design/doc_1/artifact_1/v1.html')
  })
