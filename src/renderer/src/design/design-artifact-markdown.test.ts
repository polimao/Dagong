...
describe('design artifact markdown', () => {
  it('captures brief, paths, selected context, and version info', () => {
    const artifact: DesignArtifact = {
      ...
      kind: 'html',
      title: 'Login screen',
      relativePath: '.dagong-design/screen/v2.html',
      ...
      designMdPath: '.dagong-design/screen/DESIGN.md'
    }
      ...
      selectedContext: [
        {
          ...
          label: 'Login screen',
          detail: '420 x 340 - .dagong-design/screen/v2.html'
        }
      ]
    })

    expect(markdown).toContain('# Design Notes: Login screen')
    expect(markdown).toContain('Source HTML path: `.dagong-design/screen/v2.html`')
    expect(markdown).toContain('Design notes file: `.dagong-design/screen/DESIGN.md`')
    expect(markdown).toContain('Current version: v2 (`.dagong-design/screen/v2.html`)')
  ...
  it('labels the selected historical version without reordering version history', () => {
    ...
    expect(markdown).toContain('Current version: v1 (`.dagong-design/screen/v1.html`)')
    expect(markdown).toContain('- v2: `.dagong-design/screen/v2.html` - Make this a login screen')
    expect(markdown).toContain('- v1: `.dagong-design/screen/v1.html` - Create a hello world page')
  })
})
