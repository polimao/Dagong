 ...
 * mirrored to a single `.dagong-design/documents.json` index that records each
 * 设计稿's metadata + ordering + the active pointers. Artifact membership is NOT
 * stored here — it is implied by directory nesting (`.dagong-design/<docId>/<id>/`)
...
/** Best-effort creation of the physical `.dagong-design/<docId>/` directory. */
export async function ensureDocumentDir(workspaceRoot: string, docId: string): Promise<void> {
  if (!workspaceRoot || !docId || typeof window.dagongGui?.createWorkspaceDirectory !== 'function') return
  await window.dagongGui.createWorkspaceDirectory({ path: DESIGN_DIR, workspaceRoot }).catch(() => null)
  await window.dagongGui.createWorkspaceDirectory({ path: documentDirPath(docId), workspaceRoot }).catch(() => null)
}
...
function writeDocumentsIndex(workspaceRoot: string, content: string): Promise<void> {
  if (typeof window.dagongGui?.writeWorkspaceFile !== 'function') return Promise.resolve()
  return window.dagongGui
...
export function flushDocumentsIndex(
  ...
  if (!workspaceRoot || typeof window.dagongGui?.writeWorkspaceFile !== 'function') return Promise.resolve()
  if (_saveTimer) {
...
export function deleteDocumentDir(workspaceRoot: string, docId: string): Promise<void> {
  if (!workspaceRoot || typeof window.dagongGui?.deleteWorkspaceEntry !== 'function') return Promise.resolve()
  return window.dagongGui
