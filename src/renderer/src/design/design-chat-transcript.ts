 ...
 * `.dagong-design/<docId>/chat/<threadId>.md` plus `chat/meta.json`.
 *
...
export function designChatDir(docId: string): string | null {
  ...
  return safeDocId ? `.dagong-design/${safeDocId}/chat` : null
}
...
export async function hydrateDesignChatMetaForDoc(input: {
  ...
  if (!metaPath || typeof window.dagongGui?.readWorkspaceFile !== 'function') return false
  try {
