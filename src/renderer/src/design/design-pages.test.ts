...
function htmlArtifact(over: Partial<DesignArtifact> & { id: string }): DesignArtifact {
  return {
    ...
    relativePath: over.relativePath ?? `.dagong-design/${over.id}/v1.html`,
    createdAt: '2026-06-22T00:00:00.000Z',
    updatedAt: '2026-06-22T00:00:00.000Z',
    versions: over.versions ?? [
      { id: `${over.id}-v1`, relativePath: `.dagong-design/${over.id}/v1.html`, createdAt: '2026-06-22T00:00:00.000Z', summary: '' }
    ],
    ...over
  }
}
  ...
  it('excludes the active artifact and surfaces summary + node dims', () => {
    const artifacts = [
      htmlArtifact({
        ...
        versions: [{ id: 'a-v1', relativePath: '.dagong-design/a/v1.html', createdAt: 'x', summary: 'Landing page' }]
      }),
    ...
    expect(manifest[0]).toMatchObject({
      name: 'Home',
      htmlPath: '.dagong-design/a/v1.html',
      width: 420,
      height: 340,
  ...
  it('uses the summary for the artifact current relativePath, not just versions[0]', () => {
    const artifacts = [
      htmlArtifact({
        id: 'a',
        title: 'Home',
        relativePath: '.dagong-design/a/v1.html',
        versions: [
          { id: 'a-v2', relativePath: '.dagong-design/a/v2.html', createdAt: 'x2', summary: 'Newer experiment' },
          { id: 'a-v1', relativePath: '.dagong-design/a/v1.html', createdAt: 'x1', summary: 'Selected stable draft' }
        ]
      })
    ]
...
describe('buildPrototypeLinksForPage', () => {
  it('resolves planner links to target artifacts and local hrefs', () => {
    const links = buildPrototypeLinksForPage(
      ...
      [
        { title: 'Home', artifactId: 'home', relativePath: '.dagong-design/doc/home/v1.html' },
        { title: 'Signup', artifactId: 'signup', relativePath: '.dagong-design/doc/signup/v1.html' }
      ]
    )
  ...
  it('resolves planner links with unique partial screen title matches', () => {
    const links = buildPrototypeLinksForPage(
      ...
      '.dagong-design/doc/dashboard/v1.html',
      [
        { title: 'Dashboard', artifactId: 'dashboard', relativePath: '.dagong-design/doc/dashboard/v1.html' },
        { title: 'Account Settings', artifactId: 'settings', relativePath: '.dagong-design/doc/settings/v1.html' },
        { title: 'Reports', artifactId: 'reports', relativePath: '.dagong-design/doc/reports/v1.html' }
      ]
    )
  ...
  it('does not guess ambiguous partial screen title matches', () => {
    const links = buildPrototypeLinksForPage(
      ...
      [
        { title: 'Dashboard', artifactId: 'dashboard', relativePath: '.dagong-design/doc/dashboard/v1.html' },
        { title: 'Account Settings', artifactId: 'account-settings', relativePath: '.dagong-design/doc/account/v1.html' },
        { title: 'Team Settings', artifactId: 'team-settings', relativePath: '.dagong-design/doc/team/v1.html' }
      ]
    )
  ...
  it('adds a concrete fallback link when explicit planner links do not match any page', () => {
    const links = buildPrototypeLinksForPage(
      ...
      [
        { title: 'Dashboard', artifactId: 'dashboard', relativePath: '.dagong-design/doc/dashboard/v1.html' },
        { title: 'Review Queue', artifactId: 'review-queue', relativePath: '.dagong-design/doc/review/v1.html' },
        { title: 'Settings', artifactId: 'settings', relativePath: '.dagong-design/doc/settings/v1.html' }
      ]
    )
  ...
  it('does not guess duplicate exact screen title matches', () => {
    const links = buildPrototypeLinksForPage(
      ...
      [
        { title: 'Dashboard', artifactId: 'dashboard', relativePath: '.dagong-design/doc/dashboard/v1.html' },
        { title: 'Settings', artifactId: 'account-settings', relativePath: '.dagong-design/doc/account-settings/v1.html' },
        { title: 'Settings', artifactId: 'project-settings', relativePath: '.dagong-design/doc/project-settings/v1.html' }
      ]
    )
  ...
  it('adds a deterministic next-page prototype fallback when the planner omits links', () => {
    const links = buildPrototypeLinksForPage(
      ...
      [
        { title: 'Home', artifactId: 'home', relativePath: '.dagong-design/doc/home/v1.html' },
        { title: 'Checkout', artifactId: 'checkout', relativePath: '.dagong-design/doc/checkout/v1.html' }
      ]
    )
...
describe('buildDesignPlanPrompt', () => {
  it('embeds the brief, bounds the page count, and lists existing pages', () => {
    const prompt = buildDesignPlanPrompt({
      ...
      workspaceRoot: '/ws',
      maxPages: 99,
      existingPages: [{ name: 'Login', htmlPath: '.dagong-design/x/v1.html', summary: 'auth' }]
    })
    ...
    const running: ChatBlock[] = [
      {
        ...
        status: 'running',
        detail: JSON.stringify({ label: 'page:landing', prompt: 'write .dagong-design/doc/landing/v1.html' }),
