...
function image(partial: Partial<SddDraftImageReference> = {}): SddDraftImageReference {
  return {
    ...
    relativePath: '.magicpocketsdd/img/wireframe.png',
    mimeType: 'image/png',
...
describe('buildSddDraftToPlanPrompt', () => {
  it('keeps Markdown image syntax and maps visual attachments by image number', () => {
    const prompt = buildSddDraftToPlanPrompt({
      ...
      draftRelativePath: '.magicpocketsdd/draft/draft-1/requirement.md',
      planRelativePath: '.magicpocketsdd/plan/sdd-draft-1.md',
    ...
    expect(prompt).toContain('Reserved plan file: .magicpocketsdd/plan/sdd-draft-1.md')
    expect(prompt).toContain('`plan_relative_path` to `.magicpocketsdd/plan/sdd-draft-1.md`')
  })
