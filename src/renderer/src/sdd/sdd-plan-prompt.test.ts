...
function image(partial: Partial<SddDraftImageReference> = {}): SddDraftImageReference {
  return {
    ...
    relativePath: '.dagongsdd/img/wireframe.png',
    mimeType: 'image/png',
...
describe('buildSddDraftToPlanPrompt', () => {
  it('keeps Markdown image syntax and maps visual attachments by image number', () => {
    const prompt = buildSddDraftToPlanPrompt({
      ...
      draftRelativePath: '.dagongsdd/draft/draft-1/requirement.md',
      planRelativePath: '.dagongsdd/plan/sdd-draft-1.md',
    ...
    expect(prompt).toContain('Reserved plan file: .dagongsdd/plan/sdd-draft-1.md')
    expect(prompt).toContain('`plan_relative_path` to `.dagongsdd/plan/sdd-draft-1.md`')
  })
