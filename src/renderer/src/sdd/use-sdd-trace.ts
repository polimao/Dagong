...
export function useSddTrace(input: {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      ...
      const requirement = await window.dagongGui
      ...
      if (!planIsActive) {
        const plan = await window.dagongGui
      ...
      if (tracePath) {
        const trace = await window.dagongGui
  ...
  useEffect(() => {
    ...
    const run = async (): Promise<void> => {
      try {
        ...
        if (typeof window.dagongGui?.writeWorkspaceFile !== 'function') return
