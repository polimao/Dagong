...
export function useSddTrace(input: {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      ...
      const requirement = await window.magicpocketGui
      ...
      if (!planIsActive) {
        const plan = await window.magicpocketGui
      ...
      if (tracePath) {
        const trace = await window.magicpocketGui
  ...
  useEffect(() => {
    ...
    const run = async (): Promise<void> => {
      try {
        ...
        if (typeof window.magicpocketGui?.writeWorkspaceFile !== 'function') return
