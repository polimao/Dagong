...
describe('sdd-draft-store', () => {
  ...
  it('creates and remembers the active draft per workspace', () => {
    ...
    expect(draft.id).toBe('/tmp/app:.dagongsdd/requirements/123e4567-e89b-12d3-a456-426614174000/requirement.md')
    ...
    localStorage.setItem(SDD_DRAFT_REGISTRY_STORAGE_KEY, JSON.stringify({
      ...
      drafts: {
        valid: {
          ...
          relativePath: '.dagongsdd/requirements/123e4567-e89b-12d3-a456-426614174000/requirement.md',
  ...
  it('saves the active draft to disk and updates clean state', async () => {
    const writeWorkspaceFile = vi.fn().mockResolvedValue({
      ok: true,
      path: '/tmp/app/.dagongsdd/requirements/123e4567-e89b-12d3-a456-426614174000/requirement.md',
      savedAt: '2026-01-01T00:00:00.000Z'
    })
    window.dagongGui.writeWorkspaceFile = writeWorkspaceFile
    const draft = createSddDraft({
      id: '123e4567-e89b-12d3-a456-426614174000',
      workspaceRoot: '/tmp/app',
      now: 1
    ...
    expect(writeWorkspaceFile).toHaveBeenCalledWith({
      workspaceRoot: '/tmp/app',
      path: '.dagongsdd/requirements/123e4567-e89b-12d3-a456-426614174000/requirement.md',
  ...
  it('deletes a draft folder and clears the active remembered draft', async () => {
    const deleteWorkspaceEntry = vi.fn().mockResolvedValue({
      ok: true,
      path: '/tmp/app/.dagongsdd/requirements/123e4567-e89b-12d3-a456-426614174000',
      deletedAt: '2026-01-01T00:00:00.000Z'
    })
    window.dagongGui.deleteWorkspaceEntry = deleteWorkspaceEntry
    const draft = createSddDraft({
      id: '123e4567-e89b-12d3-a456-426614174000',
      workspaceRoot: '/tmp/app',
      now: 1
    ...
    expect(deleteWorkspaceEntry).toHaveBeenCalledWith({
      workspaceRoot: '/tmp/app',
      path: '.dagongsdd/requirements/123e4567-e89b-12d3-a456-426614174000'
    })
    expect(readRememberedSddDraft('/tmp/app')).toBeNull()
  ...
  it('forgets a remembered draft when its disk folder is already missing', async () => {
    window.dagongGui.deleteWorkspaceEntry = vi.fn().mockResolvedValue({
      ok: false,
  ...
  it('refreshes a clean active draft from disk changes', async () => {
    const draft = createSddDraft({
      id: '123e4567-e89b-12d3-a456-426614174000',
      workspaceRoot: '/tmp/app',
      absolutePath: '/tmp/app/.dagongsdd/requirements/123e4567-e89b-12d3-a456-426614174000/requirement.md',
      now: 1
    })
    useSddDraftStore.getState().setActiveDraft(draft, '# Draft')
  ...
  it('keeps the draft dirty when disk save fails', async () => {
    window.dagongGui.writeWorkspaceFile = vi.fn().mockResolvedValue({
      ok: false,
