...
function directory(entries: Array<{ name: string, type: 'file' | 'directory', path?: string }>): WorkspaceDirectoryListResult {
  return {
    ok: true,
    root: '/tmp/app/.dagongsdd/requirements',
    entries: entries.map((entry) => ({
      name: entry.name,
      path: entry.path ?? `/tmp/app/.dagongsdd/requirements/${entry.name}`,
      type: entry.type,
...
describe('sdd-draft-history', () => {
  ...
  it('discovers disk-only requirement drafts under .dagongsdd/requirements', async () => {
    ...
    const listWorkspaceDirectory = vi.fn(async (options: WorkspaceDirectoryTarget) => {
      if (options.path === '.dagongsdd/requirements') {
        return directory([
      ...
      if (options.path === `.dagongsdd/requirements/${draftId}`) {
        return directory([
          {
            name: 'requirement.md',
            type: 'file',
            path: `/tmp/app/.dagongsdd/requirements/${draftId}/requirement.md`
          }
        ])
      }
    ...
    expect(history[0]).toMatchObject({
      ...
      relativePath: `.dagongsdd/requirements/${draftId}/requirement.md`,
