...
describe('plan-path', () => {
  it('keeps readable Chinese feature names', () => {
    ...
    expect(buildPlanRelativePath('做一个登录页')).toBe('.dagongsdd/plan/做一个登录页.md')
  })
  ...
  it('falls back for empty or unsafe names', () => {
    ...
    expect(buildPlanRelativePath('../')).toBe('.dagongsdd/plan/plan.md')
  })

  it('selects the next available duplicate path', () => {
    expect(
      nextAvailablePlanRelativePath('login', [
        '.dagongsdd/plan/login.md',
        '.dagongsdd/plan/login-2.md'
      ])
    ).toBe('.dagongsdd/plan/login-3.md')
  })

  it('accepts only direct markdown files inside the GUI plan directory', () => {
    expect(isGuiPlanRelativePath('.dagongsdd/plan/login.md')).toBe(true)
    expect(isGuiPlanRelativePath('.deepseekgui/plan/login.md')).toBe(true)
    expect(isGuiPlanRelativePath('.dagongsdd/plan/nested/login.md')).toBe(false)
    expect(isGuiPlanRelativePath('../.dagongsdd/plan/login.md')).toBe(false)
    expect(isGuiPlanRelativePath('.dagongsdd/plan/login.txt')).toBe(false)
  })
})
