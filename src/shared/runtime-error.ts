/**
 * Shared parser for Dagong runtime error bodies.
 *
 * Dagong's contract (`dagong/src/contracts/errors.ts`) returns
 ...
 * always carries either a Dagong contract code or one of the
 * `LEGACY_MAIN_GUARD_CODES` (main-process guard codes that aren't
 * part of the Dagong schema). `details` carries the original
 * payload untouched so callers that need more context can read it.
 */
export type DagongErrorCode =
  | 'validation_error'
  | 'unauthorized'
  | 'forbidden'
  | 'not_found'
...
export type RuntimeErrorCode = DagongErrorCode | LegacyMainGuardCode
...
const KNOWN_KUN_CODES: ReadonlySet<DagongErrorCode> = new Set<DagongErrorCode>([
  'validation_error',
  'unauthorized',
  'forbidden',
  'not_found',
...
function normalizeCode(value: unknown): RuntimeErrorCode {
  ...
  if ((KNOWN_KUN_CODES as Set<string>).has(value)) return value as DagongErrorCode
...
/**
 * Parse a Dagong runtime error body. Falls back to the supplied
...
export function isKnownDagongErrorCode(value: unknown): value is DagongErrorCode {
