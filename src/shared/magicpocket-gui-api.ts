...
export type MagicPocketRuntimeStatusPayload = {
...
export type LegacySessionSourceKind = 'magicpocket' | 'coreagent' | 'custom'
...
export type LegacySessionDetectResult = {
  /** Destination threads directory (current MagicPocket data dir + /threads). */
  destDir: string
...
export type MagicPocketGuiApi = {
  ...
  getMagicPocketConfigFile: () => Promise<DeepseekConfigFileResult>
  setMagicPocketConfigFile: (content: string) => Promise<DeepseekConfigSaveResult>
  openMagicPocketConfigDir: () => Promise<PathOpenResult>
  ...
  onRuntimeStatus: (handler: (payload: MagicPocketRuntimeStatusPayload) => void) => () => void
