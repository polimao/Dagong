export const GUI_PLAN_RELATIVE_DIR = '.dagongsdd/plan'
...
/**
 * Stable name of the native Dagong plan tool. Kept distinct from the
 * historical `gui_plan_create` MCP bridge so the renderer and Dagong
 ...
 * plan/refine turn so Dagong can scope tool availability to the
 * active plan context.
 */
 ...
 * the public surface the model sees; validation is enforced by Dagong
...
/**
 * Build the deterministic plan id used by both renderer and Dagong.
...
export function validateCreatePlanToolInput(input: Partial<CreatePlanToolInput>): string[] {
  ...
  if (input.plan_relative_path != null) {
    ...
    if (!path) {
      ...
      issues.push('plan_relative_path must be a direct Markdown file under .dagongsdd/plan')
    }
  }
 ...
 * plan path checks. Used by Dagong to verify the active workspace
