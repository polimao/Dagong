export const GUI_PLAN_RELATIVE_DIR = '.magicpocketsdd/plan'
...
/**
 * Stable name of the native MagicPocket plan tool. Kept distinct from the
 * historical `gui_plan_create` MCP bridge so the renderer and MagicPocket
 ...
 * plan/refine turn so MagicPocket can scope tool availability to the
 * active plan context.
 */
 ...
 * the public surface the model sees; validation is enforced by MagicPocket
...
/**
 * Build the deterministic plan id used by both renderer and MagicPocket.
...
export function validateCreatePlanToolInput(input: Partial<CreatePlanToolInput>): string[] {
  ...
  if (input.plan_relative_path != null) {
    ...
    if (!path) {
      ...
      issues.push('plan_relative_path must be a direct Markdown file under .magicpocketsdd/plan')
    }
  }
 ...
 * plan path checks. Used by MagicPocket to verify the active workspace
