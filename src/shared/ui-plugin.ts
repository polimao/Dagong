...
/** 形象槽位:缺失的槽位回退默认 Dagong 美术(允许"半皮肤") */
...
/**
 * 与内置模式、DOM 属性值保留字互斥。
 * 注意:'idagong' 不在保留字里 —— 内置的 iDagong 模式本身就是一个预装 UI 插件
 * (见 src/main/ui-plugin-bundled.ts),id 为 'idagong' 时额外点亮
 * data-idagong-mode 的手工 CSS 机制。
 */
const UI_PLUGIN_RESERVED_IDS = new Set(['default', 'dagong', 'on', 'off', 'none'])

/** 预装示例插件(iDagong)的 id:激活时会同时启用 data-idagong-mode 手工动画机制 */
export const UI_PLUGIN_BUNDLED_IKUN_ID = 'idagong'
 ...
 * 插件配色的根因。对应 iDagong 既有的
 * `[data-theme='dark'][data-idagong-mode='on'] .ds-workbench-shell` 处理。
