...
export type DagongRuntimeStatusPayload = {
...
export type LegacySessionSourceKind = 'dagong' | 'coreagent' | 'custom'
...
export type LegacySessionDetectResult = {
  /** Destination threads directory (current Dagong data dir + /threads). */
  destDir: string
...
export type DagongGuiApi = {
  ...
  getDagongConfigFile: () => Promise<DeepseekConfigFileResult>
  setDagongConfigFile: (content: string) => Promise<DeepseekConfigSaveResult>
  openDagongConfigDir: () => Promise<PathOpenResult>
  ...
  onRuntimeStatus: (handler: (payload: DagongRuntimeStatusPayload) => void) => () => void
