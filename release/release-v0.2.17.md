...
这一版的主角是子代理系统。Dagong 开始支持更完整的子代理 profile、按 profile 路由模型、独立子会话、实时子任务展示，以及 `.dagong/agents/*.md` 这样的项目级代理定义。同时，Skill/MCP、文档附件、Agents 设置和上下文压缩也完成了一轮系统升级。
...
- 支持 `.dagong/agents/*.md` 覆盖项目代理定义，并提供 AI 草稿、detach 与 abort 能力。
...
- Agents 配置界面重写，整合模型配置、完整管理、动画 Dagong 与 i18n。
...
- 如果你想使用子代理，可以在 Agents 设置中配置 profile；项目内也可以用 `.dagong/agents/*.md` 保存团队共享代理定义。
- 如果之前手动维护过 Skill 目录，升级后建议检查全局和项目 Skill roots 的开关状态。

### 总结

v0.2.17 让 Dagong 的“多代理协作”正式成型：子代理不再只是一次工具调用，而是有 profile、有模型、有权限、有 UI、有项目定义的独立工作单元。配合全局 Skill、MCP `cwd`、文档附件和设置重写，这一版把 Agent 生态往可管理、可复用的方向推进了一大步。
