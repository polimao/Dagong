...
copy_mac_arch_artifacts() {
  ...
  shopt -s nullglob
  files=("${output_dir}"/Dagong-*-mac-"${arch}".*)
  shopt -u nullglob
...
# artifactName: Dagong-${version}-mac-${arch}.dmg|zip
collect "macOS arm64 dmg" "dist/Dagong-*-mac-arm64.dmg"
collect "macOS x64 dmg" "dist/Dagong-*-mac-x64.dmg"
collect "macOS arm64 zip" "dist/Dagong-*-mac-arm64.zip"
collect "macOS x64 zip" "dist/Dagong-*-mac-x64.zip"
collect_optional "macOS blockmap" "dist/Dagong-*-mac-*.zip.blockmap"
...
cyan "  https://github.com/DagongAgent/Dagong/releases/tag/${TAG_NAME}"
