...
describe('AgentLoop', () => {
  ...
  it('includes the failure reason on turn_failed events', async () => {
    ...
    expect(failed?.kind === 'turn_failed' ? failed.message : '').toContain('[Dagong turn failed]')
  })
  ...
  it('stops the turn when an existing tool schema mutates in-place', async () => {
    ...
    expect(items.find((item) => item.kind === 'error' && item.code === 'tool_catalog_changed'))
      .toMatchObject({
        kind: 'error',
        message: expect.stringContaining('Dagong stopped this turn')
      })
  })
  ...
  it('uses persisted GUI plan context to advertise and execute create_plan', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-'))
    ...
    try {
      ...
      await bootstrapThread(h, {
        ...
        request: {
          ...
          guiPlan: {
            ...
            planId: `${workspace}:.dagongsdd/plan/auth.md`,
      ...
      await expect(readFile(join(workspace, '.dagongsdd/plan/auth.md'), 'utf8')).resolves.toBe('# Generated plan')
      const turn = await h.turns.getTurn(h.threadId, h.turnId)
      expect(turn?.guiPlan?.relativePath).toBe('.dagongsdd/plan/auth.md')
      const items = await h.sessionStore.loadItems(h.threadId)
      ...
      if (result?.kind === 'tool_result') {
        ...
        expect(result.output).toMatchObject({
          relative_path: '.dagongsdd/plan/auth.md',
  ...
  it('materializes assistant plan text when a GUI plan turn misses create_plan', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-missing-tool-'))
    try {
      const h = makeHarness(
        {
          provider: 'planner',
      ...
      expect(status).toBe('completed')
      await expect(readFile(join(workspace, '.dagongsdd/plan/auth.md'), 'utf8')).resolves.toBe(
        '## Plan\nImplement auth.'
      )
  ...
  it('materializes assistant plan text for plan-mode turns without a reserved context', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-free-form-text-'))
    try {
      const h = makeHarness(
        {
          provider: 'planner',
      ...
      ).toBe('.dagongsdd/plan/plan-sidebar-footer-polish.md')
      await expect(readFile(join(workspace, '.dagongsdd/plan/plan-sidebar-footer-polish.md'), 'utf8')).resolves.toBe(
        '## Plan\nPolish the sidebar footer.'
      )
    } finally {
      await rm(workspace, { recursive: true, force: true })
  ...
  it('pauses a GUI plan turn for a clarifying question instead of materializing a plan', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-clarify-'))
    try {
      const h = makeHarness(
        {
          provider: 'planner',
  ...
  it('rejects forged write calls during plan mode without touching workspace files', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-forged-write-'))
    const observedToolLists: string[][] = []
    let calls = 0
    try {
      const h = makeHarness(
      ...
      await expect(readFile(join(workspace, '.dagongsdd/plan/plan-a-safe-change.md'), 'utf8')).resolves.toBe(
        '## Plan\nStay read-only until build mode.'
      )
    } finally {
      await rm(workspace, { recursive: true, force: true })
  ...
  it('rejects forged bash calls during plan mode without running mutating commands', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-forged-bash-'))
    let calls = 0
    try {
      const h = makeHarness(
        {
      ...
      expect(bashResult?.kind === 'tool_result' ? JSON.stringify(bashResult.output) : '')
        .toContain('not advertised by active tool policy')
      await expect(readFile(join(workspace, 'forbidden.txt'), 'utf8')).rejects.toThrow()
      await expect(readFile(join(workspace, '.dagongsdd/plan/plan-without-shell-mutations.md'), 'utf8')).resolves.toBe(
        '## Plan\nUse read-only inspection only.'
      )
    } finally {
      await rm(workspace, { recursive: true, force: true })
  ...
  it('fails GUI plan turns only when neither create_plan nor plan text is returned', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-empty-'))
    try {
      const h = makeHarness(
        {
          provider: 'planner',
  ...
  it('keeps requiring create_plan after unrelated tool calls in a GUI plan turn', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-loop-plan-other-tool-'))
    const observedRequiredToolNames: Array<string | undefined> = []
    let calls = 0
    try {
      const h = makeHarness(
  ...
  it('embeds a digest marker and skips frozen messages when compacting history', () => {
    ...
    expect(result.summaryItem.kind === 'compaction' ? result.summaryItem.digestMarker : '')
      .toBe(`<dagong:tool_digest sha256="${result.summaryItem.kind === 'compaction' ? result.summaryItem.sourceDigest : ''}">`)
    expect(result.summaryItem.kind === 'compaction' ? result.summaryItem.sourceItemIds : [])
      .toEqual(['u1', 'a1'])
  ...
  it('records a visible fallback event when configured model compaction summaries fail', async () => {
    ...
    expect(persisted.some((item) =>
      ...
      item.summary.includes('<dagong:tool_digest sha256=')
    )).toBe(true)
  })
...
describe('FileSessionStore', () => {
  let dataDir = ''
  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'dagong-test-'))
