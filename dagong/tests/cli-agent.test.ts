...
describe('Dagong agent CLI commands', () => {
  let dataDir = ''

  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'dagong-cli-'))
  })
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitDagongCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitDagongCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitDagongCliCommand(['nope']).error).toMatch(/unknown command/)
  })

  it('lists tools from dagong exec with JSON output', async () => {
    const c = capture()
  ...
  it('invokes a direct tool through dagong exec', async () => {
    const c = capture()
    const code = await runAgentCommand('exec', [
      '--data-dir',
      dataDir,
  ...
  it('lists dynamic runtime tools from dagong exec', async () => {
  ...
  it('shares config loading between serve and agent commands', async () => {
    const configPath = join(dataDir, 'dagong.config.json')
