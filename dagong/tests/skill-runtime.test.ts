...
describe('SkillRuntime', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'dagong-skills-'))
  })
  ...
  it('discovers a skill package symlinked into a root (e.g. cc switch)', async (ctx) => {
    ...
    const realDir = await mkdtemp(join(tmpdir(), 'dagong-skill-real-'))
    try {
  ...
  it('scopes the dynamic skill catalog to the current workspace', async () => {
    const workspaceA = await mkdtemp(join(tmpdir(), 'dagong-skill-workspace-a-'))
    const workspaceB = await mkdtemp(join(tmpdir(), 'dagong-skill-workspace-b-'))
    try {
      ...
      const disabledConfig = DagongCapabilitiesConfig.parse({
        skills: {
          enabled: true,
          roots: [rootA],
          workspaceRoots: [workspaceA, workspaceB],
