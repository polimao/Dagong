...
describe('Dagong built-in tools', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'dagong-tools-'))
    backgroundShellDataDir = await mkdtemp(join(tmpdir(), 'dagong-bg-shell-data-'))
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello dagong' },
    ...
    expect(editedDisk).toContain('hello dagong')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello dagong')
