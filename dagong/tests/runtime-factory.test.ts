...
import { createDagongServeRuntime, seedUsageCarryover } from '../src/server/runtime-factory.js'
...
describe('runtime factory usage carryover', () => {
  ...
  it('hot-applies model and tool capabilities into runtime info and diagnostics', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'dagong-runtime-apply-'))
    tempDirs.push(dataDir)
    const runtime = await createDagongServeRuntime({
      host: '127.0.0.1',
      port: 0,
      dataDir,
      runtimeToken: 'tok',
      ...
      capabilities: DagongCapabilitiesConfig.parse({})
    })
