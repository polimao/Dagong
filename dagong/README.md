...
Dagong is the local HTTP/SSE agent runtime for the Dagong desktop app. It exposes a
TypeScript-typed agent loop with a stable, GUI-friendly contract:

- `dagong serve` starts a local HTTP server with `/v1/*` routes.
...
The name Dagong is inspired by the great fish in Zhuangzi's line,
"In the northern sea there is a fish; its name is Dagong." In
...
Dagong's core goal is to improve the ROI of every token. Tokens should be
...
Start a Dagong runtime, set `KUN_RUNTIME_URL` and `KUN_RUNTIME_TOKEN`, then run the five-task smoke set:
...
TTFT, full latency, tool time, SSE delivery delay, token/cache/cost counters, and Dagong process peak RSS. The runtime
token is accepted only through `KUN_RUNTIME_TOKEN`, so it does not leak through process arguments.

## CLI

`dagong serve` accepts the following flags:

| Flag | Description | Default |
| --- | --- | --- |
| `--config` | JSON config file. If omitted, Dagong reads `{--data-dir}/config.json` when present | optional |
...
```bash
dagong serve \
  --config ~/.deepseekgui/dagong/config.json \
  --host 127.0.0.1 \
  --port 18899 \
  --data-dir ~/.deepseekgui/dagong \
  --runtime-token dev-token \
  --api-key "$DEEPSEEK_API_KEY" \
  --model deepseek-v4-pro
```

Dagong can also run as a standalone agent without the GUI:

```bash
dagong run --data-dir ~/.deepseekgui/dagong --workspace "$PWD" "summarize this repo"
dagong chat --data-dir ~/.deepseekgui/dagong --workspace "$PWD"
dagong exec --data-dir ~/.deepseekgui/dagong --workspace "$PWD" --list-tools
dagong exec --data-dir ~/.deepseekgui/dagong --workspace "$PWD" read --args '{"path":"README.md"}'
```

- `dagong run` creates a thread, runs one turn, streams assistant text, and exits.
- `dagong chat` starts a line-oriented REPL. Use `/exit`, `/quit`, or an empty line to stop.
- `dagong exec --list-tools` prints the effective dynamic tool registry for the chosen config/workspace.
- `dagong exec <tool> --args <json>` invokes one tool directly. Use `--json` on `run` or `exec` for machine-readable output.
...
Dagong supports a JSON config file so runtime behavior can be managed
...
Dagong also reads `{data-dir}/config.json` when it exists. In the GUI's
...
Dagong defaults to hybrid session storage: `threads/{threadId}/messages.jsonl`
...
- `capabilities.mcp` starts configured MCP clients and imports their tools into the dynamic registry. Workspace-scoped servers require `trustedWorkspaceRoots`. Remote HTTP/SSE servers can use `oauth`; Dagong stores OAuth tokens under the data directory instead of the config file. Use `GET /v1/mcp/oauth` to inspect redacted OAuth state and `DELETE /v1/mcp/oauth/{serverId}` to clear a server's saved authorization.
...
lifecycle without rebuilding Dagong. They are configured under the
top-level `hooks` key in `config.json` (so the GUI's
`~/.deepseekgui/dagong/config.json` works out of the box) and run inside
  ...
  "hooks": [
    {
      ...
      "command": "node ~/.dagong-hooks/guard.js",
      "timeoutMs": 10000
    },
    { "phase": "UserPromptSubmit", "command": "~/.dagong-hooks/prompt-context.sh" },
    { "phase": "TurnEnd", "command": "~/.dagong-hooks/notify.sh" }
  ]
}
```
...
See `../docs/dagong-hooks.en.md` for the full reference: per-phase stdin
...
```
{--data-dir}/
  config.json      # Optional Dagong runtime config
...
3. Restart Dagong or refresh diagnostics.
...
- Memory is not injected: enable `capabilities.memory`, confirm
  ...
  `lastInjectedIds`.
- `dagong run`, `dagong chat`, or `dagong exec` cannot authenticate or load
  config: pass the same `--config`, `--data-dir`, `--api-key`,
  `--base-url`, and `--runtime-token` values used by `dagong serve`.
  `dagong exec --list-tools --json` is the quickest way to verify the
...
starts Dagong through `dagong-process.ts` and routes all
...
CodeWhale provider because Dagong speaks the same HTTP/SSE
contract. Settings live under `agents.dagong` in
...
`agents.dagong`; the saved settings file no longer keeps live
