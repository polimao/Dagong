...
export const GuiPlanContextSchema = z.object({
  ...
  relativePath: z
    ...
    .refine(isGuiPlanRelativePath, {
      message: 'relativePath must be a direct Markdown file under .dagongsdd/plan'
    }),
...
export const TurnSchema = z.object({
  ...
  /**
   * True for renderer-owned design canvas turns. Dagong advertises the
   ...
   * headless runs). Dagong hides `user_input`/`request_user_input` and
...
export const StartTurnRequest = z.object({
   ...
   * mode Dagong advertises `create_plan` for the whole conversation.
   */
  mode: TurnModeSchema.optional(),
  attachments: z
    .array(
  ...
  workspaceCheckpointId: z.string().min(1).optional(),
  /**
   * Optional GUI plan context. When set, Dagong advertises the
