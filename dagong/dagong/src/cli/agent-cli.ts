...
import { createDagongServeRuntime } from '../server/runtime-factory.js'
...
export const KUN_CLI_USAGE = `dagong <command> [options]
...
export type DagongCliCommand = 'serve' | 'run' | 'chat' | 'exec' | 'help'

export function splitDagongCliCommand(argv: readonly string[]): {
  command: DagongCliCommand
  args: string[]
  error?: string
} {
  const first = argv[0]
...
export async function runAgentCommand(
  command: Exclude<DagongCliCommand, 'serve' | 'help'>,
...
async function runOneShot(argv: readonly string[], io: CliIo): Promise<number> {
  ...
  if (!parsed.ok) return writeParseError(parsed, io, 'dagong run')
  const jsonl = hasFlag(argv, 'jsonl')
  if (parsed.json && jsonl) {
    io.stderr.write('dagong run: --json and --jsonl are mutually exclusive\n')
    return ServeExitCode.usage
  }
  const prompt = stringFlag(argv, ['prompt', 'p']) ?? positionals(argv).join(' ').trim()
  if (!prompt) {
    io.stderr.write('dagong run: missing prompt\n')
    return ServeExitCode.usage
  }
  ...
  try {
    ...
    io.stderr.write(`dagong run: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'dagong run')
  }
}
...
async function runChat(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'dagong chat')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
    const thread = await runtime.threadService.create({
    ...
    io.stderr.write(`dagong chat: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'dagong chat')
  }
}
...
async function runExec(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'dagong exec')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
  } catch (error) {
    io.stderr.write(`dagong exec: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  }
  ...
  try {
    ...
    if (!toolName) {
      io.stderr.write('dagong exec: missing tool name (use --list-tools to inspect tools)\n')
      return ServeExitCode.usage
    }
    const argsText = stringFlag(argv, ['args']) ?? '{}'
    const args = parseJsonObject(argsText)
    if (!args.ok) {
      io.stderr.write(`dagong exec: ${args.message}\n`)
    ...
    await shutdownRuntime(runtime, io, 'dagong exec')
  }
}
...
function createRuntime(options: ServeOptions, io: CliIo): Promise<ServerRuntime> {
  return io.createRuntime ? io.createRuntime(options) : createDagongServeRuntime(options)
}
