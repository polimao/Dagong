/**
 * Translates the Claude Agent SDK message stream into dagong's native runtime
 * events, so a subscription turn renders in the GUI exactly like a turn driven
 * by dagong's own agent loop. This is the load-bearing half of the fusion: the SDK
 * owns the loop, but every assistant token, tool call, and tool result it emits
 * is re-projected onto dagong's event contract.
 *
 ...
 *
 * Streaming model (mirrors dagong's native loop exactly, or the GUI double-renders):
 * a dagong `assistant_text_delta` event carries an INCREMENTAL CHUNK (the GUI
...
/** Claude Code built-in tool names that imply a richer dagong tool kind. */
...
/** Collapse an SDK tool_result content payload into a dagong tool output value. */
...
/**
 * Map the Agent SDK's per-request usage onto dagong's UsageSnapshot. Anthropic's
