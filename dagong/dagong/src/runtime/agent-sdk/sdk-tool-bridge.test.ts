...
describe('selectBridgeableTools', () => {
  test('drops Claude Code overlap tools and excluded tools, keeps dagong-exclusive', () => {
    const tools = [
      tool('read'),
      tool('bash'),
      tool('edit'),
    ...
    // now bridged so dagong's GUI input panel handles interactive questions.
...
describe('mapDagongResultToSdkContent', () => {
  test('passes string output through', () => {
    expect(mapDagongResultToSdkContent({ output: 'hello' })).toEqual({
      content: [{ type: 'text', text: 'hello' }]
    })
  })

  test('JSON-stringifies structured output', () => {
    const out = mapDagongResultToSdkContent({ output: { a: 1 } })
    expect(out.content[0].text).toContain('"a": 1')
  })

  test('flags errors', () => {
    expect(mapDagongResultToSdkContent({ output: 'bad', isError: true })).toMatchObject({ isError: true })
  })
})

describe('buildBridgedToolSpecs', () => {
  test('handler invokes the dagong executor and maps the result', async () => {
  ...
  test('a dagong isError result is propagated', async () => {
...
describe('bridgedToolModelNames', () => {
  test('namespaces under mcp__dagong__', () => {
    const specs = buildBridgedToolSpecs([tool('memory_create'), tool('lsp')], async () => ({ output: '' }))
    expect(bridgedToolModelNames(specs)).toEqual(['mcp__dagong__memory_create', 'mcp__dagong__lsp'])
  })
})
