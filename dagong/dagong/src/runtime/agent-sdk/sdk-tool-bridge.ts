/**
 * Re-exposes dagong's own tools to the Claude Agent SDK as in-process MCP tools.
 * This is the inbound half of the fusion: the SDK owns the loop, but when the
 * model calls one of dagong's tools the handler runs dagong's real executor in-process
 * — so generate_image, computer_use, memory, web search, delegate_task (and thus
 * dagong's richer subagents), etc. all keep working on a subscription turn.
 *
 ...
 * native ones. We only bridge dagong-EXCLUSIVE tools. `delegate_task` is bridged
 * rather than mapped to the SDK `agents` option because dagong's delegation is
...
/** Structural view of a dagong LocalTool (decoupled from dagong's tool internals). */
...
export interface DagongToolResult {
  output: unknown
  isError?: boolean
}

/** Executes a dagong tool by name for the active turn (closes over ToolHostContext). */
export type DagongToolExecutor = (toolName: string, args: Record<string, unknown>) => Promise<DagongToolResult>
...
/** dagong built-ins that overlap Claude Code built-ins — use the SDK's instead. */
...
/**
 * dagong tools better handled by the SDK's own surfaces or meaningless here.
 * NOTE: user_input/request_user_input are intentionally NOT excluded — they are
 * bridged so the model uses dagong's own GUI input panel (wired via the tool
...
/** Filter a tool catalog down to the dagong-exclusive tools worth bridging. */
...
/** Collapse a dagong tool result into the SDK MCP tool content shape. */
export function mapDagongResultToSdkContent(result: DagongToolResult): SdkToolContent {
  const text =
...
/**
 * Build SDK MCP tool specs whose handlers run dagong's executor. A throwing or
  ...
  return tools.map((tool) => ({
    ...
    handler: async (args: Record<string, unknown>): Promise<SdkToolContent> => {
      try {
        ...
        return mapDagongResultToSdkContent(result)
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err)
        return {
          content: [{ type: 'text', text: `dagong tool "${tool.name}" failed: ${message}` }],
          isError: true
        }
      }
    }
 ...
 * dagong validates arguments inside its own executor, so this only needs to convey
 ...
 * server named `dagong`. The model sees these as `mcp__dagong__<toolName>`.
...
export function bridgedToolModelNames(specs: readonly BridgedToolSpec[], serverName = 'dagong'): string[] {
