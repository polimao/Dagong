...
export class FileMcpOAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    return {
      client_name: this.server.oauth?.clientName ?? `Dagong MCP Client (${this.serverId})`,
  ...
  private startCallbackServer(): { code: Promise<string>; ready: Promise<void>; cancel: () => void } {
    ...
    const server = createServer((request, response) => {
      ...
      if (!this.pendingState || state !== this.pendingState) {
        ...
        response.end('<!doctype html><title>Dagong OAuth</title><p>Authorization state mismatch.</p>')
        return
      }
      ...
      if (code) {
        ...
        response.end('<!doctype html><title>Dagong OAuth</title><p>Authorization complete. You can close this window.</p>')
        resolveOnce(code)
      } else {
        response.writeHead(400, { 'content-type': 'text/html; charset=utf-8' })
        response.end('<!doctype html><title>Dagong OAuth</title><p>Authorization failed.</p>')
