...
export async function createSdkMcpClient(
  ...
  const client = new Client({ name: `dagong-${serverId}`, version: '0.1.0' })
  ...
  ;(client as { onerror?: (error: Error) => void }).onerror = (error) => {
    process.stderr.write(`dagong mcp[${serverId}]: transport error: ${redactSecretText(errorMessage(error))}\n`)
  }
