...
export class HybridThreadStore implements ThreadStore {
  ...
  private async maybeCompactMetadata(threadId: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `[dagong] metadata compaction skipped for ${threadId}: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
...
function warnSqlite(action: string, error: unknown): void {
  ...
  console.warn(`[dagong] hybrid sqlite ${action} failed; using JSONL fallback: ${message}`)
}
