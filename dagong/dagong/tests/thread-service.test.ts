...
import { DEFAULT_KUN_MODEL } from '../src/config/dagong-config.js'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'dagong-todos-'))
    try {
      const relativePath = '.dagongsdd/plan/demo.md'
...
describe('ThreadService.resumeSession', () => {
  it('uses the Dagong default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
