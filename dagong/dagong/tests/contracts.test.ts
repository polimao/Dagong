    ...
    const todos = ThreadTodoListSchema.parse({
      ...
      items: [{
        ...
        source: {
          kind: 'plan',
          planId: 'plan_1',
          relativePath: '.dagongsdd/plan/plan.md',
          ordinal: 0,
          contentHash: 'abc'
        },
        createdAt: '2026-06-03T00:00:00.000Z',
...
describe('cli', () => {
  ...
  it('loads serve and context compaction settings from an explicit config file', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dagong-config-'))
    try {
      const configPath = join(dir, 'dagong.config.json')
  ...
  it('rejects a defaultProfile that is not defined in profiles', () => {
    expect(() => DagongCapabilitiesConfig.parse({
  ...
  it('surfaces subagent profiles and policy in the runtime capability manifest', () => {
    const manifest = buildRuntimeCapabilityManifest({
      ...
      config: DagongCapabilitiesConfig.parse({
        subagents: {
          enabled: true,
          maxParallel: 2,
          maxChildRuns: 6,
  ...
  it('loads config.json from the data dir when present', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'dagong-data-'))
    try {
  ...
  it('exposes a usage string', () => {
    expect(SERVE_USAGE).toContain('dagong serve')
  })
