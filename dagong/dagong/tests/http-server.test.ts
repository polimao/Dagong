...
describe('HTTP server', () => {
  let dataDir = ''
  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'dagong-http-'))
  })
  afterEach(async () => {
  ...
  it('returns 200 on /health without auth', async () => {
    ...
    expect(body).toEqual({ status: 'ok', service: 'dagong', mode: 'serve' })
  })
  ...
  it('resumes a persisted session into a new Dagong thread', async () => {
    const h = buildHarness()
    await h.threadService.create(
      { workspace: '/tmp/project', model: 'deepseek-chat', mode: 'agent' },
