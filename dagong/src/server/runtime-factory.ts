...
import { KUN_SYSTEM_PROMPT } from '../prompt/dagong-system-prompt.js'
...
export type DagongServeRuntimeOptions = {
  host: string
  port: number
  configPath?: string
  dataDir: string
  ...
  capabilities?: DagongCapabilitiesConfig
...
export type DagongServeHandle = NodeHttpServerHandle & {
 ...
 */
export async function createDagongServeRuntime(
  options: DagongServeRuntimeOptions
): Promise<ServerRuntime> {
  await mkdir(options.dataDir, { recursive: true })
  let activeOptions: DagongServeRuntimeOptions = { ...options }
  ...
  let prefix = createImmutablePrefix({
    ...
    pinnedConstraints: [
      ...
      'system: keep the stable Dagong prefix byte-stable for prompt-cache reuse'
    ]
  })
  ...
  const withBackgroundShellTools = (
    tools: LocalTool[],
    optionsForTools: DagongServeRuntimeOptions = activeOptions
  ): LocalTool[] => {
  ...
  // subscription set as the main model). dagong-process signals that via env so we
...
function agentSdkProviderIdsForOptions(options: DagongServeRuntimeOptions): Set<string> {
...
function agentSdkProviderSignature(options: DagongServeRuntimeOptions): string {
  return [...agentSdkProviderIdsForOptions(options)].sort().join('\n')
}

function mergeRuntimeConfigApplyOptions(
  current: DagongServeRuntimeOptions,
  request: RuntimeConfigApplyRequest
): DagongServeRuntimeOptions {
...
function tokenEconomyConfigForOptions(
  options: Pick<DagongServeRuntimeOptions, 'tokenEconomyMode' | 'tokenEconomy'>
): TokenEconomyConfig {
  return {
...
function toolOutputLimitsForOptions(
  options: Pick<DagongServeRuntimeOptions, 'toolOutputLimits'>
...
function builtinToolOptionsForOptions(options: DagongServeRuntimeOptions) {
...
export async function startDagongServe(
  options: DagongServeRuntimeOptions
): Promise<DagongServeHandle> {
  const runtime = await createDagongServeRuntime(options)
  ...
  void runtime.turnService
    ...
    .then(async (threadIds) => {
      if (threadIds.length > 0) {
        console.warn(`[dagong] marked orphaned turn(s) on ${threadIds.length} thread(s) as failed after restart`)
      }
      if (threadIds.length > 0 && runtime.resumeInterruptedGoals) {
        const resumed = await runtime.resumeInterruptedGoals(threadIds)
        if (resumed > 0) {
          console.warn(`[dagong] auto-resumed ${resumed} interrupted goal(s) after restart`)
        }
      }
    })
    .catch((error) => {
      console.warn('[dagong] orphaned turn reconciliation failed:', error)
    })
  ...
  void runtime.delegationRuntime
    ...
    .then((count) => {
      if (count > 0) {
        console.warn(`[dagong] marked ${count} orphaned subagent run(s) as failed after restart`)
      }
    })
    .catch((error) => {
      console.warn('[dagong] orphaned child-run reconciliation failed:', error)
    })
  return {
    ...server,
    runtime,
