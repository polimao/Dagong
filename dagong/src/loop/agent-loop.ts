...
import { buildToolPreferenceInstruction } from '../prompt/dagong-system-prompt.js'
import {
...
export type AgentLoopOptions = {
  ...
  /** Dagong runtime data root for sandbox-safe background shell output reads. */
   ...
   * Agent SDK instead of dagong's own model loop, billing the user's Claude
   * subscription. dagong's tools/persona/permissions are injected into the SDK.
   */
...
export class AgentLoop {
  ...
  async runTurn(threadId: string, turnId: string): Promise<'completed' | 'failed' | 'aborted'> {
    ...
    // the user's subscription; dagong's brain is injected). All other providers
    // fall through to dagong's native loop below.
    ...
    try {
      ...
      // instead of the bare "Dagong turn failed" string. See issue #26.
  ...
  private async modelStep(
    ...
    const baseRequest: ModelRequest = {
      ...
      // (child-agent-executor) — so the agent keeps dagong's tool/safety
  ...
  static defaultPrefix(): ImmutablePrefix {
    return createImmutablePrefix({
      systemPrompt: 'You are Dagong, a careful and helpful assistant.',
...
function buildToolCatalogDriftMessage(toolCatalog: {
  ...
  const policy = changeKind === 'additive'
    ? 'Only additive tool changes are allowed in-place; Dagong will continue with the refreshed tool list.'
    : 'Non-additive tool changes can invalidate prompt-cache assumptions; Dagong stopped this turn. Start a new thread after editing, removing, or reordering tool schemas.'
  return [
