...
const KEYCHAIN_SERVICE = 'dagong-secret-key'
const KEYCHAIN_ACCOUNT = 'dagong'
...
async function tryWriteOsKey(platform: NodeJS.Platform, run: CommandRunner, key: Buffer): Promise<boolean> {
  ...
  try {
    ...
    if (platform === 'linux') {
      const res = await run('secret-tool', ['store', '--label=dagong secret key', 'service', KEYCHAIN_SERVICE, 'account', KEYCHAIN_ACCOUNT], b64)
      return res.code === 0
    }
  } catch {
    return false
