...
import {
  dagongConfigPathForDataDir,
  readDagongConfigFile,
  readOptionalDagongConfigFile,
  type LoadedDagongConfig
} from '../config/dagong-config.js'

/**
 * Parse the `dagong serve` command line into validated options.
 *
 ...
 * main process when Dagong is started programmatically.
 */
...
export const SERVE_USAGE = `dagong serve [options]
...
function loadServeConfig(
  ...
  env: Record<string, string | undefined>
): LoadedDagongConfig | null {
  const explicitConfigPath =
    stringFlag(raw, 'config') ??
    stringFlag(raw, 'config-file') ??
    env.KUN_CONFIG
  if (explicitConfigPath) {
    return readDagongConfigFile(explicitConfigPath)
  }
  const dataDir = dataDirFromRawOrEnv(raw, env)
  return readOptionalDagongConfigFile(dagongConfigPathForDataDir(dataDir))
}
