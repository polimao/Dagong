...
const TOOL_DESCRIPTION = [
  ...
  '.dagongsdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .dagongsdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.dagongsdd/plan/foo.md`)
 ...
 *    `.dagongsdd/plan/<feature>.md` under the active workspace.
...
function resolveReservedTarget(
  ...
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .dagongsdd/plan' }
  }
 ...
 * allocates a fresh, non-colliding `.dagongsdd/plan/<feature>.md`.
 */
