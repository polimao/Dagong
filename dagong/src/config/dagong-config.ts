...
export const DagongServeConfigSchema = z
  .object({
...
export const DagongConfigSchema = z
  .object({
    serve: DagongServeConfigSchema.optional(),
    models: ModelConfigSchema.optional(),
    contextCompaction: ContextCompactionConfigSchema.optional(),
    runtime: RuntimeTuningConfigSchema.optional(),
    roles: RolesConfigSchema.optional(),
    capabilities: DagongCapabilitiesConfig.default(DEFAULT_KUN_CAPABILITIES_CONFIG),
    hooks: HooksConfigSchema.optional(),
    quality: QualityConfigSchema.optional()
  })
  .strict()

export type DagongConfig = z.infer<typeof DagongConfigSchema>
export type QualityConfig = z.infer<typeof QualityConfigSchema>
export const DEFAULT_QUALITY_CONFIG: QualityConfig = QualityConfigSchema.parse({})
export type DagongServeConfig = z.infer<typeof DagongServeConfigSchema>
...
export type LoadedDagongConfig = {
  path: string
  config: DagongConfig
}

export function readDagongConfigFile(path: string): LoadedDagongConfig {
  ...
  try {
    ...
    throw new Error(`Failed to parse Dagong config JSON at ${resolvedPath}: ${message}`)
  }
  const parsed = DagongConfigSchema.safeParse(json)
  if (!parsed.success) {
    throw new Error(
      `Invalid Dagong config at ${resolvedPath}: ${JSON.stringify(parsed.error.issues, null, 2)}`
    )
  }
  return { path: resolvedPath, config: parsed.data }
}

export function readOptionalDagongConfigFile(path: string | undefined): LoadedDagongConfig | null {
  if (!path) return null
  const resolvedPath = expandHomePath(path)
  if (!existsSync(resolvedPath)) return null
  return readDagongConfigFile(resolvedPath)
}

export function dagongConfigPathForDataDir(dataDir: string | undefined): string | undefined {
