...
describe('buildClaudeSystemPrompt', () => {
  test('appends dagong persona onto the claude_code preset', () => {
    const sp = buildClaudeSystemPrompt('You are dagong.', 'Persona: terse.')
    expect(sp).toEqual({ type: 'preset', preset: 'claude_code', append: 'You are dagong.\n\nPersona: terse.' })
  })
  test('omits persona when absent', () => {
    expect(buildClaudeSystemPrompt('You are dagong.').append).toBe('You are dagong.')
  })
})
...
describe('buildCanUseTool', () => {
  test('allow passes through, with optional updatedInput', async () => {
    ...
    // when dagong doesn't rewrite, it echoes the original input through.
...
describe('assembleSdkOptions', () => {
  const base = {
    ...
    bridgedToolModelNames: ['mcp__dagong__generate_image', 'mcp__dagong__memory_create'],
    baseEnv: { ANTHROPIC_API_KEY: 'k', PATH: '/bin' },
    oauthToken: 'sk-ant-oat01-z'
  }

  test('unions SDK built-ins with bridged dagong tools and turns on partial streaming', () => {
    ...
    expect((opts.systemPrompt as { append: string }).append).toBe('You are dagong.')
