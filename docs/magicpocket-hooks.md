# MagicPocket Hooks 体系

本文是 MagicPocket agent runtime hook 体系的完整参考：设计动机、六个阶段的
...
重构后 hook 成为 MagicPocket 对外开放的第一类扩展机制，与 MCP、skills、
memory、subagents 并列。
...
4. **全运行时一致**：主循环、子代理（delegation）、CLI
   （`magicpocket serve` / `magicpocket exec`）共用同一套 hook。

## 模块与装配链

```text
magicpocket/src/hooks/hook-engine.ts    # 阶段、载荷、结果类型；匹配器；执行器
magicpocket/src/hooks/hook-config.ts    # config.json 的 zod schema + resolveConfiguredHooks
magicpocket/src/hooks/index.ts          # 包导出（magicpocket/hooks 子路径）
...
GUI 通过 `--data-dir` 启动 MagicPocket，`{dataDir}/config.json` 自动加载，
...
命令 hook 以 MagicPocket runtime 的权限执行任意 shell 命令。`config.json`
...
- 引擎单测：`magicpocket/tests/hooks.test.ts`（匹配器、链式、退出码协议、
  超时、auto-approve）。
- 循环集成：`magicpocket/tests/hooks-lifecycle.test.ts`（TurnStart/TurnEnd
  载荷、deny 落盘、`<hook-context>` 注入、PreCompact、观察者容错）。
- 工具宿主接入点：`magicpocket/src/adapters/tool/local-tool-host.ts`
  （`execute` 的 pre/post 段）。
- 循环接入点：`magicpocket/src/loop/agent-loop.ts`
