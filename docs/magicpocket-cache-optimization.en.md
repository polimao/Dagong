# MagicPocket cache optimization technical documentation

This article records the cache optimization design, implementation location, and implementation of the current MagicPocket runtime.
...
MagicPocket's cache optimization serves four goals:
...
- Let the GUI only bear the responsibility of HTTP/SSE calls, and consolidate the caching discipline inside MagicPocket.
...
MagicPocket therefore treats token optimization as a combined strategy, not a
...
MagicPocket's cache-first design is adapted to the GUI scenario:

- The GUI does not spell prompt, and does not make cache judgments in renderer or main process.
- `magicpocket serve` is the only request exit, and cache-related strategies are placed inside the runtime.
...
MagicPocket uses a separate stable system prompt word file:

- `magicpocket/src/prompt/magicpocket-system-prompt.ts`

This prefix only carries long-term stable operation contracts, for example:

- MagicPocket identity
- GUI call boundaries
...
MagicPocket manages system prompts, tools, and pinned constraints through `ImmutablePrefix`
...
The toolset itself is part of the prompt prefix. MagicPocket will do the following before sending a request:
...
- `magicpocket/src/adapters/model/compat-model-client.ts`
- `magicpocket/src/cache/tool-catalog-fingerprint.ts`
- `magicpocket/src/loop/agent-loop.ts`
...
MagicPocket currently performs a shared model history repair on messages at the model request boundary:
...
- `magicpocket/src/domain/model-history-repair.ts`
- `magicpocket/src/adapters/model/compat-model-client.ts`
- `magicpocket/src/loop/agent-loop.ts`

MagicPocket will also do a shared layer of history hygiene at model request boundaries:
...
- `magicpocket/src/loop/request-history-hygiene.ts`
- `magicpocket/src/loop/agent-loop.ts`

Repeat-loop guard is also enabled within the same turn:

- The third identical `(toolName, arguments)` will be suppressed.
- MagicPocket will write an error `tool_result` to let the model converge to a narrower query or explain the reason.
...
- `magicpocket/src/loop/tool-storm-breaker.ts`
- `magicpocket/src/loop/agent-loop.ts`
...
- `magicpocket/src/domain/model-history-repair.ts`- `magicpocket/src/services/thread-service.ts`
...
MagicPocket's cache hit statistics preferentially use provider-native usage fields:
...
- `magicpocket/src/telemetry/usage-counter.ts`
- `magicpocket/src/domain/usage.ts`

MagicPocket will also use a single round of real `prompt_tokens` as the compaction pressure for the next request.
...
- `magicpocket/src/loop/agent-loop.ts`
- `magicpocket/src/loop/context-compactor.ts`
...
- `magicpocket/src/services/usage-service.ts`
...
MagicPocket has covered these key behaviors:
...
- `magicpocket/tests/cache.test.ts`
- `magicpocket/tests/model-client.test.ts`

### 2. Runtime usage API

The GUI uses a unified interface to read usage through MagicPocket:
...
MagicPocket actual measurement results on 2026-06-02:
...
This document emphasizes a production-level constraint: cache optimization belongs to MagicPocket, not the GUI.
...
MagicPocket cache mechanisms that have been implemented:
...
- Architecture overview: `docs/magicpocket-architecture.md`
- MagicPocket Contributing Guide: `docs/magicpocket-contributing.md`
- MagicPocket usage instructions: `magicpocket/README.md`
