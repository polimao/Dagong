...
of Dagong's requirement -> design -> plan -> code -> verify loop, sharing the same
...
- **On disk**: each artifact is a directory `.dagong-design/<id>/`:
...
- **Durability**: the artifact list used to be in-memory only and was lost on reload.
  ...
  `window.dagongGui.listWorkspaceDirectory('.dagong-design')` — reading each `meta.json`,
...
1. **Implement in code** (`implementDesignInCode`) — publishes the shared design system
   to `.dagong-design/DESIGN_SYSTEM.md`, builds an implement prompt, opens a **fresh code
...
- **Node kinds**:
  ...
  - `design` — generates an HTML artifact at `.dagong-design/<graphId>/<nodeId>.html`.
  - `image` — generates an image at `.dagong-design/<graphId>/<nodeId>.png` (multimodal).
...
`src/main/skill-bundled.ts` seeds a built-in skill into `~/.dagong/skills/design-system/`
...
Dagong should align by strengthening the same workflow spine while keeping the
...
4. **DESIGN.md compatibility**: keep `.dagong-design/DESIGN_SYSTEM.md` as the local
   source of truth. Dagong now exports a project-level `.dagong-design/DESIGN.md`
...
4. Inject the server into the Dagong config in `dagong-process.ts` (mirror
   `buildGuiScheduleDagongMcpServer`), and package the entry via `package.json`.
