# MagicPocket Contribution Guide: Architecture, design patterns and code organization ideas

> Applicable to: Any engineer who wants to contribute code to the `magicpocket/` package.
> Reading path: For your first contribution, you can read the entire text in order; come back later to find specific chapters.
> Supporting documents:[`magicpocket-architecture.md`](./magicpocket-architecture.md)
...
**MagicPocket is a strict implementation of Ports & Adapters + Functional Core / Imperative
...
MagicPocket's directory itself is a hexagonal physical layout:
...
MagicPocket uses multiple classic patterns simultaneously, which reinforce each other rather than conflict.
...
Definition (`magicpocket/src/ports/model-client.ts`):
...
Implementation (`magicpocket/src/adapters/model/compat-model-client.ts`)
Parse HTTP+SSE into a sequence of `ModelStreamChunk`.

Tests (`magicpocket/tests/ports.test.ts`) are injected directly with `makeFakeModel`
...
```ts
// magicpocket/src/cache/immutable-prefix.ts
...
```ts
// magicpocket/src/services/turn-service.ts
...
```ts
// magicpocket/src/contracts/items.ts
...
`magicpocket/src/contracts/events.ts`). This discipline allows SSE
...
```ts
// magicpocket/src/loop/agent-loop.ts
...
Test case (`magicpocket/tests/loop.test.ts`):
...
```ts
// magicpocket/src/services/runtime-event-recorder.ts (excerpt)
...
of `magicpocket/src/adapters/file/file-thread-store.ts`
...
- `magicpocket/src/server/routes/index.ts`:`buildRouter(runtime)`
  Routing table + authentication + handlers are installed together;
- serve startup files such as `magicpocket/src/server/serve.ts`: put all
  The port implementation (in-memory or file-backed) is installed and plugged into `buildRouter`.
- `magicpocket/src/cli/serve.ts` is the composition root of CLI and is only responsible for
...
| --- | --- | --- |
| contracts (zod) | Schema parsing and validation failed | `magicpocket/tests/contracts.test.ts` |
| domain/cache | Pure functional behavior | `magicpocket/tests/cache.test.ts` |
| ports (in-memory) | interface contract | `magicpocket/tests/ports.test.ts` |
| loop | Run full loop with in-memory fakes | `magicpocket/tests/loop.test.ts` |
| server | Use ephemeral port + fetch / EventSource to run HTTP contract | `magicpocket/tests/http-server.test.ts` |
| adapters (file) | tmp directory, append / atomic / corrupted JSONL fault tolerance | `magicpocket/tests/loop.test.ts` (FileSessionStore part) |
...
- `docs/magicpocket-architecture.md` describes "why it is designed this way";
- `magicpocket/README.md` description CLI/env/data dir;
- `docs/magicpocket-contributing.md` (this article) describes "how to contribute";
- `docs/AGENTS.md` describes which files should be changed when extending.

**Contribution Rules**:

- Any new/modified contract must update the `magicpocket/README.md` side at the same time
  point table;
...
1. **contracts**:`magicpocket/src/contracts/items.ts` is already supported
   `tool_call` / `tool_result`, **No need to change**.
2. **domain**:`magicpocket/src/domain/item.ts` already exists
   `makeToolCallItem` / `makeToolResultItem`, **no need to change**.
3. **adapters**: in `magicpocket/src/adapters/tool/local-tool-host.ts`
   Add a `LocalTool`:
...
4. **adapters barrel**: in `magicpocket/src/adapters/tool/local-tool-host.ts`
   Barrel already exists; just append `shellTool` to the `defaultLocalTools` array.
5. **Test**: Add one to `magicpocket/tests/ports.test.ts`
   `runs a shell tool with approval` test.
6. **Documentation**: Described in the default tools list of `magicpocket/README.md`.
...
2. **adapters**:`magicpocket/src/adapters/model/openai-compat-model-client.ts`
   Create a new one (refer to the SSE parsing logic of `compat-model-client.ts`).
3. **adapters barrel**:`magicpocket/src/adapters/index.ts` exports new classes.
4. **settings**: in `magicpocket/src/contracts/...` or the caller
   `ServeOptionsSchema` adds `modelProvider` field (if you need to switch).
5. **composition root**: serve in `magicpocket/src/server/...`
   Which adapter to inject is selected according to the settings in bootstrap.
6. **Test**: `magicpocket/tests/ports.test.ts` Add model behavior test;
   `magicpocket/tests/http-server.test.ts` adds end-to-end testing.
7. **Document**: `magicpocket/README.md` The endpoint table remains unchanged, but
...
1. **contracts**:`magicpocket/src/contracts/events.ts`:
...
3. **loop**: in `magicpocket/src/loop/context-compactor.ts` or
   The appropriate location for `agent-loop.ts` is `await this.opts.events.record({ kind: 'compaction_progress', ... })`.
4. **server**: SSE of `magicpocket/src/server/routes/events.ts`
   The encoder is already based on `RuntimeEvent.kind` routing and does not need to be changed.
5. **renderer mapping** (GUI side): `src/renderer/src/agent/magicpocket-runtime.ts`
   Dispatch to `sink.onCompactionProgress(...)` in `subscribeThreadEvents`
   (if needed). **This is a GUI package, not magicpocket**.
6. **Test**:
   - `magicpocket/tests/contracts.test.ts`: Add variant parsing test;
   - `magicpocket/tests/loop.test.ts`: Add emit assertion.
7. **Document**:`magicpocket/README.md` The endpoint table remains unchanged, only SSE events
...
1. **ports**:`ThreadStore` / `SessionStore` is already an interface,
   **No changes required**.
2. **adapters**:`magicpocket/src/adapters/sqlite/sqlite-thread-store.ts`
   Create new and implement `ThreadStore`.
3. **adapters barrel** + **composition root** in serve bootstrap
   Switch store type.
4. **Test**: `magicpocket/tests/ports.test.ts` Add a copy for SQLite
...
- [ ] run `cd magicpocket && npm run typecheck && npm test`
- [ ] run `cd .. && npm run typecheck && npm test`
- [ ] Do not modify any files other than those described in `docs/magicpocket-architecture.md`
      to "bypass" typecheck
...
- [ ] When the public contract is changed, update the `magicpocket/README.md` endpoint table
- [ ] When references/acknowledgments change, update root directories `README.md` and `README.en.md`
- [ ] When adding a new SSE event, add a line to `docs/magicpocket-architecture.md`- [ ] in the borrow map of `docs/magicpocket-architecture.md` (as in the future
...
- **Not familiar with zod discriminated union**: refer to `magicpocket/src/contracts/items.ts`
  And `magicpocket/src/contracts/events.ts`, see how to write the existing variant.
...
> **MagicPocket is a strict implementation of Ports & Adapters + Functional Core /
