...
本文记录 Dagong 桌面应用现在应该如何围绕一个专门服务 GUI 的
Dagong 改造。结论先说清楚：GUI 只保留一个 agent，唯一 ID 是
`dagong`；Code、Design、Write、连接手机都通过同一条 `dagong serve`
...
stdio/RPC 混跑多个状态机，只把 `dagong serve` 当成稳定协议。Dagong 内部使用
...
Dagong 的缓存命中率要按 provider 原生 usage 字段优先计算和优化：
...
- `dagong/src/prompt/dagong-system-prompt.ts` 是稳定前缀。它只放长期
  不变的 Dagong 运行契约，不能放 workspace、时间戳、文件片段、选中文本、
  用户动态信息或一次性工具结果。
...
超过 90%。2026-06-02 的真实 Dagong 临时线程验证：
...
- 工具集合 mutation gate：新增工具允许 append，编辑、重排、删除工具时要求
  restart 或新会话边界，避免热前缀突然全量 miss。当前 Dagong 已排序工具
...
Renderer 只应展示 Dagong。需要删除或保持删除的 UI 面包括：
...
- 设置页 provider selector：Settings -> Agents 直接展示 Dagong 配置，
...
- 删除 Dagong 之外的 diagnostics/importer 模块。用户要的是可用的单
  agent，不是运行时检测中心。

主进程现在只需要：

- `dagongRuntimeAdapter`：启动/停止 `dagong serve`、同步 config、
  计算 base URL、附加 auth header。
- `runtimeRequestViaHost`：确保 Dagong running 后转发 `/v1/*`。
- `startSse/stopSse`：按 `threadId + sinceSeq` 转发 Dagong SSE。
...
- 历史 `agentProvider` 值归一为 `dagong`。
- 历史 provider 的 port、autoStart、API key、base URL、runtime token、
  approval、sandbox、model 会种到 `agents.dagong`。
- 迁移后的落盘文件不再保留历史 provider 配置块。
- 连接手机（内部旧名 Claw）的历史 `agentThreadIds` 只折叠成
  `agentThreadIds.dagong`，不保留 per-agent map。

## Code / Design / Write / 连接手机如何走 Dagong

- Code：`DagongRuntimeProvider` 负责 list/create thread、send turn、
  steer、interrupt、compact、approval、SSE 映射。Chat UI 不知道旧
  provider。
- Design：设计工作区创建/复用 Dagong thread，设计稿、原型和设计流程图落在
  `.dagong-design/`，通过画布预览和版本记录迭代；确认后的设计可以发布
  `DESIGN_SYSTEM.md`，再打开新的 Code thread 执行实现。
- Write：写作助手和 inline completion 读取同一份 Dagong API key /
  base URL 配置。Write thread registry 只把写作线程识别为 Dagong
  thread，不再区分旧运行时会话。
- 连接手机：定时任务、飞书/Lark/微信、IM webhook 创建或复用 Dagong thread。
  代码内部仍沿用 `claw` route / settings key / runtime 文件名，作为旧命名兼容。
  `threadId` / `localThreadId` 字段只作为旧 settings 兼容字段存在，真正
  当前映射写入 `agentThreadIds.dagong`。

## GUI HTTP 功能等价面

运行时归一不是只保留聊天。Dagong 的 GUI HTTP 面必须覆盖 store/UI
已经依赖的能力：
...
- `POST /v1/sessions/{id}/resume-thread` 沿用历史 resume 路径。
  Dagong 优先从同名 thread 恢复；没有 thread 时从 session snapshot
...
- `GET /v1/usage?group_by=thread|day` 返回累计 token、turn、cache hit 数据。
  Workbench 首页和 composer 底部只消费 Dagong usage，不再打开 runtime
  insights 面板。
...
- Dagong 之外的 diagnostics/importers
...
新增能力时优先加 Dagong tool 或 HTTP endpoint，不新增 GUI 内第二个
agent。
...
5. 连接手机能保存设置、运行手动 task、把 thread id 写回 Dagong mapping。
6. Settings -> Agents 只看得到 Dagong，没有 provider switch、runtime
