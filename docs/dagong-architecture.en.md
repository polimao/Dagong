# Dagong GUI single-runtime architecture

This document describes how the Dagong desktop app should now be organized around one dedicated runtime,
`Dagong`, that serves the GUI through a single HTTP/SSE boundary.
The conclusion is clear up front: the GUI keeps one agent with the only ID
`dagong`; Code, Design, Write, and Connect phone all flow through the same `dagong serve`
HTTP/SSE boundary.
...
Renderer (React + Zustand)
  Code / Design / Write / Connect phone UI
        |
        | window.dagongGui.runtimeRequest(path, method, body)
        | window.dagongGui.startSse(threadId, sinceSeq)
        v
Preload IPC bridge
        |
        v
Main process
  ...
  process/config/port/token management only
        |
        v
dagong serve (TypeScript package)
  /health
  /v1/threads
  /v1/threads/{id}/turns
...
does not juggle multiple state machines through stdio/RPC, and treats `dagong serve`
as the stable API boundary.
Inside `dagong`, the cache-first agent loop uses immutable prompt prefixes,
append-only logs, bounded LRU/TTL caches, inflight cleanup, steering queues,
context compaction, and usage/cache telemetry.

## Cache-hit optimization

Dagong cache-hit metrics should be computed and optimized using provider-native usage fields first:
...
- `dagong/src/prompt/dagong-system-prompt.ts` is the stable prefix.
  It may only contain long-lived Dagong run contract content and must not include
...
- Tool-collection mutation policy: adding tools should be append-only; edit/reorder/remove
  ...
  Current Dagong canonicalizes schema, but this mutation policy still needs explicit product-level
  enforcement.
...
Renderer should only expose Dagong. The UI sections listed below should be removed or
kept removed:

- Agent switcher: `AgentSwitcher` is no longer shown; `AGENT_CATALOG` only includes `dagong`.
...
- Settings provider selector: `Settings -> Agents` directly edits Dagong config including:
...
- Remove diagnostic/importer modules unrelated to Dagong.

Main process now only needs:

- `dagongRuntimeAdapter`: start/stop `dagong serve`, sync config, calculate base URL,
and append auth headers.
- `runtimeRequestViaHost`: forward `/v1/*` after ensuring Dagong is running.
- `startSse` / `stopSse`: forward Dagong SSE streams keyed by `threadId + sinceSeq`.
...
- Historical `agentProvider` values normalize to `dagong`.
...
- Legacy Connect phone fields (internally still named Claw) `agentThreadIds` are collapsed
  to `agentThreadIds.dagong`; per-provider maps are not retained.

## Code / Design / Write / Connect phone flows under Dagong

- Code: `DagongRuntimeProvider` handles list/create thread, send turn,
  steer, interrupt, compact, approval, and SSE mapping.
  Chat UI does not directly know about old providers.
- Design: the design workspace creates or reuses Dagong threads, persists drafts,
  prototypes, and design graphs under `.dagong-design/`, iterates through the
  canvas, and can publish `DESIGN_SYSTEM.md` before opening a fresh Code thread
  to implement the approved design.
- Write: writing assistant and inline completion share the same Dagong API key/base URL.
  Write thread registry identifies write threads as Dagong threads only, with no legacy-runtime distinction.
- Connect phone: scheduled tasks, Feishu/Lark/WeChat, and IM webhooks create or reuse Dagong threads.
  ...
  canonical mapping is written to `agentThreadIds.dagong`.

## Functional parity in GUI HTTP path

Runtime unification is not only preserving chat.
Dagong GUI HTTP must expose the capabilities already consumed by the store/UI:
...
- `POST /v1/sessions/{id}/resume-thread` follows the historical resume path.
  Dagong should first attempt same-name thread restore, then session snapshot/JSONL reconstruction,
...
- `GET /v1/usage?group_by=thread|day` returns accumulated token/turn/cache-hit counters.
  Workbench home and composer footer consume Dagong usage only and do not open runtime
  insight panels.
...
- Diagnostics/importers outside Dagong
...
Dagong packages are organized by ports & adapters:
...
and forwards results. When adding capability, add Dagong tool or HTTP endpoint first,
...
5. Connect phone can save settings, run manual tasks, and write thread IDs back to Dagong mapping.
6. `Settings -> Agents` shows only Dagong, with no provider switch, runtime diagnostics,
