# MagicPocket 缓存优化技术文档

本文记录 MagicPocket 运行时当前的缓存优化设计、实现位置、
...
MagicPocket 的 cache-first 设计按 GUI 场景做了收束：

- GUI 不拼 prompt，不在 renderer 或 main process 做缓存判断。
- `magicpocket serve` 是唯一请求出口，缓存相关策略都放在运行时内部。
...
MagicPocket 通过 `ImmutablePrefix` 管理系统 prompt、tools、pinned constraints
...
工具集合本身就是 prompt prefix 的一部分。MagicPocket 在发送请求前会统一做：
...
MagicPocket 当前在模型请求边界对消息做一层共享的 model history repair：
...
MagicPocket 也会在模型请求边界做一层共享的 history hygiene：
...
- MagicPocket 会写入一个 error `tool_result`，让模型收敛到更窄的查询或解释原因。
...
- `magicpocket/src/domain/model-history-repair.ts`
- `magicpocket/src/services/thread-service.ts`
...
MagicPocket 的缓存命中统计优先使用 provider 原生 usage 字段：
...
MagicPocket 也会把单轮真实 `prompt_tokens` 作为下一次请求的 compaction pressure。
...
GUI 通过 MagicPocket 使用统一接口读取 usage：
...
这个文档强调一个产品级约束：缓存优化属于 MagicPocket，而不是 GUI。
...
- 架构总览：`docs/magicpocket-architecture.md`
- MagicPocket 贡献指南：`docs/magicpocket-contributing.md`
- MagicPocket 使用说明：`magicpocket/README.md`
