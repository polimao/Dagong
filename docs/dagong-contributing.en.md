# Dagong Contribution Guide: Architecture, design patterns and code organization ideas

> Applicable to: Any engineer who wants to contribute code to the `dagong/` package.
> Reading path: For your first contribution, you can read the entire text in order; come back later to find specific chapters.
> Supporting documents:[`dagong-architecture.md`](./dagong-architecture.md)
...
**Dagong is a strict implementation of Ports & Adapters + Functional Core / Imperative
...
Dagong's directory itself is a hexagonal physical layout:
...
Dagong uses multiple classic patterns simultaneously, which reinforce each other rather than conflict.
...
Definition (`dagong/src/ports/model-client.ts`):
...
Implementation (`dagong/src/adapters/model/compat-model-client.ts`)
Parse HTTP+SSE into a sequence of `ModelStreamChunk`.

Tests (`dagong/tests/ports.test.ts`) are injected directly with `makeFakeModel`
...
```ts
// dagong/src/cache/immutable-prefix.ts
...
```ts
// dagong/src/services/turn-service.ts
...
`dagong/src/contracts/events.ts`). This discipline allows SSE
...
Test case (`dagong/tests/loop.test.ts`):
...
```ts
// dagong/src/services/runtime-event-recorder.ts (excerpt)
...
of `dagong/src/adapters/file/file-thread-store.ts`
...
- `dagong/src/server/routes/index.ts`:`buildRouter(runtime)`
  Routing table + authentication + handlers are installed together;
- serve startup files such as `dagong/src/server/serve.ts`: put all
  The port implementation (in-memory or file-backed) is installed and plugged into `buildRouter`.
- `dagong/src/cli/serve.ts` is the composition root of CLI and is only responsible for
...
| --- | --- | --- |
| contracts (zod) | Schema parsing and validation failed | `dagong/tests/contracts.test.ts` |
| domain/cache | Pure functional behavior | `dagong/tests/cache.test.ts` |
| ports (in-memory) | interface contract | `dagong/tests/ports.test.ts` |
| loop | Run full loop with in-memory fakes | `dagong/tests/loop.test.ts` |
| server | Use ephemeral port + fetch / EventSource to run HTTP contract | `dagong/tests/http-server.test.ts` |
| adapters (file) | tmp directory, append / atomic / corrupted JSONL fault tolerance | `dagong/tests/loop.test.ts` (FileSessionStore part) |
...
- `docs/dagong-architecture.md` describes "why it is designed this way";
- `dagong/README.md` description CLI/env/data dir;
- `docs/dagong-contributing.md` (this article) describes "how to contribute";
- `docs/AGENTS.md` describes which files should be changed when extending.

**Contribution Rules**:

- Any new/modified contract must update the `dagong/README.md` side at the same time
  point table;
...
1. **contracts**:`dagong/src/contracts/items.ts` is already supported
   `tool_call` / `tool_result`, **No need to change**.
2. **domain**:`dagong/src/domain/item.ts` already exists
   `makeToolCallItem` / `makeToolResultItem`, **no need to change**.
3. **adapters**: in `dagong/src/adapters/tool/local-tool-host.ts`
   Add a `LocalTool`:
...
4. **adapters barrel**: in `dagong/src/adapters/tool/local-tool-host.ts`
   Barrel already exists; just append `shellTool` to the `defaultLocalTools` array.
5. **Test**: Add one to `dagong/tests/ports.test.ts`
   `runs a shell tool with approval` test.
6. **Documentation**: Described in the default tools list of `dagong/README.md`.
...
2. **adapters**:`dagong/src/adapters/model/openai-compat-model-client.ts`
   Create a new one (refer to the SSE parsing logic of `compat-model-client.ts`).
3. **adapters barrel**:`dagong/src/adapters/index.ts` exports new classes.
4. **settings**: in `dagong/src/contracts/...` or the caller
   `ServeOptionsSchema` adds `modelProvider` field (if you need to switch).
5. **composition root**: serve in `dagong/src/server/...`
   Which adapter to inject is selected according to the settings in bootstrap.
6. **Test**: `dagong/tests/ports.test.ts` Add model behavior test;
   `dagong/tests/http-server.test.ts` adds end-to-end testing.
7. **Document**: `dagong/README.md` The endpoint table remains unchanged, but
...
1. **contracts**:`dagong/src/contracts/events.ts`:
...
3. **loop**: in `dagong/src/loop/context-compactor.ts` or
   The appropriate location for `agent-loop.ts` is `await this.opts.events.record({ kind: 'compaction_progress', ... })`.
4. **server**: SSE of `dagong/src/server/routes/events.ts`
   The encoder is already based on `RuntimeEvent.kind` routing and does not need to be changed.
5. **renderer mapping** (GUI side): `src/renderer/src/agent/dagong-runtime.ts`
   Dispatch to `sink.onCompactionProgress(...)` in `subscribeThreadEvents`
   (if needed). **This is a GUI package, not dagong**.
6. **Test**:
   - `dagong/tests/contracts.test.ts`: Add variant parsing test;
   - `dagong/tests/loop.test.ts`: Add emit assertion.
7. **Document**:`dagong/README.md` The endpoint table remains unchanged, only SSE events
...
1. **ports**:`ThreadStore` / `SessionStore` is already an interface,
   **No changes required**.
2. **adapters**:`dagong/src/adapters/sqlite/sqlite-thread-store.ts`
   Create new and implement `ThreadStore`.
3. **adapters barrel** + **composition root** in serve bootstrap
   Switch store type.
4. **Test**: `dagong/tests/ports.test.ts` Add a copy for SQLite
...
- [ ] run `cd dagong && npm run typecheck && npm test`
- [ ] run `cd .. && npm run typecheck && npm test`
- [ ] Do not modify any files other than those described in `docs/dagong-architecture.md`
      to "bypass" typecheck
...
- [ ] When the public contract is changed, update the `dagong/README.md` endpoint table
- [ ] When references/acknowledgments change, update root directories `README.md` and `README.en.md`
- [ ] When adding a new SSE event, add a line to `docs/dagong-architecture.md`- [ ] in the borrow map of `docs/dagong-architecture.md` (as in the future
...
- **Not familiar with zod discriminated union**: refer to `dagong/src/contracts/items.ts`
  And `dagong/src/contracts/events.ts`, see how to write the existing variant.
...
> **Dagong is a strict implementation of Ports & Adapters + Functional Core /
