# Dagong 贡献指南:架构、设计模式与代码组织思想

> 适用对象:任何想向 `dagong/` 包贡献代码的工程师。
> 阅读路径:第一次贡献可以按顺序读完全文;之后再回来查找具体章节。
> 配套文档:[`dagong-architecture.md`](./dagong-architecture.md)
...
**Dagong 是一个严格执行 Ports & Adapters + Functional Core / Imperative
...
Dagong 同时使用了多个经典模式,它们互相加强而不是冲突。
...
定义 (`dagong/src/ports/model-client.ts`):
...
实现 (`dagong/src/adapters/model/compat-model-client.ts`)
把 HTTP+SSE 解析为 `ModelStreamChunk` 序列。

测试 (`dagong/tests/ports.test.ts`) 直接 `makeFakeModel` 注入
loop;无需任何网络。
...
`dagong/src/contracts/events.ts`)。这种纪律让 SSE
...
测试用例 (`dagong/tests/loop.test.ts`):
...
```ts
// dagong/src/services/runtime-event-recorder.ts(摘要)
...
`dagong/src/adapters/file/file-thread-store.ts` 的
...
- `dagong/src/server/routes/index.ts`:`buildRouter(runtime)` 把
  路由表 + 鉴权 + handlers 装到一起;
- `dagong/src/server/serve.ts` 之类的 serve 启动文件:把所有
  port 的实现(in-memory 或 file-backed)装好,塞给 `buildRouter`。
- `dagong/src/cli/serve.ts` 是 CLI 的 composition root,只负责
...
| 层次 | 测试类型 | 文件 |
| --- | --- | --- |
| contracts (zod) | schema 解析、validation 失败 | `dagong/tests/contracts.test.ts` |
| domain / cache | 纯函数行为 | `dagong/tests/cache.test.ts` |
| ports (in-memory) | 接口契约 | `dagong/tests/ports.test.ts` |
| loop | 用 in-memory fakes 跑全 loop | `dagong/tests/loop.test.ts` |
| server | 用 ephemeral 端口 + fetch / EventSource 跑 HTTP 合约 | `dagong/tests/http-server.test.ts` |
| adapters (file) | tmp 目录,append / atomic / 损坏 JSONL 容错 | `dagong/tests/loop.test.ts` (FileSessionStore 部分) |

如果一个改动没在某一层有覆盖,先问"该改哪一层"再加测试。

### 4.5 文档先于代码,但和代码同步

- `docs/dagong-architecture.md` 描述"为什么这样设计";
- `dagong/README.md` 描述 CLI / env / data dir;
- `docs/dagong-contributing.md`(本文)描述"如何贡献";
- `docs/AGENTS.md` 描述扩展时该改哪些文件。

**贡献规则**:
- 任何新增/修改的 contract,必须同时更新 `dagong/README.md` 端
  点表;
...
1. **contracts**:`dagong/src/contracts/items.ts` 已经支持
   `tool_call` / `tool_result`,**不需要改**。
2. **domain**:`dagong/src/domain/item.ts` 已有
   `makeToolCallItem` / `makeToolResultItem`,**不需要改**。
3. **adapters**:在 `dagong/src/adapters/tool/local-tool-host.ts`
   加一个 `LocalTool`:
   ```ts
   ...
   ```
4. **adapters barrel**:在 `dagong/src/adapters/tool/local-tool-host.ts`
   已有 barrel;只需在 `defaultLocalTools` 数组里追加 `shellTool`。
5. **测试**:在 `dagong/tests/ports.test.ts` 加一个
   `runs a shell tool with approval` 测试。
6. **文档**:在 `dagong/README.md` 的 default tools 列表里说明。
...
3. **adapters barrel**:`dagong/src/adapters/index.ts` 导出新类。
4. **settings**:在 `dagong/src/contracts/...` 或调用方
   `ServeOptionsSchema` 加 `modelProvider` 字段(若需要切换)。
5. **composition root**:在 `dagong/src/server/...` 的 serve
   bootstrap 里根据设置选择哪个 adapter 注入。
6. **测试**:`dagong/tests/ports.test.ts` 加 model 行为测试;
   `dagong/tests/http-server.test.ts` 加端到端测试。
7. **文档**:`dagong/README.md` 端点表不变,只是
   "model" 字段语义丰富。
...
3. **loop**:在 `dagong/src/loop/context-compactor.ts` 或
   `agent-loop.ts` 的合适位置 `await this.opts.events.record({ kind: 'compaction_progress', ... })`。
4. **server**:`dagong/src/server/routes/events.ts` 的 SSE
   encoder 已经基于 `RuntimeEvent.kind` 路由,**不需要改**。
5. **renderer 映射**(GUI 端):`src/renderer/src/agent/dagong-runtime.ts`
   在 `subscribeThreadEvents` 里给 `sink.onCompactionProgress(...)` 派发
   (若需要)。**这是 GUI 包,不是 dagong**。
6. **测试**:
   - `dagong/tests/contracts.test.ts`:加 variant 解析测试;
   - `dagong/tests/loop.test.ts`:加 emit 断言。
7. **文档**:`dagong/README.md` 端点表不变,只是 SSE events
   列表里加一行。
...
3. **adapters barrel** + **composition root** 在 serve bootstrap 里
   切换 store 类型。
4. **测试**:`dagong/tests/ports.test.ts` 加一份针对 SQLite 的
   合约测试(用临时数据库)。
...
- [ ] 跑 `cd dagong && npm run typecheck && npm test`
- [ ] 跑 `cd .. && npm run typecheck && npm test`
- [ ] 不修改任何 `docs/dagong-architecture.md` 描述以外的文件
      来"绕过" typecheck
...
- [ ] 复杂的设计决策(为什么 ports 这样切、为什么用某种事件流)
      写在 PR 描述里
- [ ] 改了 public contract 时,更新 `dagong/README.md` 端点表
- [ ] 借鉴/致谢有变化时,更新根目录 `README.md` 和 `README.en.md`
- [ ] 新增 SSE 事件时,给 `docs/dagong-architecture.md` 加一行
- [ ] 在 `docs/dagong-architecture.md` 的 borrow map 中(如未来
...
- **不熟悉 zod discriminated union**:参考 `dagong/src/contracts/items.ts`
  和 `dagong/src/contracts/events.ts`,看已有 variant 怎么写。
...
> **Dagong 是一个严格执行 Ports & Adapters + Functional Core /
