...
This is the full reference for the MagicPocket agent runtime hook system:
...
4. **One system everywhere**: the main loop, delegated subagents, and
   the CLI (`magicpocket serve` / `magicpocket exec`) share the same hooks.

## Modules and wiring

```text
magicpocket/src/hooks/hook-engine.ts    # phases, payload/result types, matcher, executors
magicpocket/src/hooks/hook-config.ts    # zod schema for config.json + resolveConfiguredHooks
magicpocket/src/hooks/index.ts          # package export (magicpocket/hooks subpath)

config.json (top-level hooks array)
  → MagicPocketConfigSchema (magicpocket/src/config/magicpocket-config.ts)
  → ServeOptions (magicpocket/src/cli/cli-options.ts, serve.ts)
  → MagicPocketServeRuntimeOptions (magicpocket/src/server/runtime-factory.ts)
...
The GUI launches MagicPocket with `--data-dir`, and `{dataDir}/config.json` is
  ...
  "hooks": [
    {
      "phase": "PreToolUse",
      "matcher": "bash",
      "command": "node ~/.magicpocket-hooks/bash-guard.js",
      "timeoutMs": 10000
    },
    {
      "phase": "PostToolUse",
      "toolNames": ["write_file", "edit_file"],
      "command": "~/.magicpocket-hooks/format-after-write.sh"
    },
    { "phase": "UserPromptSubmit", "command": "cat ~/.magicpocket-hooks/standing-context.txt" },
    { "phase": "TurnEnd", "command": "~/.magicpocket-hooks/notify-done.sh" }
  ]
}
```
...
```ts
import { LocalToolHost } from 'magicpocket/adapters'
import type { ResolvedHook } from 'magicpocket/hooks'
...
Command hooks execute arbitrary shell commands with the MagicPocket runtime's
...
- Engine unit tests: `magicpocket/tests/hooks.test.ts` (matchers, chaining,
  exit-code protocol, timeouts, auto-approve).
- Loop integration: `magicpocket/tests/hooks-lifecycle.test.ts`
...
- Tool host integration point:
  `magicpocket/src/adapters/tool/local-tool-host.ts` (pre/post sections of
  `execute`).
- Loop integration points: `magicpocket/src/loop/agent-loop.ts`
