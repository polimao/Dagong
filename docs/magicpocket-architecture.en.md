# MagicPocket GUI single-runtime architecture

This document describes how the MagicPocket desktop app should now be organized around one dedicated runtime,
`MagicPocket`, that serves the GUI through a single HTTP/SSE boundary.
The conclusion is clear up front: the GUI keeps one agent with the only ID
`magicpocket`; Code, Design, Write, and Connect phone all flow through the same `magicpocket serve`
HTTP/SSE boundary.
...
Renderer (React + Zustand)
  Code / Design / Write / Connect phone UI
        |
        | window.magicpocketGui.runtimeRequest(path, method, body)
        | window.magicpocketGui.startSse(threadId, sinceSeq)
        v
Preload IPC bridge
        |
        v
Main process
  ...
  process/config/port/token management only
        |
        v
magicpocket serve (TypeScript package)
  /health
  /v1/threads
  /v1/threads/{id}/turns
...
does not juggle multiple state machines through stdio/RPC, and treats `magicpocket serve`
as the stable API boundary.
Inside `magicpocket`, the cache-first agent loop uses immutable prompt prefixes,
append-only logs, bounded LRU/TTL caches, inflight cleanup, steering queues,
context compaction, and usage/cache telemetry.

## Cache-hit optimization

MagicPocket cache-hit metrics should be computed and optimized using provider-native usage fields first:
...
- `magicpocket/src/prompt/magicpocket-system-prompt.ts` is the stable prefix.
  It may only contain long-lived MagicPocket run contract content and must not include
...
- Tool-collection mutation policy: adding tools should be append-only; edit/reorder/remove
  ...
  Current MagicPocket canonicalizes schema, but this mutation policy still needs explicit product-level
  enforcement.
...
Renderer should only expose MagicPocket. The UI sections listed below should be removed or
kept removed:

- Agent switcher: `AgentSwitcher` is no longer shown; `AGENT_CATALOG` only includes `magicpocket`.
...
- Settings provider selector: `Settings -> Agents` directly edits MagicPocket config including:
...
- Remove diagnostic/importer modules unrelated to MagicPocket.

Main process now only needs:

- `magicpocketRuntimeAdapter`: start/stop `magicpocket serve`, sync config, calculate base URL,
and append auth headers.
- `runtimeRequestViaHost`: forward `/v1/*` after ensuring MagicPocket is running.
- `startSse` / `stopSse`: forward MagicPocket SSE streams keyed by `threadId + sinceSeq`.
...
- Historical `agentProvider` values normalize to `magicpocket`.
...
- Legacy Connect phone fields (internally still named Claw) `agentThreadIds` are collapsed
  to `agentThreadIds.magicpocket`; per-provider maps are not retained.

## Code / Design / Write / Connect phone flows under MagicPocket

- Code: `MagicPocketRuntimeProvider` handles list/create thread, send turn,
  steer, interrupt, compact, approval, and SSE mapping.
  Chat UI does not directly know about old providers.
- Design: the design workspace creates or reuses MagicPocket threads, persists drafts,
  prototypes, and design graphs under `.magicpocket-design/`, iterates through the
  canvas, and can publish `DESIGN_SYSTEM.md` before opening a fresh Code thread
  to implement the approved design.
- Write: writing assistant and inline completion share the same MagicPocket API key/base URL.
  Write thread registry identifies write threads as MagicPocket threads only, with no legacy-runtime distinction.
- Connect phone: scheduled tasks, Feishu/Lark/WeChat, and IM webhooks create or reuse MagicPocket threads.
  ...
  canonical mapping is written to `agentThreadIds.magicpocket`.

## Functional parity in GUI HTTP path

Runtime unification is not only preserving chat.
MagicPocket GUI HTTP must expose the capabilities already consumed by the store/UI:
...
- `POST /v1/sessions/{id}/resume-thread` follows the historical resume path.
  MagicPocket should first attempt same-name thread restore, then session snapshot/JSONL reconstruction,
...
- `GET /v1/usage?group_by=thread|day` returns accumulated token/turn/cache-hit counters.
  Workbench home and composer footer consume MagicPocket usage only and do not open runtime
  insight panels.
...
- Diagnostics/importers outside MagicPocket
...
MagicPocket packages are organized by ports & adapters:
...
and forwards results. When adding capability, add MagicPocket tool or HTTP endpoint first,
...
1. Open the MagicPocket desktop app.
...
5. Connect phone can save settings, run manual tasks, and write thread IDs back to MagicPocket mapping.
6. `Settings -> Agents` shows only MagicPocket, with no provider switch, runtime diagnostics,
...
8. Thread search, archive, fork/resume, and request_user_input answer/cancel flows all operate
   through MagicPocket HTTP paths.
