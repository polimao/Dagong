# Dagong cache optimization technical documentation

This article records the cache optimization design, implementation location, and implementation of the current Dagong runtime.
...
Dagong's cache optimization serves four goals:
...
- Let the GUI only bear the responsibility of HTTP/SSE calls, and consolidate the caching discipline inside Dagong.
...
Dagong therefore treats token optimization as a combined strategy, not a
...
Dagong's cache-first design is adapted to the GUI scenario:

- The GUI does not spell prompt, and does not make cache judgments in renderer or main process.
- `dagong serve` is the only request exit, and cache-related strategies are placed inside the runtime.
...
Dagong uses a separate stable system prompt word file:

- `dagong/src/prompt/dagong-system-prompt.ts`
...
Dagong manages system prompts, tools, and pinned constraints through `ImmutablePrefix`
...
The toolset itself is part of the prompt prefix. Dagong will do the following before sending a request:
...
- `dagong/src/adapters/model/compat-model-client.ts`
- `dagong/src/cache/tool-catalog-fingerprint.ts`
- `dagong/src/loop/agent-loop.ts`
...
Dagong currently performs a shared model history repair on messages at the model request boundary:
...
- `dagong/src/domain/model-history-repair.ts`
- `dagong/src/adapters/model/compat-model-client.ts`
- `dagong/src/loop/agent-loop.ts`

Dagong will also do a shared layer of history hygiene at model request boundaries:
...
- `dagong/src/loop/request-history-hygiene.ts`
- `dagong/src/loop/agent-loop.ts`

Repeat-loop guard is also enabled within the same turn:

- The third identical `(toolName, arguments)` will be suppressed.
- Dagong will write an error `tool_result` to let the model converge to a narrower query or explain the reason.
...
- `dagong/src/loop/tool-storm-breaker.ts`
- `dagong/src/loop/agent-loop.ts`
...
- `dagong/src/domain/model-history-repair.ts`- `dagong/src/services/thread-service.ts`
...
Dagong's cache hit statistics preferentially use provider-native usage fields:
...
- `dagong/src/telemetry/usage-counter.ts`
- `dagong/src/domain/usage.ts`

Dagong will also use a single round of real `prompt_tokens` as the compaction pressure for the next request.
...
- `dagong/src/loop/agent-loop.ts`
- `dagong/src/loop/context-compactor.ts`
...
- `dagong/src/services/usage-service.ts`
...
Dagong has covered these key behaviors:
...
- `dagong/tests/cache.test.ts`
- `dagong/tests/model-client.test.ts`

### 2. Runtime usage API

The GUI uses a unified interface to read usage through Dagong:
...
Dagong actual measurement results on 2026-06-02:
...
This document emphasizes a production-level constraint: cache optimization belongs to Dagong, not the GUI.
...
Dagong cache mechanisms that have been implemented:
...
- Architecture overview: `docs/dagong-architecture.md`
- Dagong Contributing Guide: `docs/dagong-contributing.md`
- Dagong usage instructions: `dagong/README.md`
